...
static int lavf_check_file(demuxer_t *demuxer){
    ...
    //Polun 2011-12-2 ++s fixed amr file smaller than 32kB size  can't playback
    int i;
    if(demuxer->filename)
    {
        for(i=0;i<strlen(demuxer->filename);i++)
        {
            if(*(demuxer->filename + i) == '.')
            {
                if((*(demuxer->filename + i +1) == 'a' && *(demuxer->filename + i + 2) == 'm' && *(demuxer->filename + i +3) == 'r'  ) ||
                   (*(demuxer->filename + i +1) == 'A' && *(demuxer->filename + i + 2) == 'M' && *(demuxer->filename + i +3) == 'R'  ) )
                {
                    if(demuxer->stream->buf_len < read_size)
                        read_size = (8*1024);
                }
            }
        }
...
//Fuchun 2010.02.10
static const char *detect_mov = "mov,mp4,m4a,3gp,3g2,mj2";
...
static int lavf_check_preferred_file(demuxer_t *demuxer){
	...
	//Fuchun 2010.02.10
	if(strcmp(detect_mov, priv->avif->name) == 0 && demuxer->stream->type == STREAMTYPE_STREAM)
		return DEMUXER_TYPE_LAVF_PREFERRED;
    }
    return 0;
}
...
#if 1	//Fuchun 2011.03.23 fix wrong fps when stream type is streaming
		if(sh_video->fps > 59.0 && demuxer->stream->type == STREAMTYPE_STREAM)
		{
			sh_video->fps /= 2;
		}
#endif
...
static demuxer_t* demux_open_lavf(demuxer_t *demuxer){
	...
	first_start_code = 1;	//Fuchun 2010.09.25 reset global variable
...
#if 1	//Barry 2011-02-01
	if(priv->audio_streams)
	{
		if ( demuxer->audio && demuxer->audio->sh &&
			((sh_audio_t *)demuxer->audio->sh)->format == mmioFOURCC('s', 'a', 'm', 'r') &&
			((lavf_priv_t *)demuxer->priv)->avfc->duration == AV_NOPTS_VALUE
		)
		{
			off_t ori_pos = stream_tell(demuxer->stream);
			stream_seek(demuxer->stream, 0);
			off_t pos = 6;
			...
			// get duration
			while (pos < demuxer->movi_end)
			{
				stream_seek(demuxer->stream, pos);
				stream_read(demuxer->stream, &tmpbuf, 1);

				frame_type = (tmpbuf >> 3) & 0xf;
				frame_size = packed_size[frame_type] + 1;
				pos += frame_size;
				frame_num ++;
			}
			((lavf_priv_t *)demuxer->priv)->avfc->duration = (int)((frame_num * 0.02)*AV_TIME_BASE);
			stream_seek(demuxer->stream, ori_pos);
		}
	}
#endif
    //Polun 2011-09-05 ++s for mantis3224
    extern int num_reorder_frames;
    if(demuxer->type == DEMUXER_TYPE_LAVF_PREFERRED && demuxer->video->sh && ( (sh_video_t *)demuxer->video->sh)->format == mmioFOURCC('H', '2', '6', '4') &&
        num_reorder_frames != 0 && priv->avfc->bit_rate == 0 ){
        printf("!!!!! num_reorder_frames != 0 && priv->avfc->bit_rate == 0  ==> seekable = 0 \n");
        demuxer->seekable = 0 ;
    }
    //Polun 2011-09-05 ++e
    //Polun 2011-07-28 ++s amr audio file seekable disable
    if((!priv->video_streams) &&  demuxer->audio && demuxer->audio->sh && ((sh_audio_t *)demuxer->audio->sh)->format  == mmioFOURCC('s', 'a', 'm', 'r')) //amr formate 0x726d6173 
        demuxer->seekable = 0 ;
    //Polun 2011-07-28 ++e
    return demuxer;
}
...
//    if (av_seek_frame(priv->avfc, -1, priv->last_pts, avsflags) < 0) {		//Fuchun 2010.04.29 disable
/*
    if (av_seek_frame(priv->avfc, -1, priv->last_pts, avsflags) < 0 && strcmp("flv", priv->avif->name) != 0) {
        avsflags ^= AVSEEK_FLAG_BACKWARD;
        av_seek_frame(priv->avfc, -1, priv->last_pts, avsflags);
    }
*/
	if(av_seek_frame(priv->avfc, -1, priv->last_pts, avsflags) < 0 && speed_mult == 0)
	{
		avsflags ^= AVSEEK_FLAG_BACKWARD;
		av_seek_frame(priv->avfc, -1, priv->last_pts, avsflags);
	}
...
static int demux_lavf_control(demuxer_t *demuxer, int cmd, void *arg)
{
	...
	case DEMUXER_CTRL_SWITCH_VIDEO:
	{
		...
		if(cmd == DEMUXER_CTRL_SWITCH_AUDIO && !demuxer->audio->sh)
			return DEMUXER_CTRL_NOTIMPL;
