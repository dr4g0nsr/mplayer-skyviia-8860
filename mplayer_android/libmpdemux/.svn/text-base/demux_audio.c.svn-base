...
//Barry 2010-12-02
float mp3_dur = 0.0;
float mp3_frame_time = 0.0;
extern int stream_cache_size;
extern unsigned int upnp_duration;
...
static int demux_audio_open(demuxer_t* demuxer) {
  ...
  while(n < 30000 && !s->eof) {
	...
	step = 4;
      ...
      stream_skip(s, mp3_flen - HDR_SIZE); //Polun 2011-06-16 add for mantis3679 no this line maybe cause mp3_found is NULL                         
...
#if 1	//Barry 2010-12-02
	if ((stream_cache_size == -1 || upnp_duration) && s->seek)	//Barry 2010-12-13 //Skyviia_Vincent02152011 streaming mp3 the duration computed wrong
	{
		if (sh_audio->audio.dwRate == 22050)
	          mp3_frame_time = ((float)576/sh_audio->audio.dwRate);	
		else
	          mp3_frame_time = ((float)1152/sh_audio->audio.dwRate);

		unsigned int pos = 0;
		int mp3_freq, mp3_chans, mp3_flen, mpa_layer, mpa_spf, mpa_br, aframe_cnt=0;
		uint8_t hdr[4];
		int Max_mp3_check_size = 30*1024*1024; //30MB //Skyviia_Vincent02152011

		stream_seek(s,demuxer->movi_start);
		pos = demuxer->movi_start;

		while( !s->eof && pos < Max_mp3_check_size)
		{
			stream_seek(s, pos);
			stream_read(s, hdr, HDR_SIZE);
			if( ( mp3_flen = mp_get_mp3_header(hdr, &mp3_chans, &mp3_freq, &mpa_spf, &mpa_layer, &mpa_br) ) > 0 )
			{
				aframe_cnt++;
				pos += mp3_flen;
			}
		...
		if(pos >= Max_mp3_check_size)
			printf("mp3 file size > %d, the duration maybe wrong. demuxer->movi_start[%d]\n",Max_mp3_check_size,demuxer->movi_start);

		stream_reset(s);
		stream_seek(s,demuxer->movi_start);

		mp3_dur = aframe_cnt * mp3_frame_time;	
		mp_msg(MSGT_DEMUX,MSGL_V,"========>  Frame = %d, mp3_dur = %4.2f\n", aframe_cnt, mp3_dur);
		
		sh_audio->i_bps = (int)( ( demuxer->movi_end - demuxer->movi_start ) / mp3_dur );
		mp_msg(MSGT_DEMUX,MSGL_V,"sh_audio->i_bps = %d kbps\n", sh_audio->i_bps);
	}
#endif
...
//    printf("wav: %X .. %X\n",(int)demuxer->movi_start,(int)demuxer->movi_end);
	...
	{
#ifdef DTS_CERTIFICATION	//Barry 2011-11-02
		if(0<=SpkrOut && SpkrOut<=14 && sky_hwac3)
			DTS_in_WAV = 1;
		stream_seek(s, i+44);
		printf("\n####  [%s - %d]  DTS sync offset = %d   DTS_in_WAV = %d\n\n", __func__, __LINE__, i, DTS_in_WAV);
#endif
...
static int demux_audio_fill_buffer(demuxer_t *demux, demux_stream_t *ds) {
  ...
  case fLaC: {
    ...
    priv->next_pts += l/(double)sh_audio->i_bps;
    if (audio_speed != 0 && l == 0 && stream_tell(s) >= demux->movi_end)
		return 0;
...
static int demux_audio_control(demuxer_t *demuxer,int cmd, void *arg){
	...
	//Barry 2010-12-02
	//if (mp3_dur && priv->frmt == MP3 && stream_cache_size == -1)
	if (mp3_dur && priv->frmt == MP3) //Skyviia_Vincent02152011
		audio_length = (int)mp3_dur;
