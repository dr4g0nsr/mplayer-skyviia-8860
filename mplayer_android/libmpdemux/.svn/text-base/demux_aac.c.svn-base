...
typedef struct {
	...
	int srate_factor;		//Fuchun 2010.11.09
} aac_priv_t;
...
//		nf = time * sh_audio->samplerate/1024;
		if(priv->srate_factor == 0)
		{
			...
			if(srate == sh_audio->samplerate)
				priv->srate_factor = 1024;
			else
				priv->srate_factor = 2048;
			stream_seek(demuxer->stream, orig_pos);
		}
		nf = time * sh_audio->samplerate / priv->srate_factor;
