...
extern int FAST_MPEG_PS_FLAG, FAST_MPEG_PS_SEEK;	//Barry 2010-04-22
#include "stream/stream_dvdnav.h"
extern DvdnavState   dvdnavstate;

#ifdef jfueng_2011_0311  
extern int  have_audio_long_period;
extern int  is_vob;
#endif

float MPEG_ES_START_PTS = 0.0;	//Barry 2011-08-11
...
extern int dvdnav_audio_need_uninit;
extern int is_dvdnav;
static int mpeg_pts_error=0;
off_t ps_probe = 0;
int dvdnav_seek_to_end = 0;	//Fuchun 2011.03.14 dvdnav can't used position < 0 to detect FR_to_end

static int correct_audio_id(demuxer_t *demux, int aid) //Fuchun 20110715
{
	int cur_a_num = dvdnavstate.cur_audio_num;
	if(cur_a_num == -1)
		return aid;
	if(aid == dvdnavstate.stream_aid[cur_a_num] && aid != dvdnavstate.aid[cur_a_num])
		return dvdnavstate.aid[cur_a_num];
	return aid;
}
...
#define TIMESTAMP_PROBE_LEN 900000	//Barry 2010-12-13  900000 is a wild guess
...
static demuxer_t* demux_mpg_open(demuxer_t* demuxer) {
  ...
  mpg_demuxer_t* mpg_d;
  extern int upnp;
#ifdef ADJUST_STREAM_PTS    /* Carlos add on 2011-08-18 , mantis 3678, 5602*/
extern int first_set_sub_pts;
extern double correct_sub_pts;
#endif /* end of ADJUST_STREAM_PTS */

  //reset parameter Fuchun 2010.03.01
  mpeg_fast = 0;
  change_flag = 0;
  ...
  if(mpg_d)
  {
	...
	if(demuxer->seekable
				//&& demuxer->stream->type == STREAMTYPE_FILE || demuxer->stream->type == STREAMTYPE_STREAM
		/* Fixed upnp had mosaic at the first packet : add condition demuxer->stream->type == STREAMTYPE_STREAM , 2011-07-28 mantis : 5594 */
		&& (demuxer->stream->type == STREAMTYPE_FILE || (upnp && demuxer->stream->type == STREAMTYPE_STREAM)
    ...
    )
    {
	  ...
	  /* Fixed upnp had mosaic at the first packet : add condition demuxer->stream->type != STREAMTYPE_STREAM , 2011-07-28 mantis : 5594 */
	  if (demuxer->stream->type != STREAMTYPE_STREAM || (upnp == 1 && demuxer->stream->type == STREAMTYPE_STREAM) )
	  {
		  ...
		  if(first_pts != -1.0)
		  {
			  ...
			  if(middle_pts != -1.0)
			  {
				  float final_pts = read_first_mpeg_pts_at_position(demuxer, demuxer->movi_end - TIMESTAMP_PROBE_LEN);
                               #if 0  //Polun fixed mantis 6365 VTS_01_4.VOB Play video time bar showed error
                               if(final_pts != -1.0)
                               #else 
                               if(final_pts != -1.0 && (final_pts > middle_pts) && (middle_pts > first_pts) )
                               #endif 
				  {
					  ...
					  if((0.5 < proportion) && (proportion < 2))
					  {
						  ...
						  mpg_d->has_valid_timestamps = 1;
                              ...
                              //Polun 2011-10-24 ++s fixed mantis 6365 VTS_01_4.VOB Play video time bar showed error
                              else
                              {
                                   printf("@@@@ MPG : pts not correct ; first_pts = %f middle_pts = %f  final_pts = %f \n",first_pts,middle_pts,final_pts);
					mpg_d->first_pts = first_pts;
					mpg_d->first_to_final_pts_len = 0.0f;
					mpg_d->has_valid_timestamps = 1;
                              } 
                              //Polun 2011-10-24 ++e
			  }
...
#ifdef ADJUST_STREAM_PTS         
	  /*
	   * Fixed mantis 3678 :Resume playback caused timebar reset , root cause: set correct_sub_pts to the begin pts of the file 
	   * File name is 20MB-Hitachi HD Demo HDTV 1080i.avi
	   */
	  if (correct_sub_pts == 0.0 && mpg_d && mpg_d->first_pts > 0 && first_set_sub_pts == 1)
	  {
		  correct_sub_pts = mpg_d->first_pts;
		  first_set_sub_pts = 0;
		  //printf("#### In [%s][%d] set correct_sub_pts to [%f] ####\n", __func__, __LINE__, correct_sub_pts);
	  }
#endif /* end of ADJUST_STREAM_PTS */    
	  ...
	  /* Fixed upnp had mosaic at the first packet : add condition demuxer->stream->type != STREAMTYPE_STREAM , 2011-07-28 mantis : 5994 */
...
//JF 0721 
    int JF_non_standard_0x1BD_need_initial_new_audio_stream = 0;
...
//JF 0721 
    JF_non_standard_0x1BD_need_initial_new_audio_stream = 0;
...
//JF 0721 
    JF_non_standard_0x1BD_need_initial_new_audio_stream = 1;
  ...
  if((c>>4)==3){
    ...
    c=stream_read_char(demux->stream);
    /* 
        * 20110516 charleslin in *groupies*.mpeg (MPEG-1), the DTS prefix is not 0x1n.
        * But the pts value is still valid. Don't set_pts to 0, otherwise video pts is alway 0.
        */
#if 0
...
#else
    set_pts = 1;
    if((c>>4)!=1) printf("Warning: The prefix of DTS value is wrong:0x%x\n", c);
#endif
      ...
      if((aid & 0xE0) == 0x20){
        ...
        if(dvdnavstate.dvdnav_title_state == DVDNAV_TITLE_STATE_MENU)
        {
    		if(!dvdsub_lang && demux->sub->id == -1)
          		demux->sub->id = dvdnavstate.sid[0]; //mantis:3683
          	else
          		demux->sub->id = aid;
    	}
        else
        	 if(!dvdsub_lang && demux->sub->id == -1)
          		demux->sub->id = aid;
	if ((demux->stream->type == STREAMTYPE_DVDNAV) && (demux->sub->id >= 0) && (demux->sub->id < 32) &&dvdnavstate.dvdnav_title_state == DVDNAV_TITLE_STATE_MENU)
	{
	    if (dvdnavstate.sid[demux->sub->id] == aid)
	    {
		//printf("dvdnavstate.sid[demux->sub->id][%d],demux->sub->id[%d], aid[%d]\n",dvdnavstate.sid[demux->sub->id],demux->sub->id, aid);
		ds = demux->sub;
	    }
	} else 
		{
...
//        aid=128+(aid&0x7F);
        // aid=0x80..0xBF
        if(is_dvdnav && demux->audio->id != aid)	//Fuchun 20110715 fix trailer no sound (dvd 300)
        	aid = correct_audio_id(demux, aid);
        new_audio_stream(demux, aid);
//printf("demux->audio->id[%d], aid[%d]\n",demux->audio->id, aid);
      if(demux->audio->id==aid){
        int type;
        ds=demux->audio;
//        if(!ds->sh) ds->sh=demux->a_streams[aid];
	if(!ds->sh || ds->sh != demux->a_streams[aid])	//Fuchun 2011.04.26 add a condition
...
#if 1  //JF 0721         
if ( (   id  ==  0x1BD  )  && (demux->audio->id   != -2)   )
{   
       unsigned char stuffing=0;
	   
//JF  JF_non_standard_0x1BD_need_initial_new_audio_stream  by C=0X21

   if  ( (demux->audio->id == -1)  && (JF_non_standard_0x1BD_need_initial_new_audio_stream  ) )
      {  // aid =160; 
          new_audio_stream(demux, 160);   
          printf("\nJF Warning: Not normal audio Packet (id=0x1BD)..only with byte6=0X21\n\n");
      }
   else
   	  if ( !JF_non_standard_0x1BD_need_initial_new_audio_stream  )
	  goto 	JF_non_standard_0x1BD_exit;             //JF NOTE ::  for normal MPEG2 with DVDPCM (aid=160)  can pass through


//JF 0727 Special case ::audio Packet(id = 0x1BD)  AND byte 6 = 0X21  ///////////////////// 

//stuffing 4 byte ///////////////////////////

       stuffing=stream_read_char(demux->stream);

       //JF 0727   //2.restrict   0XA0 ~ 0XA9
       if ( ( stuffing  < 0xA0  )  || (stuffing  >  0xA9  ))
	 goto 	JF_non_standard_0x1BD_exit;       //JF NOTE : audio Packet (id = 0x1BD) ..with byte6=0X21 ,but not PCM sample  need exit!
...
//    printf("packet start = 0x%X  \n",stream_tell(demux->stream)-packet_start_pos);
    ...
    if(set_pts){
      dp->pts=pts/90000.0f;
      //printf("%s dp->pts %f\n", ds == demux->video ? "video" : "audio", dp->pts);
    }
...
int ret=0;
int null_head_cnt=0;//Skyviia_Vincent12062010 avoid too many 0x00 data
  ...
  if((head&0xFFFFFF00)!=0x100){
   ...
   while(1){
    ...
    if(head!=0x100){
     ...
     if(is_vob)	 
     {
      if ( (null_head_cnt > 1024*10000 ) && (have_audio_long_period ==0) )
      {

           //JF f(" \n have_audio_long_period = %d \n",have_audio_long_period);
    
          //don't check start code anymore 
          printf("null_head_cnt > [%d]\n",null_head_cnt);
          demux->stream->eof=1;
          null_head_cnt = 0;
          return 0;
      }
...
#if 0
if(change_flag == 1)	//Fuchun 2011.01.05
{
	change_flag = 0;
                           ...
                           float audio_delay, int flags)
{
    ...
    int found_keyframe = 0;	//Fuchun 2010.06.17

    //Barry 2011-08-11
    if (demuxer->file_format==DEMUXER_TYPE_MPEG_ES && sh_video && !sh_audio && (flags & SEEK_ABSOLUTE))
    {
    	MPEG_ES_START_PTS = rel_seek_secs;
	//printf("#####  [%s - %d]   sh_video->pts = %f\n", __func__, __LINE__, sh_video->pts);
    }
	...
	/*
	 * Fixed Mantis 5602 , newpts must be based on oldpts (the start pts) 
	 * File name is 03_The Fifth Element .VOB.
	 */
	if (flags & SEEK_ABSOLUTE)
	{
		//printf("### In [%s][%d] mpg_d[%p] newpts [%f] oldpts[%f] $$$\n", __func__, __LINE__, mpg_d, newpts, oldpts);
		//newpts += oldpts; 
		newpts += mpg_d->first_pts; //Polun 2011-12-07 fixed Ashley.Tisdale.-.[Be.Good.to.Me].MV.MPEG FR seek fail.
	}
  //================= seek in MPEG ==========================
  //calculate the pts to seek to
	...
	// time seek (secs)
        ...
        else
        {
          ...
          //Polun 2011-11-11 fixed mantis 6481 playback Formula.m2v Press FR key¡Atimebar display 00:00:00 second 
          MPEG_ES_START_PTS = (newpos -demuxer->movi_start ) / sh_video->i_bps ;
        } 
    }

    while (1) {
		//Fuchun 2010.03.01
		if (!FR_to_end && speed_mult < 0 && (newpos < 0 || dvdnav_seek_to_end == 1)){
#if 0
...
//			speed_mult = 0;
	...
	if(found_keyframe) d_video->buffer_pos -= found_keyframe;
	if(!mpg_d)
          break;
        if (!precision || abs(newpts - mpg_d->last_pts) < 0.5 || (mpg_d->last_pts == oldpts) || FR_to_end) break;
	      ...
	      if(!sh_audio)
	        return DEMUXER_CTRL_NOTIMPL;
              if (*((int*)arg) < 0)
              {
              	...
              	if(is_dvdnav)
              	{
              		int cur_aid=0,next_aid=0;
              		
              		tmp_skip = 1;
              		for (cur_aid = 0; cur_aid < 8; cur_aid++) {
                		if (d_audio->id == dvdnavstate.aid[cur_aid]) break;
              		}
                	
              		for (i = 1; i < 8; i++) {
                		if (dvdnavstate.aid[(cur_aid+i)%8] != -1)
                		{
                			next_aid = (cur_aid+i)%8;
                			break; 
                		}
              		}
//printf("=== In[%s][%d]   next_aid:%d  dvdnavstate.aid:%d ===\n", __func__, __LINE__, next_aid, dvdnavstate.aid[next_aid]);
              		sh_a = (sh_audio_t*)demuxer->a_streams[dvdnavstate.aid[next_aid]];
              		d_audio->id = dvdnavstate.aid[next_aid];
                	d_audio->sh = sh_a;
              ...
              if (i < mpg_d->num_a_streams && d_audio->id != mpg_d->a_stream_ids[i] && tmp_skip==0) {
                d_audio->id = mpg_d->a_stream_ids[i];
                d_audio->sh = sh_a;
                ds_free_packs(d_audio);
              }
            }
//printf("++++control++++demuxer->audio->id[%d]\n",demuxer->audio->id);
...
static demuxer_t* demux_mpg_ps_open(demuxer_t* demuxer)
{
    ...
    if(demuxer->audio->id >= 0 || ((demuxer->stream->type == STREAMTYPE_DVDNAV) && (demuxer->audio->id != -2))) {
        ...
        printf("No audio stream found.audio->id[%d]\n",demuxer->audio->id);
