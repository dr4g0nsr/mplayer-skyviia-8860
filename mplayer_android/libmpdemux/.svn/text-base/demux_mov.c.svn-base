...
typedef struct {
    ...
    //int desc;            // for multiple codecs mode - not used //Polun  2011.05.11 remove
    //off_t pos;
    unsigned int pos; //Polun  2011.05.11 modify type form off_t to unsigned int
} mov_chunk_t;

//Polun  2011.05.11
//add for chunk ops high dword to reduce chunk struct size on QT
typedef struct{
	unsigned int idx;
	unsigned int hpos;
}mov_chunkidx_t;	
...
//for FF/FB Fuchun 2010.02.24
static int packet_cnt;
static int forward_keyframe_idx=0;	
static int backward_keyframe_idx=-1;
int Only_One_I = 0;
int mov_keyframe_size = 0;	//Fuchun 2010.03.01
int is_FD;		//Fuchun 2010.02.11
off_t file_size;
int last_frame;	//Fuchun 2010.02.24
int ss_trak_pos = 0;	//Barry 2010-04-26
extern unsigned int no_osd;
extern int AAC_MAIN_PROFILE;
int MOV_3GP5_flg = 0;//Polun 2011-11-09 fixed mantis 6447 jpn_engSUB_sample.3gp no video output

//extern unsigned char* videobuffer;	// Raymond 2009/04/28
extern int check_avc1_sps_bank0(unsigned char *in_buf, int len);

#define BE_16(x) (((unsigned char *)(x))[0] <<  8 | ((unsigned char *)(x))[1])
		  
static void mov_build_index(mov_track_t* trak,long long timescale){
    int i,j,s;
    ...
    mp_msg(MSGT_DEMUX, MSGL_V, "pts=%llu  scale=%llu  time=%5.3f\n",trak->length,trak->timescale,(float)trak->length/(float)trak->timescale);
    ...
    // calc sample offsets
    s=0;
#if 1	//Fuchun 2010.02.23		fault tolerant
	for(j=0;j<trak->chunks_size;j++){
		int oversize = 0;
		#if 0
		off_t pos=trak->chunks[j].pos;
              #else  //Polun  2011.05.11  chunk pos process 
	       off_t pos;
		int checknum;
		checknum = trak->chunkidxsize;
		while(checknum)
		{
                    if(j >= trak->chunkidx[checknum-1].idx)
                    	{
                           pos = (off_t)trak->chunkidx[checknum-1].hpos;
			      pos = (pos<<32)+(off_t)trak->chunks[j].pos;	
			      break;
			}
                    checknum--;
	       }
...
typedef struct {
    ...
    long long timescale; // movie timescale
    long long duration;  // movie duration (in movie timescale units)
    subtitle subs;
...
static int mov_check_file(demuxer_t* demuxer){
    ...
    MOV_3GP5_flg = 0;//Polun 2011-11-09 fixed mantis 6447 jpn_engSUB_sample.3gp no video output
	  ...
	  switch(tmp) {
	    ...
	    case MOV_FOURCC('3','g','p','5'):
	      mp_msg(MSGT_DEMUX,MSGL_INFO,"ISO: File Type Major Brand: 3GPP Profile 5\n");
             MOV_3GP5_flg = 1;//Polun 2011-11-09 fixed mantis 6447 jpn_engSUB_sample.3gp no video output
     	      break;
...
static int gen_sh_audio(sh_audio_t* sh, mov_track_t* trak, long long timescale) {
#if 0
		struct {
...
#if 1	//Barry 2011-05-22
                             //Polun 2011-08-04 for mantis 5663 HE-AAC V2 main profile is not support 
				//else if(esds.objectTypeId==MP4OTI_MPEG4Audio && esds.streamType==0x15 && sh->format==0x6134706D && esds.decoderConfigLen >= 2)
				else if(( esds.objectTypeId==MP4OTI_MPEG2AudioLowComplexity || esds.objectTypeId==MP4OTI_MPEG4Audio) && esds.streamType==0x15 && sh->format==0x6134706D && esds.decoderConfigLen >= 2)
				{
					int aac_object_type = (esds.decoderConfig[0]>>3) & 0x1F;
					if (aac_object_type == 1)	//main profile
					{
						printf("***********  [%s - %d]   aac_object_type = %d      esds.decoderConfig[0]=0x%X\n", __func__, __LINE__, aac_object_type, esds.decoderConfig[0]);
						//AAC_MAIN_PROFILE = 1;
						return 0; //Polun 2011-08-04 for mantis 5663 HE-AAC V2 main profile is not support return 0;
					}
				}
#endif
			...
			if(atom_len > 8) {
			    ...
			    memcpy(sh->codecdata, &trak->stdata[28], sh->codecdata_len);
                      ...
                      case MOV_FOURCC('d','a','m','r'):
                        ...
                        break;
#if 1	//Barry 2011-09-01 mantis:5973 fix iphone video playback without sound
		      case MOV_FOURCC('f','r','m','a'): {
			  	mp_msg(MSGT_DEMUX, MSGL_V, "MOV: Found frma atom (%d)!\n", atom_len);
			  	if (char2int(trak->stdata,36+adjust) == MOV_FOURCC('m','p','4','a'))
			  	{
					int z;
					for (z=36+adjust ; z<trak->stdata_len-4 ; z++)
					{
						if (trak->stdata[z] == 'e' && trak->stdata[z+1] == 's' && trak->stdata[z+2] == 'd' && trak->stdata[z+3] == 's')
							break;
					}
					if (z == (trak->stdata_len-4))
						break;

					if((trak->stdata_len - z) > 8)
					{
						esds_t esds;
						int esds_len = char2int(trak->stdata, z-4);

						if(!mp4_parse_esds(&trak->stdata[z+4], esds_len-8, &esds))
						{
							//printf("#####  [%s - %d]   esds.objectTypeId = 0x%X\n", __func__, __LINE__, esds.objectTypeId);
							if(( esds.objectTypeId==MP4OTI_MPEG2AudioLowComplexity || esds.objectTypeId==MP4OTI_MPEG4Audio) && esds.streamType==0x15 && sh->format==0x6134706D && esds.decoderConfigLen >= 2)
							{
								int aac_object_type = (esds.decoderConfig[0]>>3) & 0x1F;
								if (aac_object_type == 1)	//main profile
								{
...
static int gen_sh_video(sh_video_t* sh, mov_track_t* trak, long long timescale) {
		int depth, i, entry;
		  ...
		  while(pos+8<=trak->stdata_len &&
		   ...
		   switch(char2int(trak->stdata,pos+4)) { // switch atom type
		    ...
		    case MOV_FOURCC('a','v','c','C'):
		      ...
		      if(atom_len > 8) {
		        int i, poffs, cnt;
			int spos = 0, slen = 0, nal_length_size = 0;//, nal_size = 0;	
			...
			nal_length_size = ((*(trak->stdata+pos+12))&0x03)+1;
			trak->stream_header_len = 0;
			trak->stream_header = malloc(atom_len);

			//Fuchun 2010.02.11
			if(nal_length_size == 2) is_FD = 1;
			else is_FD = 0;
			
		        for (i = 0; i < cnt; i++) 
			{
#if 1		          
				slen = BE_16(trak->stdata+poffs);
				printf("slen = %d\n", slen);

				trak->stream_header[spos] = 0x0;
				trak->stream_header[spos+1] = 0x0;

#if 1	//Fuchun 2010.02.12
				if(is_FD)
				{
					spos += nal_length_size;
					memcpy(trak->stream_header + spos, trak->stdata+poffs, slen+2);
					spos+=2;
				}
				else
				{
					trak->stream_header[spos+2] = *(trak->stdata+poffs);
					trak->stream_header[spos+3] = *(trak->stdata+poffs+1);
					spos += nal_length_size;
					memcpy(trak->stream_header + spos, trak->stdata+poffs+2, slen);
				}
#endif					
				spos += slen;
				trak->stream_header_len += slen + 4;
#endif			
...
#if 1
			printf("trak->stream_header_len = %d\n", trak->stream_header_len);
			//Barry 2011-03-25
			if ( (trak->stream_header_len-5) <= 3 )
				printf("******  MOV: SPS len [%d], not enough to analysis\n", trak->stream_header_len-5);
			if (!check_avc1_sps_bank0(trak->stream_header+5, trak->stream_header_len-5))
			{
...
//	    if(trak) printf("MOV: Warning! trak in trak?\n");
	    ...
	    switch(trak->type){
	    case MOV_TRAK_AUDIO: {
              int gen_ret = 0; //Polun 2011-08-04 for mantis 5663 HE-AAC V2 main profile is not support 
		sh_audio_t* sh=new_sh_audio(demuxer,priv->track_db);
		mp_msg(MSGT_DEMUX, MSGL_INFO, MSGTR_AudioID, "mov", priv->track_db);
              //gen_sh_audio(sh, trak, priv->timescale);
              //Polun 2011-08-04 ++s for mantis 5663 HE-AAC V2 main profile is not support 
		gen_ret = gen_sh_audio(sh, trak, priv->timescale);
              if(gen_ret == 0)
                 free_sh_audio(demuxer,priv->track_db);
              //Polun 2011-08-04 ++e
		break;
	    }
	    case MOV_TRAK_VIDEO: {
...
//		mov_keyframe_size = trak->keyframes_size;	//Fuchun 2010.03.01   2011.03.30 move to if(demuxer->video->id >= 0)
		break;
	    }
	    case MOV_TRAK_GENERIC:
		break;	//Fuchun 2011.05.13 remove subtitle for mp4, FIX later
		...
		for (i=0; i<trak->samples_size; i++)
		{
		    ...
		    fd = open((char *)&name[0], O_CREAT|O_WRONLY, 0666);
...
//      if(!i)
    ...
    case MOV_FOURCC('s','t','t','s'): {
      ...
      if (trak->length > 2147483648)
      {
          float tmp_length, uint_range = (float)(2147483648);

          printf("MPlayer: MOV too large, trak->timescale = %f length=%f\n", (float)trak->timescale, (float)trak->length);
          scale_factor = scale_factor << 1;
          tmp_length = (float)trak->length / scale_factor;

//printf(" dbg 01 tmp_length=%f  scale_factor=%d\n", tmp_length, scale_factor);  
//printf("uint_range=%f\n", uint_range);
          while (tmp_length > uint_range)
          {
              scale_factor = scale_factor << 1;
              tmp_length = (float)trak->length / scale_factor;
//printf(" dbg  scale_factor=%d tmp_length=%f\n", (int)scale_factor, (float)tmp_length);
          }
          trak->length = trak->length / scale_factor;
          trak->timescale = trak->timescale / scale_factor;
          printf("MPlayer: new scale_factor = %d  length=%f, timescale=%f\n", scale_factor, (float)trak->length, (float)trak->timescale);
      }
      ...
      for (i = 0; i < trak->durmap_size; i++) {
        ...
        trak->durmap[i].dur = stream_read_dword(demuxer->stream) / scale_factor;
//printf("trak->durmap[i].num=%d trak->durmap[i].dur=%d\n", trak->durmap[i].num, trak->durmap[i].dur);
        pts += trak->durmap[i].num * trak->durmap[i].dur;
      }
//Robert 20110421 it's ok, just Warning...
      if (trak->length != pts)
        mp_msg(MSGT_DEMUX, MSGL_WARN, "Warning! pts=%llu  length=%llu\n",
               pts, trak->length);
      break;
    }
    ...
    case MOV_FOURCC('s','t','s','z'): {
      ...
      trak->samplesize = ss;
      if (!ss) {
#if 1	//Barry 2011-04-07
        if (trak->type == MOV_TRAK_AUDIO && no_osd)
        {
        	if (trak->samples)
			free(trak->samples);
  		trak->samples = NULL;
		...
		//printf("!!!!!!!!  stsz  [%s - %d]   skip size = %d\n", __func__, __LINE__, entries * 4);
		break;
        }
#endif
//printf("*******   stsz [%s - %d]   sizeof(mov_sample_t)=%d   total_size = %d\n", __func__, __LINE__, sizeof(mov_sample_t), entries * sizeof(mov_sample_t));
        // variable samplesize
//        total_alloc_index_size += entries*sizeof(mov_sample_t);
    ...
    case MOV_FOURCC('s','t','c','o'): {
      ...
      if (len > trak->chunks_size) {
		...
		//printf("!!!!!!!!  stco  [%s - %d]   skip size = %d\n", __func__, __LINE__, len * 4);
		break;
        }
#endif
//printf("*******   stco [%s - %d]   sizeof(mov_chunk_t)=%d   total_size = %d\n", __func__, __LINE__, sizeof(mov_chunk_t), len * sizeof(mov_chunk_t));
//        total_alloc_index_size += len*sizeof(mov_chunk_t);
      ...
      for(i = 0; i < trak->chunks_size; i++)
     {
          ...
          //Polun  2011.05.11  chunk pos process  save high dword pos
         if(i == 0)
         {
             trak->chunkidx= realloc_struct(trak->chunkidx, 1, sizeof(mov_chunkidx_t)); 
             trak->chunkidx[0].idx = 0;
             trak->chunkidx[0].hpos =0;
             trak->chunkidxsize = 1;
        }
    }
      break;
    ...
    case MOV_FOURCC('c','o','6','4'): {
      ...
      // extend array if needed:
      if (len > trak->chunks_size) {
//printf("*******   co64 [%s - %d]   sizeof(mov_chunk_t)=%d   total_size = %d\n", __func__, __LINE__, sizeof(mov_chunk_t), len * sizeof(mov_chunk_t));
//        total_alloc_index_size += len*sizeof(mov_chunk_t);
        trak->chunks = realloc_struct(trak->chunks, len, sizeof(mov_chunk_t));
        trak->chunks_size = trak->chunks ? len : 0;
      }
      ...
      for (i = 0; i < trak->chunks_size; i++) {
       ...
       #else     //Polun  2011.05.11  chunk pos process  save high dword pos
	temppos = stream_read_dword(demuxer->stream);
       trak->chunks[i].pos = stream_read_dword(demuxer->stream);
	if(i==0)
	{	
	       trak->chunkidx= realloc_struct(trak->chunkidx, idxlen, sizeof(mov_chunkidx_t)); 
		trak->chunkidx[idxlen-1].idx = i;
		trak->chunkidx[idxlen-1].hpos =temppos;
		trak->chunkidxsize = idxlen;
	}
	if(trak->chunkidx[idxlen-1].hpos != temppos)
	{
		idxlen += 1;
	       trak->chunkidx= realloc_struct(trak->chunkidx, idxlen, sizeof(mov_chunkidx_t)); 
	       trak->chunkidx[idxlen-1].idx = i;
...
static demuxer_t* mov_read_header(demuxer_t* demuxer){
	...
	file_size = demuxer->stream->end_pos;
	//reset parameter Fuchun 2010.02.24
	packet_cnt = 0;	
	Only_One_I = 0;
	is_mjpeg = 0;
...
/*	    //Fuchun 2010.02.24  2011.03.30 move to if(demuxer->video->id >= 0)
	    if(trak->fourcc == mmioFOURCC('j', 'p', 'e', 'g') || trak->fourcc == mmioFOURCC('A', 'V', 'D' ,'J'))
	    {
		is_mjpeg = 1;
		mp_msg(MSGT_DEMUX, MSGL_V, "MOV: is_mjpeg = 1\n");
	    }
*/
	}
    }
	...
	if(sh){
		...
		//Fuchun 2011.03.30 detect mjpeg
		mov_track_t* trak = NULL;
		trak = stream_track(priv, demuxer->video);
		if(trak)
		{
	 		mov_keyframe_size = trak->keyframes_size;
			if(trak->fourcc == mmioFOURCC('j', 'p', 'e', 'g') || trak->fourcc == mmioFOURCC('A', 'V', 'D' ,'J'))
			{
				is_mjpeg = 1;
				mp_msg(MSGT_DEMUX, MSGL_V, "MOV: is_mjpeg = 1\n");
...
static int demux_mov_fill_buffer(demuxer_t *demuxer,demux_stream_t* ds){
    ...
    int x;
    off_t pos;
    int i, frame=0;	//Fuchun 2010.02.24
    sh_video_t *sh_video = demuxer->video->sh;		//Fuchun 2010.02.24
...
#ifdef STREAM_NI
    demux_switch_stream(demuxer, ds);
#endif

if(trak->samplesize){
	sh_audio_t *sh_audio = demuxer->audio->sh;	//Fuchun 2010.04.14
    // read chunk:
    ...
    while(checknum)
    {
             if(trak->pos >= trak->chunkidx[checknum-1].idx)
            {
                    pos = (off_t)trak->chunkidx[checknum-1].hpos;
		      pos = (pos<<32)+(off_t)trak->chunks[trak->pos].pos;	
                    break;
	     }
             checknum--;
    }
...
if(sh_audio->format != 85 && trak->fourcc != MOV_FOURCC('3','-','c','a') && trak->fourcc != MOV_FOURCC('a','c','-','3'))	//Fuchun 2010.04.14 if format is't mp3 or ac3
...
//	x*=char2int(trak->stdata,32); // bytes/packet
	...
	{
#if 1	//Barry 2011-07-12
		if ((trak->fourcc == MOV_FOURCC('v','4','p','m')) && trak->editlist[trak->editlist_pos].start_sample == 1 && trak->editlist_size == 2)
		{
			printf("##### [%s - %d]  editlist_pos=%d,  editlist_size=%d\n", __func__, __LINE__, trak->editlist_pos, trak->editlist_size);
			return 0;	//EOF
		}
		else
			++trak->editlist_pos;
	...
	//Barry 2010-11-03
	if(trak->fourcc != MOV_FOURCC('1','c','v','a'))
	{
		if (!((trak->fourcc == MOV_FOURCC('v','4','p','m')) && (trak->editlist[trak->editlist_pos].start_frame == 0) && (trak->editlist[trak->editlist_pos].start_sample == 1)))	//Barry 2011-07-12  fix mantis:5205
		{
			//printf("trak->editlist[%d].start_frame=%d     trak->editlist[%d].start_sample=%d\n", trak->editlist_pos, trak->editlist[trak->editlist_pos].start_frame, trak->editlist_pos, trak->editlist[trak->editlist_pos].start_sample);
		...
		{
			if (!frame)
				printf("##### [%s - %d]: trak->editlist[%d].start_frame=%d,   trak->editlist[%d].start_sample=%d\n", __func__, __LINE__, trak->editlist_pos, trak->editlist[trak->editlist_pos].start_frame, trak->editlist_pos, trak->editlist[trak->editlist_pos].start_sample);
		}
	}
	//printf("[%s - %d] frame = %d    editlist_pos=%d    editlist_size = %d\n", __func__, __LINE__, frame, trak->editlist_pos, trak->editlist_size);

	// calc pts:
	if (trak->timescale != 0)	//Barry 2010-11-03
		pts=(float)(trak->samples[frame].pts+trak->editlist[trak->editlist_pos].pts_offset)/(float)trak->timescale;
    } else {
	if(frame>=trak->samples_size) return 0; // EOF
	pts=(float)trak->samples[frame].pts/(float)trak->timescale;
    }

#if 1	//Fuchun 2010.02.24
	if(speed_mult >= 2 && trak->keyframes_size == 1)
		Only_One_I = 1;
	if(speed_mult < 0 && trak->keyframes_size == 1)
		ds->eof = 1;

	if(speed_mult >= 2 && is_mjpeg == 0)
	{
		if(forward_keyframe_idx >= trak->keyframes_size-1)
			ds->eof = 1;
		if(!forward_keyframe_idx)
		{
			for(i=0; i < trak->keyframes_size; i++)
			{
				if(trak->keyframes[i] >= frame)
				{
					frame = trak->keyframes[i];
		...
		else
		{
			do
			{
				frame = trak->keyframes[forward_keyframe_idx];
				forward_keyframe_idx++;
				
				if(forward_keyframe_idx >= trak->keyframes_size-1)
				{
					ds->eof = 1;
					forward_keyframe_idx = trak->keyframes_size-1;
					break;
				}
			}while((frame-last_frame) < (sh_video->fps >= 1000 ? 25 : sh_video->fps));
			last_frame = frame;
		}
	}
	else
	...
	if(!FR_to_end && speed_mult < 0 && is_mjpeg == 0)
	{
...
//			speed_mult = 0;
		...
		{
			for(i=(trak->keyframes_size-1); i >= 0; i--)
			{
				if(trak->keyframes[i] <= frame)
				{
					frame = trak->keyframes[i];
		...
		else
		{
			do
			{
				frame = trak->keyframes[backward_keyframe_idx];
...
//					speed_mult = 0;
			...
			}while((last_frame-frame) < (sh_video->fps >= 1000 ? 25 : sh_video->fps));
			last_frame = frame;
		}
	}
	else
...
//	if(speed_mult != 0 && is_mjpeg == 0 && Only_One_I == 0)
//		pts = trak->samples[frame].pts;
#endif
...
if( (trak->pos==0 || ss_trak_pos ) && trak->stream_header_len>0){
	if (ss_trak_pos)
		ss_trak_pos = 0;

    // we have to append the stream header...
    if(is_FD) x+=2;		//Fuchun 2010.02.11
    demux_packet_t* dp=new_demux_packet(x+trak->stream_header_len);
//	demux_packet_t* dp=new_video_packet(x+trak->stream_header_len, videobuffer + (2<<20));	// Raymond 2009/12/17
	...
	if(is_FD)
	{
		stream_read(demuxer->stream,dp->buffer+trak->stream_header_len+2,x-2);
		dp->buffer[trak->stream_header_len] = dp->buffer[trak->stream_header_len+1] = 0;

		int k = trak->stream_header_len;
		int l = 0;
		unsigned char *ps = dp->buffer;

		l = ((int)ps[k] << 24) + ((int)ps[k+1] << 16) + ((int)ps[k+2] << 8) + ps[k+3] ;
		k += (l + 4);
		
		while(k < x+trak->stream_header_len)
		{
			int ret = x+trak->stream_header_len;
			unsigned char *pss = malloc(ret-k);
			memcpy(pss, dp->buffer + k, ret - k);
			memcpy(dp->buffer + k + 2, pss, ret - k);
			ps[k] = ps[k+1] = 0;
			l = ((int)ps[k] << 24) + ((int)ps[k+1] << 16) + ((int)ps[k+2] << 8) + ps[k+3];
			x += 2;
			k += (l + 4);
		...
		dp->len = x+trak->stream_header_len;
	}
	else	
...
else if ( trak->type == MOV_TRAK_VIDEO )
{
	...
	int len = x;
//	demux_packet_t* dp=new_video_packet(len, videobuffer);	// Raymond 2009/06/02
	demux_packet_t* dp=new_demux_packet(len);	// Raymond 2010/03/26
	...
	if(is_FD)
	{
		stream_read(demuxer->stream,dp->buffer+2,len-2);
		dp->buffer[0] = dp->buffer[1] = 0;
...
//	if (speed_mult != 0 && Only_One_I == 0)
//		ds->pts = pts=(float)trak->samples[frame].pts/(float)trak->timescale;
#endif
}
// end Raymond 2009/04/18
else
	...
	if(!FR_to_end && (speed_mult < 0 || speed_mult >= 2) && is_mjpeg == 0 && Only_One_I == 0)
		trak->pos = frame;
	else if(!FR_to_end && speed_mult < 0 && is_mjpeg == 1)
	{
		--trak->pos;
		if(trak->pos < 0)
		{
...
if(trak->samplesize){
    long long sample=pts/trak->duration;
...
//Barry 2010-04-26
if ( trak->type == MOV_TRAK_VIDEO && flags == 1 )	//for -ss option
	ss_trak_pos = trak->pos;

if (trak->type == MOV_TRAK_VIDEO && flags == 0 && trak->editlist)	//Barry 2010-10-05
{
	if (trak->editlist[trak->editlist_pos].start_sample != 0
		&& (trak->fourcc != MOV_FOURCC('1','c','v','a')))	//Barry 2010-11-03
		trak->pos -= trak->editlist[trak->editlist_pos].start_sample;
}
...
//    printf("MOV seek called  %5.3f  flag=%d  \n",pts,flags);
	...
	{
		ds->eof = 1;
		demuxer->audio->eof = 1;	//Fuchun 2010.08.20 enable a_eof when video EOF
	}
	else pts = ds->pts;
...
static int demux_mov_control(demuxer_t *demuxer, int cmd, void *arg){
  ...
  switch(cmd) {
    ...
    //Polun 2011-11-15 ++s fixed mantis 6510 can't switch audio mp4 file 
   case DEMUXER_CTRL_SWITCH_AUDIO:{
	    ...
	    demux_stream_t *ds;
           mov_track_t* trak;
           float pts;
           void **streams = demuxer->a_streams;
	    int maxid = FFMIN(100, MAX_A_STREAMS);
           ds = demuxer->audio;
           if (ds->id < -1)
	    ...
	    if (*(int *)arg >= 0)
          ...
          trak = stream_track(priv, ds);
          if (trak) {
	       ds->pts=mov_seek_track(trak,pts,SEEK_ABSOLUTE);
	       if (ds->pts < 0) ds->eof = 1;
           }
