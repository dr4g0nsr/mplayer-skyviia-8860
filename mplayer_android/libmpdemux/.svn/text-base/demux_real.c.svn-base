...
extern int upnp;
float skynet_rel_seek_secs = 0.0;
...
read_index:
    //Barry 2011-03-07
    if (next_header_pos > demuxer->stream->end_pos)
    {
...
#if 1	//Barry 2011-03-14
	if (is_skynet)
	{
		sh_audio_t *sh_audio = demuxer->audio->sh;
		sh_video_t *sh_video = demuxer->video->sh;
		real_priv_t *priv = demuxer->priv;
		unsigned char *mem = malloc(4096);
		int i, version, find_frame = 0;
		off_t pos = stream_tell(demuxer->stream);

		pos += (off_t)(((demuxer->movi_end - demuxer->movi_start)/priv->duration-sh_audio->i_bps)*(skynet_rel_seek_secs -sh_video->pts));
		stream_seek(demuxer->stream, pos);
		pos = stream_tell(demuxer->stream);

		while (1)
		{
			len = stream_read(demuxer->stream, mem, 4096);
			if (len < 4096)
			{
				printf("[%s - %d] EOF: len[%d] < 4096 ?\n", __func__, __LINE__, len);
				return;
			}
			for (i=0;i<4096-12;i++)
			{
				version = (mem[i]<<8) |mem[i+1];
				len = (mem[i+2]<<8) |mem[i+3];
				stream_id = (mem[i+4]<<8) |mem[i+5];
				timestamp = (mem[i+6]<<24) |(mem[i+7]<<16) |(mem[i+8]<<8) |mem[i+9];
				flags = mem[i+11];
				if ( len && timestamp && (version == 0 || version ==1) && (stream_id == demuxer->video->id) && (flags == 0 || flags == 2))
				{
					stream_seek(demuxer->stream, pos+i);
					find_frame = 1;
//					printf("[%s - %d]  find it!! now_pos = %lld\n", __func__, __LINE__, pos+i);
//					printf("version=%d,   stream_id=%d,   flags=%d    timestamp = %d\n", version, stream_id, flags, timestamp);
					break;
				}
			}
			if (find_frame)
	...
	while (1)
	{
		...
		if (flags & 2)
		{
//			if (stream_id == seek_stream_id)
//				printf("[%s - %d]   KEYFRAME: flags=%d, timestamp=%d,  seek_timestamp=%lld   demuxer->filepos=%lld\n", __func__, __LINE__, flags, timestamp, seek_timestamp, demuxer->filepos);
//			else
//				printf("[%s - %d]   Audio: flags=%d, timestamp=%d,   seek_timestamp = %lld   demuxer->filepos=%lld\n", __func__, __LINE__, flags, timestamp, seek_timestamp, demuxer->filepos);
...
//				printf("[%s - %d]  final_seek_pos = %lld     timestamp=%d,     seek_timestamp=%ld\n\n", __func__, __LINE__, demuxer->filepos, timestamp, seek_timestamp);
				return;
			}
		}
...
#define DP_HDR_T_SIZE sizeof(dp_hdr_t)//rywu20090601

//rywu20090601++ added to store some rv information
...
//rywu20090601--
int ss_need_rv_sequence_hdr = 0;		 //Barry 2010-04-28
...
//    int j;
    ...
    //rywu20090601++
//    int restore_stream_pos;
    int frame_hdr_pos;
    unsigned int num_slice = 0;
  ...
  if (demuxer->video->id >= 0 && !demuxer->video->sh)
	...
	if(!FR_to_end && speed_mult < 0 && read_nextframe == 1)
	{
		...
		int aid = demuxer->audio->id;
		int64_t current_timestamp = 0;
		int next_offset = 0;
		
		if(priv->index_table_size[vid])	//use the video index if we have one
		{
			if(priv->current_vpacket >= priv->index_table_size[vid]) priv->current_vpacket = priv->index_table_size[vid] - 1;
			current_timestamp = priv->index_table[vid][priv->current_vpacket].timestamp;
			while((current_timestamp - priv->index_table[vid][priv->current_vpacket].timestamp) < 1000)
			{
...
//					speed_mult = 0;
		...
		else if(priv->index_table_size[aid])	//use the audio index if no the video index
		{
			if(priv->current_apacket >= priv->index_table_size[aid]) priv->current_apacket = priv->index_table_size[aid] - 1;
			current_timestamp = priv->index_table[aid][priv->current_apacket].timestamp;
			while((current_timestamp - priv->index_table[aid][priv->current_apacket].timestamp) < 1000)
			{
...
//					speed_mult = 0;
  ...
  while(!stream_eof(demuxer->stream)){
    ...
    demuxer->filepos = stream_tell(demuxer->stream);
	frame_hdr_pos = demuxer->filepos;//rywu20090601 for seeking slice number
    ...
    else if(version != 0 && version != 1 && version != 0x4441 && version != 0x494e)		//Fuchun 2010.07.01
    {
    	 printf("=== real version 0x%04X , not support!!\n", version);
    	 int vid = demuxer->video->id;
	 int aid = demuxer->audio->id;
	 int64_t current_timestamp = 0;
	 int next_offset = 0;
	 if(priv->index_table_size[vid])
	 {
		if(priv->current_vpacket >= priv->index_table_size[vid]) priv->current_vpacket = priv->index_table_size[vid] - 1;
		current_timestamp = priv->index_table[vid][priv->current_vpacket].timestamp;
		while((priv->index_table[vid][priv->current_vpacket].timestamp - current_timestamp) < 1000)
		{
			priv->current_vpacket += 1;
	 ...
	 else if(priv->index_table_size[aid])
	 {
		if(priv->current_apacket >= priv->index_table_size[aid]) priv->current_apacket = priv->index_table_size[aid] - 1;
		current_timestamp = priv->index_table[aid][priv->current_apacket].timestamp;
		while((priv->index_table[aid][priv->current_apacket].timestamp - current_timestamp) < 1000)
		{
			priv->current_apacket += 1;
...
//	printf("len==-256!\n");
    ...
    //Barry 2010-04-28
    if ( ss_need_rv_sequence_hdr && (flags != 2) )
		ss_need_rv_sequence_hdr = 0;
...
got_video:
	...
	// parse video chunk:
	{
	    ...
	    int vpkg_seqnum=-1;
	    unsigned int vpkg_subseq=0; //rywu20090612 int vpkg_subseq=0;
	    int num_packets	= 0;//rywu20090612
	    unsigned char broken_up_by_encoder=0;//rywu20090629
...
//		uint32_t* extra;
		uint8_t* extra;
		int alignment_padding_size = 0;//rywu20090701
		...
		if (0x40==(vpkg_header&0xc0)) {
			...
			// sub-seqnum (bits 0-6: number of fragment. bit 7: ???)
		        ...
		        //rywu20090612++
		        num_packets = vpkg_header;
		        num_packets = ((num_packets&0x3f)<<1);
		        num_packets += (vpkg_subseq&0x80)>>7;
		        //rywu20090612--
		    ...
		    //rywu20090629++
		    broken_up_by_encoder = 0;
		    if (vpkg_length&0x8000) 
			{
				broken_up_by_encoder = 1;
		    }
		    //rywu20090629--
		    
		    if (!(vpkg_length&0x4000)) { //rywu20090629 if (!(vpkg_length&0xC000)) {
			vpkg_length<<=16;
		    ...
		    if (!(vpkg_offset&0x4000)) { //rywu20090629 if (!(vpkg_offset&0xC000)) {
			vpkg_offset<<=16;
		...
		if(ds->asf_packet){
		    ...
		    //rywu20090601 dp_data=dp->buffer+sizeof(dp_hdr_t);
		    //rywu20090601++
		    rv_extra_info = (rv_info *)(dp->buffer + dp_hdr->chunktab - RV_INFO_BLOCK_SIZE);
		    dp_data = dp->buffer + DP_HDR_T_SIZE + rv_extra_info->rv_header_len;
			//rywu20090601--
			//rywu20090629++
			{
				rv_phdr = (rv_pic_header *)(dp->buffer + DP_HDR_T_SIZE);
			}
			//rywu20090629--
			
		    extra=(uint8_t*)(dp->buffer+dp_hdr->chunktab);
			...
			if(dp_hdr->chunktab+8*(1+dp_hdr->chunks)>dp->len){
			    ...
			    //dp->buffer=srealloc(dp->buffer,dp->len+FF_INPUT_BUFFER_PADDING_SIZE);	//Barry 2010-03-26 disable
...
//			extra[2*dp_hdr->chunks+1]=dp_hdr->len;
			PUT32(1, extra + 4 * 2*dp_hdr->chunks);
			PUT32(dp_hdr->len, extra + 4 * 2*dp_hdr->chunks + 4);
			//rywu20090601++
			//fill slice data at the end of pictue header			
			PUT32(0x00000001,  ((unsigned char *)(&rv_phdr->num_slices))+4+dp_hdr->chunks*8);
			PUT32(dp_hdr->len, ((unsigned char *)(&rv_phdr->num_slices))+8+dp_hdr->chunks*8);
			//rywu20090601--
			...
			if(0x80==(vpkg_header&0xc0)){
			    ...
			    if(priv->video_after_seek){
			    	...
			    	{
			    dp->pts = real_fix_timestamp(priv,dp_data,dp_hdr->timestamp,sh_video->frametime,sh_video->format);
			}
			    ds_add_packet(ds,dp);
			    ds->asf_packet=NULL;
			    // continue parsing
		...
		//rywu20090601++
		num_slice = 1;//we have one slice at least.
		if(0x00 == (vpkg_header&0xc0))
		{
			/*rywu20090612 removed
			//backup stream position
			restore_stream_pos = stream_tell(demuxer->stream);
			
			num_slice = real_check_slice_data(demuxer, frame_hdr_pos);
			
			//restore stream position
			stream_seek(demuxer->stream, restore_stream_pos);
			*/
			num_slice = num_packets;//rywu20090612
		}
		//rywu20090601--
		
		/* rywu20090601 marked the original code
		// create new packet!
		...
		*/
		...
		//increase frame count for reference
        sh_video->rmvb_frame_count++;
		
		//Basically, each frame has its own picture header.
		//Trinity needs one sequence header within first frame, 
		//so we calculate header length for later use.
		rv_header_size = RV_PICTURE_HEADER_SIZE + 8*num_slice;

        //create new packet
		//rywu20090701 dp = new_demux_packet(DP_HDR_T_SIZE+ rv_header_size + vpkg_length + RV_INFO_BLOCK_SIZE + 8*(1+2*(vpkg_header&0x3F)));
		alignment_padding_size = 4 - ((DP_HDR_T_SIZE+ rv_header_size + vpkg_length)%4);//rywu20090701
		dp = new_demux_packet(DP_HDR_T_SIZE+ rv_header_size + vpkg_length + alignment_padding_size + RV_INFO_BLOCK_SIZE + 8*(1+2*(vpkg_header&0x3F)));//rywu20090701
        // the timestamp seems to be in milliseconds
        dp->pos = demuxer->filepos;
        dp->flags = (flags & 0x2) ? 0x10 : 0;
		ds->asf_seq = vpkg_seqnum;
		...
		dp_hdr->chunks=0;
		dp_hdr->timestamp=timestamp;
		//rywu20090701 dp_hdr->chunktab=DP_HDR_T_SIZE+ rv_header_size + vpkg_length + RV_INFO_BLOCK_SIZE;
		dp_hdr->chunktab = DP_HDR_T_SIZE+ rv_header_size + vpkg_length + alignment_padding_size + RV_INFO_BLOCK_SIZE;//rywu20090701
		dp_data=dp->buffer + DP_HDR_T_SIZE + rv_header_size;
		
		//store rv_header_size for further use
		rv_extra_info = (rv_info *)(dp->buffer + dp_hdr->chunktab - RV_INFO_BLOCK_SIZE);
		rv_extra_info->rv_header_len = rv_header_size;
		rv_extra_info->useless_slice_num = 0;//rywu20090629
		rv_extra_info->frame_padding_len = alignment_padding_size;//rywu20090701
...
//		extra[0]=1; extra[1]=0; // offset of the first chunk
        ...
        //prepare picture header
        rv_phdr = (rv_pic_header *)(dp->buffer + DP_HDR_T_SIZE);
        {
            rv_phdr = (rv_pic_header *)(dp->buffer + DP_HDR_T_SIZE);
        }

       PUT32(vpkg_length, (unsigned char *)(&rv_phdr->data_length));
        
        //For whole frame case(vpkg_header = 0xC0), vpkg_offset is the timestamp.
		//Otherwise, it is offset.
		//rywu20090612 replaced "timestamp" by "vpkg_offset", if necessary
		if (vpkg_offset != 0)
		{
			timestamp = vpkg_offset;
		}

		PUT32(timestamp, (unsigned char *)(&rv_phdr->time_stamp));
		
		value16 = sh_video->rmvb_frame_count - 1;//sequence number is counted from 0
		PUT16(value16, (unsigned char *)(&rv_phdr->sequence_number));
		rv_phdr->flags = (flags<<8);
        rv_phdr->last_packet= 0;
        PUT32(num_slice, (unsigned char *)(&rv_phdr->num_slices));
        
        //fill 1st slice data
        //*(unsigned int *)((BYTE *)(&rv_phdr->num_slices)+4) = 0x01000000;//first 4 bytes is 0x00000001
        //*(unsigned int *)((BYTE *)(&rv_phdr->num_slices)+8) = 0;//first offset is 0
        PUT32(0x00000001, ((unsigned char *)(&rv_phdr->num_slices))+4);
        PUT32(0x00000000, ((unsigned char *)(&rv_phdr->num_slices))+8);
		//rywu20090601--
...
//			    sh->format = 0x2000;
			...
			case MKTAG('a', 't', 'r', 'c'):	//Barry 2010-11-11
				printf("Audio[atrc] not support\n");
...
#undef stream_skip
	  ...
	  if (strstr(mimet,"x-pn-realvideo") || strstr(mimet,"x-pn-multirate-realvideo")) {
	    //rywu20090601++
	  	...
	  	current_stream_pos = stream_tell(demuxer->stream); 
	  	
	  	//let's getting type_specific_data
	  	type_specific_len = stream_read_dword(demuxer->stream);
	  	type_specific_data = (unsigned char *)malloc(type_specific_len);//NOTE: remember to free memory after done
	  	if (type_specific_data > 0)
		{
			//restore stream position
			stream_seek(demuxer->stream, current_stream_pos);
			
			stream_read(demuxer->stream, (char *)type_specific_data, type_specific_len);
			//memcpy(type_specific_data, &(demuxer->stream->buffer[demuxer->stream->buf_pos]), type_specific_len);
		}
		else
        {
            printf("<mp> mayday! mayday! can't allocate memory.\n");
        }
		//free(type_specific_data);
		...
		if(tmp != MKTAG('O', 'D', 'I', 'V'))
		{
			...
			//rywu20090601++
			//initialize rmvb related variables
			sh->rmvb_frame_count            = 0;
			sh->rmvb_sequence_header        = 0;
			sh->rmvb_sequence_header_len    = 0;
			//After copy spcific data, it will be back to current stream position.
			if (type_specific_data > 0)
			{
				//record data length and allocate memory to save sequence header
				sh->rmvb_sequence_header_len	= type_specific_len;
				
				//NOTE : remember to free this allocated memory
				sh->rmvb_sequence_header		= (unsigned char *)malloc(sh->rmvb_sequence_header_len);
				if (sh->rmvb_sequence_header)
				{
					//backup sequence header
					memcpy(sh->rmvb_sequence_header, type_specific_data, type_specific_len);
...
//    printf("i=%d num_of_headers=%d   \n",i,num_of_headers);
   ...
   //if(demuxer->stream->type != STREAMTYPE_STREAM)	//Fuchun 2010.07.20 don't parse index table when it's streaming
    if(upnp == 0)					//Charles 20110802 seek without index may jump to wrong position and take very long time, check upnp instead
    switch (index_mode){
	case -1: // untouched
	...
	int len;
	   ...
	   sh->format == mmioFOURCC('d','x','5','0') ||
	   sh->format == mmioFOURCC('D','I','V','3') ||
	   sh->format == mmioFOURCC('d','i','v','3') ||
	   sh->format == mmioFOURCC('D','I','V','X') ||
	   sh->format == mmioFOURCC('d','i','v','x'))
    		len=sprintf(video_names, "%s,%dx%d,%5.3f\0", "Xvid_Compatible",sh->disp_w,sh->disp_h,sh->fps);
    	else
    		len=sprintf(video_names, "%.4s,%dx%d,%5.3f\0", (char *)&sh->format,sh->disp_w,sh->disp_h,sh->fps);
	video_names[len] = '\0';
    }
...
//    printf("streams: %d\n", streams);
    ...
    if (is_skynet && flags != 1)
    {
    	printf("Need double check skynet seek function!!!\n");
    	return;
    }

    //Barry 2010-04-28
    if ( (streams & 1) && (flags & 1) )	//for -ss option
    {
    	ss_need_rv_sequence_hdr = 1;

	if (is_skynet)
	{
		flags = 0;
		if (rel_seek_secs < sh_video->pts)
			skynet_rel_seek_secs = (rel_seek_secs-1.0);
		else
			skynet_rel_seek_secs = rel_seek_secs;
//		printf("[%s - %d]  flags=%d,     skynet_rel_seek_secs=%f    now: sh_video->pts=%f\n", __func__, __LINE__, flags, skynet_rel_seek_secs, sh_video->pts);
	}
    }
...
//    if (index_mode == 1 || index_mode == 2) {
	...
	{// use the video index if we have one
		...
		if (rel_seek_secs > 0)
		{
			if (is_skynet)
			{
					add_index_segment(demuxer, vid, rel_seek_secs * 1000);
			}
			else
			{
