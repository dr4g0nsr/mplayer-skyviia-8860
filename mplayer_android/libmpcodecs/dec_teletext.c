...
typedef struct {
    ...
    int             curr_pagenum_sub;  ///< latest page with subtitle update
...
static int decode_pkt0(priv_vbi_t* priv,unsigned char* data,int magAddr)
{
    int d[8];
    int i,err;
    int erase_page, subtitle, update_indec, inhibit_disp;
    ...
    erase_page = (d[3]>>3)&1;
    subtitle = (d[5]>>3)&1;
    update_indec = (d[6]>>1)&1;
    inhibit_disp = (d[6]>>3)&1;
/*
    printf("page: %d, usbpage: %d, erase_page: %d, subtitle: %d, update_indec: %d, inhibit_disp: %d\n", 
		    priv->mag[magAddr].pt->pagenum, priv->mag[magAddr].pt->subpagenum, erase_page,
		    subtitle, update_indec, inhibit_disp);
*/
...
*/
    ...
    if (erase_page == 1)
    {
	    ...
	    if ((update_indec == 1) && (priv->mag[magAddr].pt->subpagenum != 0x3f7f))
	    {
		    priv->curr_pagenum_sub = priv->mag[magAddr].pt->pagenum;
	    }
	    if (priv->curr_pagenum_sub == priv->mag[magAddr].pt->pagenum)
		    priv->page_changed=1;
    } else {
	    if (priv->curr_pagenum_sub == priv->mag[magAddr].pt->pagenum)
		    priv->page_changed=3;
...
int teletext_control(void* p, int cmd, void *arg)
{
	...
	// set default page num
	if ((priv->pagenum == 0) && (priv->curr_pagenum_sub != -1))
	{
		priv->pagenum = priv->curr_pagenum_sub;
		//printf("reset pagenum to %d\n", priv->pagenum);
	}
        return VBI_CONTROL_TRUE;
    ...
    case TV_VBI_CONTROL_IS_CHANGED:
        ...
        if(GetTimerMS()-priv->last_rendered < 250)
		*(int*)arg=0;
	else
