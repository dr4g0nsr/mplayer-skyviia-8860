...
#define ERROR() { \
  ...
  for(i=0; i<3; i++) {
    ...
    /* replaygain: scale */
    if(!rg_gain)
      ov->rg_scale = 1.0; /* just in case pow() isn't standard-conformant */
    else
      ov->rg_scale = pow(10.f, rg_gain/20);
    /* replaygain: anticlip */
    if(ov->rg_scale * rg_peak > 1.0)
      ov->rg_scale = 1.0 / rg_peak;
    /* replaygain: security */
    if(ov->rg_scale > 15) 
      ov->rg_scale = 15;
#ifdef TREMOR
    ov->rg_scale_int = (int)(ov->rg_scale*64);
#endif
	...
	/*      for(j=0;j<bout;j++){
		...
		*ptr=val;
		ptr+=ov->vi.channels;
	    	            ...
	    	            pcmCopy1(mono,ptr,bout,ov->vi.channels);  
