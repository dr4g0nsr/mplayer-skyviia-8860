...
extern int print_video_decode_time;	// Raymond 2009/08/03
...
int get_current_video_queue_len(sh_video_t *sh_video)
{
    int ret;

    if (!mpvdec)
	return -1;
    ret = mpvdec->control(sh_video, VOCTRL_GET_QUEUE_LEN, NULL);
    return ret;
}

//Robert 20100617 current display buffers
int get_current_vq_buffers(sh_video_t *sh_video, unsigned int *qlen)
{
    int ret;
    ...
    if (!mpvdec)
	return -1;
    ret = mpvdec->control(sh_video, VOCTRL_GET_VQ_BUFFERS, qlen);
...
if(!sh_video->initialized){
	not_supported_profile_level = 1;	//Barry 2010-12-03
		   ...
		   int drop_frame, double pts)
{
	...
	// Raymond 2009/08/03
	if(print_video_decode_time && t > 0)
		printf("\t\t\t\t\tV_Dec = %3d ms\n", t/1000);

#if 0		// Raymond 2010/03/18 disable
    if (!mpi || drop_frame)
	...
	//Fuchun 2010.07.15 move to here from generate_video_frame
	if (sh_video->last_pts>0 && sh_video->pts>0 && sh_video->pts > sh_video->last_pts)
	{
		sh_video->next_frame_time = sh_video->pts - sh_video->last_pts;
	}
    }
    return mpi;
}
