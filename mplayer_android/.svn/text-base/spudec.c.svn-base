...
extern int sub_pos;
extern int is_dvdnav;		//Fuchun 2011.02.21
#ifdef DVB_SUBTITLES
...
#ifdef CONFIG_DVDNAV
extern unsigned int disp2_addr;
extern int vo_need_dvdnav_menu;	//Fuchun 2010.09.20
extern unsigned int dvdnav_btn_virtaddr[2];
extern unsigned int dvd_btn_addr_idx;
unsigned int dvd_btn_addr_idx_tmp;
int dvdnav_btn_change = 0;
#include "stream/stream_dvdnav.h"
...
struct packet_t {
  ...
  unsigned long long start_pts, end_pts;
  packet_t *next;
  packet_t *prev;
};
...
#ifdef DVB_SUBTITLES		//Fuchun 2010.12.28
  unsigned int is_decoded;	// 1:pgs, 2:xsub, 3:dvb, 4:auto_platte=0
#endif
...
//+Skyviia_Vincent07202010
  ...
  unsigned int dvdnav_frame_width; /* for 704 width menu */
//Skyviia_Vincent07202010+
} spudec_handle_t;
...
static packet_t *spudec_tail_dequeue_packet(spudec_handle_t *this)
{
	packet_t *retval = this->queue_tail;

	this->queue_tail = retval->prev;
...
buf = malloc(1920*sizeof(uint32_t));
...
switch (this->dvdnav_color_spu)
{
  ...
  case DVDNAV_SPU_RGB:
  case DVDNAV_SPU_BGR:
//    memset(this->image + y * this->stride + x, color, len); //R
//    memset(this->imageu + y * this->stride + x, coloru, len);   //G
//    memset(this->imagev + y * this->stride + x, colorv, len);   //B
//    memset(this->aimage + y * this->stride + x, alpha, len);

    if(alpha)
        pixel = 162<< 24 | (color  << 16) | (coloru<< 8) | colorv ;
    else
        pixel = alpha << 24 | color << 16 | coloru << 8 | colorv;
...
//    memcpy((uint32_t)disp2_addr+disp2_offset, buf, len*4);
//    disp2_offset += len*4;
        
//    memcpy((uint32_t)disp2_addr + (this->start_row*720*4)+this->start_col*4 + x*4 + (720*y*4), buf, len*4);

	memcpy((uint32_t)dvdnav_btn_virtaddr[dvd_btn_addr_idx_tmp] + (this->start_row*this->dvdnav_frame_width*4)+this->start_col*4 + x*4 + (this->dvdnav_frame_width*y*4), buf, len*4);
...
{
  unsigned int cmap[4], alpha[4], ymap[4], iswhite[4];
  ...
  unsigned int i, x, y;
  int spu_got_yellow_color = 0;
  float HSV_Vmap[4];
  ymap[0]=-1;
  ymap[1]=-1;
  ymap[2]=-1;
  ymap[3]=-1;
	if(vo_need_dvdnav_menu)	//Fuchun 2010.09.20 just for dvd navication menu
	{
		//printf("packet->width[%d],packet->height[%d]\n",packet->width,packet->height);
		//printf("disp2_offset =====[%d]\n",disp2_offset);
//		memset(disp2_addr, 0x00, 720*480*4 );

	    memset(dvdnav_btn_virtaddr[dvd_btn_addr_idx], 0x00, DVDNAV_BUTTON_SIZE);
        dvdnav_btn_change = 1;
        dvd_btn_addr_idx_tmp = !dvd_btn_addr_idx;
	}
  ...
  for (i = 0; i < 4; ++i)
  {
    ...
    {
    	if((is_dvdnav && !this->dvdnav_menu) || (!is_dvdnav && !this->auto_palette))	//Fuchun 2011.02.21
    	{
float tmpMax, tmpMin;
float HSV_V, HSV_S, HSV_H;
...
unsigned char RGBMax=0;
		...
		cmap[i] = (((tr >> 1) & 0x60) | ((tg >> 3) & 0x1c) | ((tb >> 6) & 0x03) | 0x80);
		cmapu[i] = tu;
		cmapv[i] = tv;

		tmpR = (float)tr; tmpG = (float)tg; tmpB = (float)tb;
		if (tr >= tg)
		{
		    if (tr >= tb)
		    {
		...
		HSV_V = (tmpMax/255) * 100;
		HSV_S = tmpMax == 0 ? 0 : (1.0 - tmpMin/tmpMax)*100;

		if (tmpMax == tmpMin)
		{
		    HSV_H = 0;
		}
		else if(RGBMax == 2)//B == rgbMaxMin.Max)
                {
                    HSV_H =  (((tmpR - tmpG)/(tmpMax - tmpMin)) + 4)*60;
                }
                else if(RGBMax == 1)//G == rgbMaxMin.Max)
                {
                    HSV_H =  (((tmpB - tmpR)/(tmpMax - tmpMin)) + 2)*60;
                }
                else if(tmpG < tmpB)
                {
                    HSV_H =  (((tmpG - tmpB)/(tmpMax - tmpMin)) + 6)*60;
                }
                else
                {   
                    HSV_H =  (((tmpG - tmpB)/(tmpR - tmpMin)) )*60;
                }
                
                HSV_Vmap[i] = HSV_V;
//printf(" tmpR:%f tmpG:%f tmpB:%f  tmpMax:%f Min:%f HSV_H:%f\n", tmpR, tmpG, tmpB, tmpMax, tmpMin, HSV_H);
//printf("H, S, V (%f , %f,  %f)\n", HSV_H, HSV_S, HSV_V);                                                                                                                                                                		
//printf("=======> i:%d  ty:%d  cmap:%d  tu :%d  tv:%d\n", i, ty, cmap[i], tu, tv);
//printf("tu :%d  tv:%d\n", tu, tv);
//                    ymap[i] = ty;

                if ((abs(128 - tu)< 2) && (abs(128-tv) < 2))
                {
                    ymap[i] = ty;	//should be white -> black
                    iswhite[i] = 1;
                }
                else
                {
                    iswhite[i] = 0;
                    if (HSV_H >= 56 && HSV_H <= 64)
                    {
                        //Yellow
                        ymap[i] = -3;
                        if (HSV_V > 70)
                        {
                            spu_got_yellow_color = 1;
                        }
                    }
/*	//Reserved for Blue
                    else if (HSV_H >= 160 && HSV_H <= 260)
                    {
                        //Blue
                        ymap[i] = -5;
                    }
*/
                    ...
                    {
                        ymap[i] = ty;       //Use white -> black
                    }
                }

//hcmap[i] = ((this->global_palette[thpalette[i]] >> 16) & 0xff);
//		printf("### In[%s][%d]  ty:%d tr[%x]  tg[%x]  tb[%x]  cmap[i]=%x  this->global_palette[packet->palette[%d]]=%x###\n"
//			, __func__, __LINE__, ty, tr, tg, tb, cmap[i], i, this->global_palette[packet->palette[i]]);
    	}
	else
	{
		cmap[i] = ((this->global_palette[packet->palette[i]] >> 16) & 0xff);
//printf("--> this->auto_palette=%d  i=%d cmap[%d]=%d\n", this->auto_palette, i, i, cmap[i]);
	}
...
#if 0
//printf("Hcmap %d %d %d %d\n", hcmap[0], hcmap[1], hcmap[2], hcmap[3]);
printf(" cmap %d %d %d %d\n", cmap[0], cmap[1], cmap[2], cmap[3]);
printf("  ty  %d %d %d %d\n", ymap[0], ymap[1], ymap[2], ymap[3]);
printf("alpha %d %d %d %d\n", alpha[0], alpha[1], alpha[2], alpha[3]);
#endif
//20110602 Robert workaround for DVD subtitle color.
//         Please un-comment and give it a try...
// color value 0 : transparency
//            128 : black
//            255 : white
//           others: other color
#if 1
  if((is_dvdnav && !this->dvdnav_menu) || (!is_dvdnav && !this->auto_palette))	//Fuchun 2011.02.21
  {
      for (i=0; i<4; i++)
      {
        if (ymap[i] == -1)
        {
            //do nothing, not alpha
        }
        else if ((ymap[i] == -3) && (spu_got_yellow_color == 1))
        {
            //Yellow, check S value (HSV)
            if (HSV_Vmap[i] > 70)
		cmap[i] = (((255 >> 1) & 0x60) | ((255 >> 3) & 0x1c) | ((0 >> 6) & 0x03) | 0x80);
            else if (HSV_Vmap[i] > 40)
            {
//                cmap[i] = (((255 >> 1) & 0x60) | ((255 >> 3) & 0x1c) | ((112 >> 6) & 0x03) | 0x80);
//20110609 Robert set this threshold to Black border
                  cmap[i] = 128;
            }
            else if (HSV_Vmap[i] > 10)
            {
//                cmap[i] = (((255 >> 1) & 0x60) | ((255 >> 3) & 0x1c) | ((196 >> 6) & 0x03) | 0x80);
                  cmap[i] = 128;
            }
            else
            {
        ...
        {
//20110613 Robert Fix DVD subtitle color for 21_20070411_死亡筆記本II - 決戰時刻
          if (alpha[i] > 1 && alpha[i] < 50 && HSV_Vmap[i] < 50)
          {
//20110610 Robert change to black color, fine tune for DVD "Enemy of the State"
              cmap[i] = 128; //0;
          }
          else
          {
            if (HSV_Vmap[i] < 20)	//should be ymap[i] == 16
            {
              cmap[i] = 128;	//set black
            }
            else if (HSV_Vmap[i] < 60)
            {
              cmap[i] = 173;	//white - low
            }
            else if (HSV_Vmap[i] < 80)
            {
              cmap[i] = 218;	//white - medium
            }
            else
            {
              cmap[i] = 255;	//white - high
            }
          }
        }
        else	//not really white but use Y as black<->white level
        {
          if (ymap[i] < 35)	//should be ymap[i] == 16
          {
              cmap[i] = 128;	//set black
          }
          else if (ymap[i] < 130)
	...
	if((is_dvdnav && !this->dvdnav_menu) || (!is_dvdnav && !this->auto_palette))	//Fuchun 2011.02.21
	{
		...
		for(ii = 0; ii < 4; ii++)
		{
			if(cmap[ii] != 0 && ii != j && cmap[ii] != max_color)
			{
//				printf("### In[%s][%d]   cmap[%d]=%x change to 0x80 ###\n", __func__, __LINE__, ii, cmap[ii]);
				cmap[ii] = 0x80;
			}
		}
	}
...
//		 printf("###### DVDNAV_SPU_RGB, ty:%d, tu:%d, tv:%d, tr:%d, tg:%d, tb:%d\n",ty,tu,tv, tr,tg,tb);
		
		 ty = (float)(0.299 * tr + 0.587 *tg + 0.114 * tb);
		 tu = (float)(-0.169 *tr - 0.331 *tg +0.499 *tb +128);
		 tv = (float)(0.499*tr -0.418 *tg - 0.0813 *tb+128);
...
static void spudec_process_control(spudec_handle_t *this, long long pts100)
{
...
//	            }
#if 1	//Fuchun 20110705 fix no sub, because a=b=c=d=0
	if(is_dvdnav && !this->dvdnav_menu)
	{
		if(a != 0 || b != 0 || c != 0 || d != 0)
		{
			this->alpha[0] = a;
	...
	if(pgs_subtitle_flag)
	{
		while (spu->queue_head != NULL && pts100 >= spu->queue_head->start_pts) 
		{
			...
			if (packet->is_decoded)
			{
				...
				spu->is_decoded = packet->is_decoded;
			} 
			else 
			{
				if (spu->auto_palette)
...
if(dvdnavstate.dvdnav_button_on) // menu button show immediately Mantis:4183
...
int spudec_visible(void *this){
    ...
    int ret=(spu->start_pts <= spu->now_pts &&
	     ...
	     spu->height > 0);
//	printf("In[%s][%d]  start_pts[%lld]  now_pts[%lld]  end_pts[%lld]  height[%d]\n", __func__, __LINE__, spu->start_pts, spu->now_pts, spu->end_pts, spu->height);
...
{
	spudec_handle_t *spu = (spudec_handle_t *)this;
	return spu->start_pts;
}

unsigned int spu_get_decodeing(void *this)
{
	spudec_handle_t *spu = (spudec_handle_t *)this;
	if(!is_dvdnav && !spu->auto_palette)
		return 4;
#ifdef DVB_SUBTITLES
	return spu->is_decoded;
#else
	return 0;
#endif
}
...
static void scale_image(int x, int y, scale_pixel* table_x, scale_pixel* table_y, spudec_handle_t * spu)
{
  ...
  if(pgs_subtitle_flag)
  {
	scale[0] = (table_x[x].left_up * table_y[y].left_up >> 16) * 255;
	if (table_y[y].left_up == 0x10000) // necessary to avoid overflow-case
		scale[0] = table_x[x].left_up * 255;
	scale[1] = (table_x[x].right_down * table_y[y].left_up >>16) * 255;
	scale[2] = (table_x[x].left_up * table_y[y].right_down >> 16) * 255;
	scale[3] = (table_x[x].right_down * table_y[y].right_down >> 16) * 255;
  }
  else
#endif
  {
...
#ifdef DVB_SUBTITLES
  if (spu->scaled_aimage[scaled] && !pgs_subtitle_flag)
#else
...
void spudec_draw_scaled(void *me, unsigned int dxs, unsigned int dys, void (*draw_alpha)(int x0,int y0, int w,int h, unsigned char* src, unsigned char *srca, int stride))
{
  ...
  scale_pixel *table_x;
  scale_pixel *table_y;
  static unsigned int original_start_row = 0;

#ifdef DVB_SUBTITLES
	if(spu->is_decoded == 2 || spu->is_decoded == 3)
		spu_aamode = 0;
#endif
	...
	if(spu->is_decoded == 1)
		spu_aamode = 0;
#endif
	if((is_dvdnav && !spu->dvdnav_menu) || (!is_dvdnav))
		spu_aamode = 0;
  ...
  if (spu->start_pts <= spu->now_pts && spu->now_pts < spu->end_pts) {
	...
	|| (spu->orig_frame_width == dxs && spu->orig_frame_height == dys))) {
      ...
      {
	if(spu->spu_changed)
		original_start_row = spu->start_row;

	if((is_dvdnav && !spu->dvdnav_menu) || (!is_dvdnav))
	{
		spu->start_row = original_start_row*sub_pos/100;
		if(spu->start_row + spu->height > dys)
	...
	if ( spu->width > (unsigned int)(spu->orig_frame_width * 0.95))
	{
		// resize osd size to avoid flick.
		float osd_scal = .8;
		scalex = osd_scal * 0x100 * dxs / spu->orig_frame_width;
		scaley = osd_scal * 0x100 * dys / spu->orig_frame_height;
		spu->scaled_width = spu->width * scalex / 0x100;
		spu->scaled_height = spu->height * scaley / 0x100;
		spu->scaled_start_col = spu->start_col * scalex / 0x100 + (dxs * (1 - osd_scal)/2);
		spu->scaled_start_row = spu->start_row * scaley / 0x100 + (dys * (1 - osd_scal)*.9);
	} else {
...
nothing_to_do:
	...
	if((is_dvdnav && !spu->dvdnav_menu) || (!is_dvdnav))
		original_start_row = spu->scaled_start_row;
      }
      if (spu->scaled_image){
        switch (spu_alignment) {
        case 0:
	...
	default:	//Fuchun 20110818 let spu can set subtitle pos
		if((is_dvdnav && !spu->dvdnav_menu) || (!is_dvdnav))
		{
			spu->scaled_start_row = original_start_row*sub_pos/100;
			if(spu->scaled_start_row + spu->scaled_height > dys)
				spu->scaled_start_row = dys - spu->scaled_height;
		}
		break;
  ...
  {
    mp_msg(MSGT_SPUDEC,MSGL_DBG2,"SPU not displayed: start_pts=%lld  end_pts=%lld  now_pts=%lld\n",
...
void *spudec_new_scaled(unsigned int *palette, unsigned int frame_width, unsigned int frame_height, uint8_t *extradata, int extradata_len)
{
  ...
  if (this){
    ...
    this->dvdnav_frame_width = frame_width;
#ifdef DVB_SUBTITLES
   ...
   if(this->orig_frame_height >= 1080)
   	this->orig_frame_height = 1080;
   if(this->orig_frame_width >= 1920)
   	this->orig_frame_width = 1920;
#endif
  }
  else
                           ...
                           int dst_stride, int w, int h)
{
  ...
  for (y = 0; y < h; y++) {
...
#ifdef FIX_SUBTITLE_FLICKING
      *dst++  = pixel;
      //*dsta++ = pixel >> 8;	// alpha is not used by vo_skyfb_2.c :: draw_alpha()
      if((x & 0x3FF) == 0) usleep(1);
#else
      *dst++  = pixel;
      *dsta++ = pixel >> 8;
#endif
    ...
    {
      *dst++ = 0;
      //*dsta++ = *dst++ = 0;	// alpha is not used by vo_skyfb_2.c :: draw_alpha()
      if((x & 0x3FF) == 0) usleep(1);
    }
#else
      *dsta++ = *dst++ = 0;
...
void spudec_set_paletted(void *this, const uint8_t *pal_img, int pal_stride,
                         ...
                         double pts, double endpts, char type, int num_rect)
{
  int i;
  ...
  if(type == 'b' && num_rect > 0)
  {
  	int last_img_len;
	packet = spudec_tail_dequeue_packet(spu);
	packet->height += h;
	packet->start_row = (y < packet->start_row ? y : packet->start_row);
	last_img_len = packet->data_len/2;
	packet->data_len = 2 * stride * packet->height;
	packet->packet = realloc(packet->packet, packet->data_len);
	memmove(packet->packet + stride * packet->height, packet->packet + last_img_len, last_img_len);
	img  = packet->packet + last_img_len;
	aimg = packet->packet + stride * packet->height + last_img_len;
  }
  else
  {
  ...
  if(packet->is_decoded == 1)
  {
	for (i = 0; i < 256; i++) 
	{
		...
		int alpha = pixel >> 24;
#ifdef DVB_TRUE_COLOR
		int gray = (((pixel)>>17)&0x60) | (((pixel)>>11)&0x1c) | (((pixel)>>6)&0x03);
		if(alpha > 0x00)
			gray |= 0x80;
  ...
  else if(packet->is_decoded == 2)
  {
	for (i = 0; i < 256; i++) 
	{
		...
		int temp_gray = FFMIN(gray, alpha);
		if(temp_gray == gray && (temp_gray != 0 || i > 0))
			temp_gray |= 0x80;

		g8a8_pal[i] = (-alpha << 8) | temp_gray;
	}
  }
  else
  {
