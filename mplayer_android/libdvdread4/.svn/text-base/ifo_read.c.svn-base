...
ifo_handle_t *ifoOpen(dvd_reader_t *dvd, int title) {
  ...
  if(title) {
    fprintf(stderr, "libdvdread,ifoOpen(): Invalid IFO for title %d (VTS_%02d_0.IFO).\n",
            title, title);
  } else {
...
ifo_handle_t *ifoOpenVTSI(dvd_reader_t *dvd, int title) {
  ...
  fprintf(stderr, "libdvdread,ifoOpenVTSI(): Invalid IFO for title %d (VTS_%02d_0.IFO).\n",
          title, title);
  ifoClose(ifofile);
  return NULL;
}
...
//printf("still_time[%d],i=%d\n",cell_playback[i].still_time,i);
    int time;
    int size = cell_playback[i].last_sector - cell_playback[i].first_sector;
    time  = (cell_playback[i].playback_time.hour   >> 4  ) * 36000;
    time += (cell_playback[i].playback_time.hour   & 0x0f) * 3600;
    time += (cell_playback[i].playback_time.minute >> 4  ) * 600;
    time += (cell_playback[i].playback_time.minute & 0x0f) * 60;
    time += (cell_playback[i].playback_time.second >> 4  ) * 10;
    time += (cell_playback[i].playback_time.second & 0x0f) * 1;

//printf("i=%d, time[%d], size[%d]\n",i,time,size);
//printf("sec-1[%x],sec-2[%x]\n",(cell_playback[i].playback_time.second >> 4  ) * 10,(cell_playback[i].playback_time.second & 0x0f) * 1);
    if(time == 0 && *nr>1 && size < 5 && i!=0 && i==(*nr-1))
    {
	//Mantis:5711 STAR WAR V
	tmp_nr--;
    }
    else if(cell_playback[i].still_time==1 && 
       	    (cell_playback[i].first_sector == cell_playback[i].last_vobu_start_sector && 
             cell_playback[i].last_sector - cell_playback[i].last_vobu_start_sector < 5 )
            && nr>1 && i!=0
           )
    {
	//Mantis:5603 02_Pearl Harb
	tmp_nr--;
    }
    else if(cell_playback[i].first_sector ==0 && cell_playback[i].last_sector == 0 && cell_playback[i].last_vobu_start_sector ==0 && nr>1 && i!=0)
    {
	//Mantis:5523 300 trailer seek chapter to 7 will be repeat to first chapter
	tmp_nr--;
    }
  }
...
static int ifoRead_PGC(ifo_handle_t *ifofile, pgc_t *pgc, unsigned int offset) {
  ...
  if(pgc->cell_playback_offset != 0 && pgc->nr_of_cells>0) {
    ...
    unsigned int tmp_nr_of_cells = pgc->nr_of_cells;

//printf("1-pgc->nr_of_cells[%d]\n",pgc->nr_of_cells);
...
//                                  pgc->nr_of_cells,
    ...
    pgc->nr_of_cells = tmp_nr_of_cells;
//printf("2-pgc->nr_of_cells[%d]\n",pgc->nr_of_cells);
  } else {
...
/* 
maybe we can using
// http://lists.mplayerhq.hu/pipermail/dvdnav-discuss/2011-August/001426.html
...
//printf("pgcit->pgci_srp[i].pgc->nr_of_cells[%d]\n",pgcit->pgci_srp[i].pgc->nr_of_cells);
  }
