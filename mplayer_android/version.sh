...
# Carlos add 
count=1
while IFS=: read svn_version ;
do 
	if [ $count -eq 11 ] ; then
		#echo $svn_version count is $count
		break;
	fi
	count=`expr $count + 1`
done < .svn/entries
echo "Skyviia mplayer version is $svn_version"
...
NEW_REVISION="#define VERSION \"${version}${extra}-skyviia $svn_version\""
