...
//			vo_spudec = spudec_new_scaled(NULL, display_width, display_height, NULL, 0);
//			vo_spudec = spudec_new_scaled(NULL, video_width, video_height, NULL, 0);// carlos marked 2011-11-17
				vo_spudec = spudec_new_scaled(NULL, ctx->width == 0 ? video_width :ctx->width , ctx->height == 0 ? video_height : ctx->height, NULL, 0);
			//fprintf(stderr, "------------- In [%s][%d] video_width [%d] video_height[%d] ctx->width[%d] ctx->height[%d]---------\n", __func__, __LINE__, video_width, video_height, ctx->width, ctx->height);
			}
#ifdef DVB_SUBTITLES
		pgs_subtitle_flag = 2;	//draw osd
		...
		if(sh->type == 'b')
		{
			...
			for(i = 0; i < sub.num_rects; i++)
			{
				...
				sub.rects[sub.num_rects-i-1]->pict.data[0],
				sub.rects[sub.num_rects-i-1]->pict.linesize[0],
				sub.rects[sub.num_rects-i-1]->pict.data[1],
				sub.rects[sub.num_rects-i-1]->x,
				sub.rects[sub.num_rects-i-1]->y,
				sub.rects[sub.num_rects-i-1]->w,
				sub.rects[sub.num_rects-i-1]->h,
				*pts,
				*endpts,
				sh->type,
				i);
