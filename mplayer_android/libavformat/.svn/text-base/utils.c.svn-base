...
static int av_read_frame_internal(AVFormatContext *s, AVPacket *pkt)
{
    ...
    for(;;) {
        if(cunt > 800)
            {
                printf("!!!!av_read_frame_internal loop cunt > 800\n");
                fflush(stdout);
...
int av_read_frame(AVFormatContext *s, AVPacket *pkt)
{
    ...
    if(speed_mult < 0) av_read_frame_flush(s);	//Fuchun 2010.07.16
...
//                    av_log(NULL, AV_LOG_ERROR, "%f\n", dur);
	...
	//don't need has parameter     Fuchun 2010.03.23 disable
//        if (!has_codec_parameters(st->codec))
