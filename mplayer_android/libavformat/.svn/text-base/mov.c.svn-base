...
int decode_264_sps_mov(const uint8_t *buf, int buf_size)
{
	GetBitContext gb;
	const uint8_t *ptr;
	int i, v, k;
...
nsc:
	...
	{
		//Barry 2010-10-26
		if ( (((mov_width) >= 1920) || ((mov_height) >= 1080)) && (num_ref_frames == 5) )
...
static int mov_read_cmov(MOVContext *c, ByteIOContext *pb, MOVAtom atom)
{
    ...
    atom.size = moov_len;
#ifdef DEBUG
//    { int fd = open("/tmp/uncompheader.mov", O_WRONLY | O_CREAT, 0666); write(fd, moov_data, moov_len); close(fd); }
#endif
...
static int mov_read_header(AVFormatContext *s, AVFormatParameters *ap)
{
	...
	is_FD = 0;	//Fuchun 2010.08.11 reset global variable
 ...
 retry:
	...
	if(st->codec->codec_id == CODEC_ID_H264) 
	{
		...
		if(first_start_code == 1 && st->codec->extradata_size != 0)
		{
			...
			if(p[4] == 0xFD) is_FD = 1;	//Fuchun 2010.02.01
        		...
        		for (i = 0; i < cnt; i++) 
			{
				...
				num_reorder_frames = decode_264_sps_mov(p, nalsize);
            			p += nalsize;
				hdr_size += nalsize + 2;		
        		}
			...
			if(is_FD)
			{
				ret = av_new_packet(pkt, sample->size + hdr_size + 2);
				pkt->pos = url_ftell(sc->pb);
				ret = get_buffer(sc->pb, pkt->data + hdr_size + 2, sample->size);
				ret += 2;
				pkt->data[hdr_size] = pkt->data[hdr_size+1] = 0;
			}
			else
			{
				ret= av_new_packet(pkt, sample->size + hdr_size);
    				pkt->pos= url_ftell(sc->pb);
				ret= get_buffer(sc->pb, pkt->data + hdr_size, sample->size);
			}
...
//			st->codec->extradata_size = 0;
		...
		else
		{
			...
			if(is_FD)
			{
				ret = av_new_packet(pkt, sample->size + 2);
				pkt->pos = url_ftell(sc->pb);
				ret = get_buffer(sc->pb, pkt->data + 2, sample->size);
				ret += 2;
...
//			pkt->data[3] = 1;
	...
	else if(st->codec->codec_id == CODEC_ID_MPEG4 && st->codec->extradata_size != 0)		//Fuchun 2010.06.21
	{
		unsigned char *p = st->codec->extradata;
		int hdr_size = st->codec->extradata_size;

		ret= av_new_packet(pkt, sample->size + hdr_size);
    		pkt->pos= url_ftell(sc->pb);
		ret= get_buffer(sc->pb, pkt->data + hdr_size, sample->size);

		memcpy(pkt->data, p, hdr_size);
		st->codec->extradata_size = 0;
	}
	else
#endif		
    ...
    if (sc->ctts_data) {
        //pkt->pts = pkt->dts + sc->dts_shift + sc->ctts_data[sc->ctts_index].duration;	//disable Fuchun 2010.03.26
