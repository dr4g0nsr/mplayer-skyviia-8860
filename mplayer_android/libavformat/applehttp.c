...
//printf("try  url_fopen, nurl_ptr=%s\n", nurl_ptr);
...
static int applehttp_read_header(AVFormatContext *s, AVFormatParameters *ap)
{
    ...
    char *nurl_ptr;
//printf("@@ in applehttp_read_header,  s->filename=%s\n", s->filename);
    nurl_ptr = strstr(s->filename, "http");

//    if ((ret = parse_playlist(c, s->filename, NULL, s->pb)) < 0)
//printf("   new nurl_ptr=%s\n", nurl_ptr);
//    if ((ret = parse_playlist(c, nurl_ptr, NULL, s->pb)) < 0)
...
//printf(" bug Empty playlist\n");
    ...
    for (i = 0; i < c->n_variants; i++) {
        ...
        nurl_ptr = strstr(v->segments[0]->url, "http");

//        ret = av_open_input_file(&v->ctx, v->segments[0]->url, NULL, 0, NULL);
        ret = av_open_input_file(&v->ctx, nurl_ptr, NULL, 0, NULL);
        if (ret < 0)
            goto fail;
        url_fclose(v->ctx->pb);
        v->ctx->pb = NULL;
...
char *nurl_ptr;
    ...
    nurl_ptr = strstr(var->segments[c->cur_seq_no - var->start_seq_no]->url, "http");
