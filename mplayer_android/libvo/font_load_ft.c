...
//Robert 20101029 set default font scale factor = 3.0
float text_font_scale_factor = 3.0;//3.5;
...
#ifndef  FT_BITMAP_CACHE_OFF
//Robert 20101029 force re-load glyph everytime
...
//fprintf(stderr, "\n%d %d cur_alloc:%d cur_cnt:%d\n", desc->pic_b[font]->charwidth, desc->pic_b[font]->charheight, desc->pic_b[font]->current_alloc, desc->pic_b[font]->current_count);
    if (desc->pic_b[font]->current_count >= desc->pic_b[font]->current_alloc) {
#ifdef FT_BITMAP_CACHE_OFF
//fprintf(stderr, "\n%d %d cur_alloc:%d cur_cnt:%d\n", desc->pic_b[font]->charwidth, desc->pic_b[font]->charheight, desc->pic_b[font]->current_alloc, desc->pic_b[font]->current_count);
	int newsize;
	int increment;
	int incr_size;
	if (desc->pic_b[font]->current_alloc == 0)
	{
		incr_size = 128;
	}
	else
#endif
	{
		incr_size = ALLOC_INCR;
	}
	newsize = desc->pic_b[font]->charwidth*desc->pic_b[font]->charheight*(desc->pic_b[font]->current_alloc+incr_size);
	increment = desc->pic_b[font]->charwidth*desc->pic_b[font]->charheight*incr_size;
	desc->pic_b[font]->current_alloc += incr_size;
...
//    off = desc->pic_b[font]->current_count*desc->pic_b[font]->charwidth*desc->pic_b[font]->charheight;
//Robert 20101029 clear next BMP area
    int out_size = desc->pic_b[font]->charwidth*desc->pic_b[font]->charheight;
    off = desc->pic_b[font]->current_count * out_size;
#ifdef FT_BITMAP_CACHE_OFF
    memset(bbuffer+off, 0, out_size);
    memset(abuffer+off, 0, out_size);
#endif
...
static int load_sub_face(const char *name, int face_index, FT_Face *face)
{
	...
	if (err) {
	    ...
	    if (err) {
//	        mp_msg(MSGT_OSD, MSGL_ERR, MSGTR_LIBVO_FONT_LOAD_FT_NewFaceFailed);	//hide this msg Fuchun 2010.03.31
		return -1;
	    }
	}
    }
...
//    t=GetTimer();
    ...
    if (err) {
//	mp_msg(MSGT_OSD, MSGL_WARN, MSGTR_LIBVO_FONT_LOAD_FT_SubFaceFailed);	//hide this msg Fuchun 2010.03.31
	goto gen_osd;
    }
    desc->face_cnt++;
