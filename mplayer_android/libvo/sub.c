...
inline static void vo_update_text_teletext(mp_osd_obj_t *obj, int dxs, int dys)
{
    ...
    vo_font->pic_b[0]->current_count = 0;
#endif
    ...
    for(i=0;i<rows;i++){
        ...
        for(j=0;j<cols;j++){
			...
			if ((sy < 0) || (sy > y))
				sy = y;
			if ((ex < 0) || (ex < x + vo_font->width[tc.unicode]))
				ex = x + vo_font->width[tc.unicode];
			if ((ey < 0) || (ey < y + vo_font->height))
				ey = y + vo_font->height;
                }
            }else{
/*
...
#ifdef FT_BITMAP_CACHE_OFF
      sub_font->pic_b[0]->current_count = 0;
#endif
	  ...
	  prevc = -1;
	    ...
	    // reading the subtitle words from vo_sub->text[]
		 ...
		 mp_msg(MSGT_OSD,MSGL_WARN,"\nMAX_UCS exceeded!\n");
	      ...
	      if (!c) c++; // avoid UCS 0
	      else if(c >= 65536) c = 65535;	//Fuchun 2010.06.10 avoid system crash
		...
		if (c == ' ') {
			...
			//Fuchun 2010.06.10 split the subtitle word if it too long (for Chinese)
			{
				struct osd_text_t *tmp_ott = calloc(1, sizeof(struct osd_text_t));
				...
				else
				{
					tmp_ott->prev = cp_ott;
					cp_ott->next = tmp_ott;
					tmp_ott->osd_kerning = sub_font->charspace + sub_font->width[' '];
					cp_ott = tmp_ott;
				}
				tmp_ott->osd_length = xsize;
				tmp_ott->text_length = char_position;
...
//    obj->bbox.y2=obj->y+obj->params.subtitle.lines*sub_font->height;
	...
	{
		obj->flags&=~OSDFLAG_VISIBLE;
		printf("In [%s][%d] Detect seek die, return. \n", __func__, __LINE__);
		return;
	}
...
void *vo_spudec=NULL;
void *vo_vobsub=NULL;
int vo_teletextsub=0;

inline static void vo_draw_teletext_sub(mp_osd_obj_t* obj, void (*draw_alpha)(int x0, int y0, int w, int h, unsigned char* src, unsigned char* srca, int stride))
{
    if (obj->allocated > 0) {
	vo_teletextsub = 1;
	int offset = obj->bbox.x1 + (obj->bbox.y1 * obj->stride);
	draw_alpha(obj->bbox.x1,obj->bbox.y1,
		   obj->bbox.x2-obj->bbox.x1,
		   obj->bbox.y2-obj->bbox.y1,
		   obj->bitmap_buffer + offset,
		   obj->alpha_buffer + offset,
		   obj->stride);
	vo_teletextsub = 0;
    }
}
...
void vo_draw_text_ext(int dxs, int dys, int left_border, int top_border,
	...
	case OSDTYPE_SPU:
	    ...
	    break;
	case OSDTYPE_TELETEXT:
	    vo_draw_teletext_sub(obj, draw_alpha);
	    break;
#ifdef CONFIG_DVDNAV
        case OSDTYPE_DVDNAV:
#endif
