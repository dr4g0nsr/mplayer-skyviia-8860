...
void RTPInterface::addStreamSocket(int sockNum,
  ...
  for (tcpStreamRecord* streams = fTCPStreams; streams != NULL;
	...
	//printf("%s:%d list: sockNum %d streamChannelId %d\n", __func__, __LINE__, streams->fStreamSocketNum, streams->fStreamChannelId);
  ...
  //printf("%s:%d sockNum %d streamChannelId %d\n", __func__, __LINE__, sockNum, streamChannelId);
}
...
SocketDescriptor::SocketDescriptor(UsageEnvironment& env, int socketNum)
  :fEnv(env), fOurSocketNum(socketNum),
   ...
   fTCPReadingState(AWAITING_DOLLAR), fChId(0xFF), fRTPIface(NULL) {
}
...
void SocketDescriptor::registerRTPInterface(unsigned char streamChannelId,
  ...
  fSubChannelHashTable->Add((char const*)(long)streamChannelId,
			    rtpInterface);
  //printf("%s:%d streamChannelId:%d rtpInterface:%p\n", __func__, __LINE__, streamChannelId, rtpInterface);
#if 0
  //if(fRTPIface == NULL)
  {
  	fChId = streamChannelId;
	fRTPIface = rtpInterface;
	printf("%s:%d set default streamChannelId:%d RTPInterface:%p\n", __func__, __LINE__, streamChannelId, rtpInterface);
  }
#endif
...
::lookupRTPInterface(unsigned char streamChannelId) {
  ...
  RTPInterface* Iface = (RTPInterface*)fSubChannelHashTable->Lookup(lookupArg);
#if 0
  if(Iface == NULL){
  	printf("%s:%d lookup fail, use default RTPInterface %p\n", __func__, __LINE__, fRTPIface);
	Iface = fRTPIface;
  }
#endif
  return Iface;
...
::deregisterRTPInterface(unsigned char streamChannelId) {
  ...
  //printf("%s:%d streamChannelId:%d\n", __func__, __LINE__, streamChannelId);
#if 0
  if(streamChannelId == fChId){
  	printf("%s:%d clear default streamChannelId:%d RTPInterface:%p\n", __func__, __LINE__, fChId, fRTPIface);
	fChId = 0xFF;
	fRTPIface = NULL;
  }
#endif
...
void SocketDescriptor::tcpReadHandler1(int mask) {
  ...
  if (fTCPReadingState != AWAITING_PACKET_DATA) {
	...
	//printf("readSocket result:%d c:0x%x(%c)\n", result, c, c);
  ...
  switch (fTCPReadingState) {
    ...
    case AWAITING_STREAM_CHANNEL_ID: {
		//printf("AWAITING_STREAM_CHANNEL_ID\n");
    ...
    case AWAITING_SIZE2: {
	  ...
	  //printf("fStreamChannelId %u rtpInterface %p\n", fStreamChannelId, rtpInterface);
