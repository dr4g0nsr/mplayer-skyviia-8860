...
unsigned RTSPClient::responseBufferSize = 40960; // default value; you can reassign this in your application if you need to
...
void RTSPClient::resetResponseBuffer() {
  ...
  //envir() << __func__ << ":" << __LINE__ << " fResponseBytesAlreadySeen:" << fResponseBytesAlreadySeen << " fResponseBufferBytesLeft:" << fResponseBufferBytesLeft << "\n";
}
...
unsigned RTSPClient::sendRequest(RequestRecord* request) {
  ...
  do {
...
#if 0	
	char *bandwidthStr = "Bandwidth: 2383345\r\n";
	char *accelerateStr = "X-Accelerate-Streaming: AccelDuration=18000;AccelBandwidth=2147483647\r\n";
	extraHeaders = new char[strlen(sessionStr) + strlen(scaleStr) + strlen(rangeStr) + strlen(bandwidthStr) + strlen(accelerateStr) + 1];
	sprintf(extraHeaders, "%s%s%s%s%s", sessionStr, scaleStr, rangeStr, bandwidthStr, accelerateStr);
#else
    ...
    if (fVerbosityLevel >= 1) envir() << "Sending request:\n" << cmd << "\n";
...
#if 0 // 20110407 charleslin
      if(rtpChannelId == 2)
	  	rtpChannelId = 0;	// both audio and video rtp channel id are 0
#endif
      foundChannelIds = True;
      //envir() << __func__ << ":" << __LINE__ << " rtpChannelId:" << rtpChannelId << " rtcpChannelId:" << rtcpChannelId << "\n";
...
void RTSPClient::handleResponseBytes(int newBytesRead) {
  //envir() << __func__ << ":" << __LINE__ << " newBytesRead:" << newBytesRead << "\n";
  do {
    //envir() << __func__ << ":" << __LINE__ << " newBytesRead:" << newBytesRead << " fResponseBufferBytesLeft:" << fResponseBufferBytesLeft << "\n";
  ...
  //envir() << __func__ << ":" << __LINE__ << " fResponseBytesAlreadySeen:" << fResponseBytesAlreadySeen << " fResponseBufferBytesLeft:" << fResponseBufferBytesLeft << " newBytesRead:" << newBytesRead << "\n";
  ...
  if (fResponseBytesAlreadySeen > 3) {
    ...
    while (ptr < ptrEnd) {
      if (*ptr++ == '\r' && *ptr++ == '\n' && *ptr++ == '\r' && *ptr++ == '\n') {
        // This is it
        //envir() << __func__ << ":" << __LINE__ << " endOfHeaders\n";
        endOfHeaders = True;
        break;
      }
    }
  ...
  do {
	...
	//envir() << __func__ << ":" << __LINE__ << " reachedEndOfHeaders:" << reachedEndOfHeaders << "\n";
	...
	//envir() << __func__ << ":" << __LINE__ << " foundRequest:" << foundRequest << "\n";
    ...
    if (contentLength > numBodyBytes) {
	  ...
	  //envir() << "contentLength:" << contentLength << "\n";
	  //envir() << "numBodyBytes:" << numBodyBytes << "\n";
	  //envir() << "numExtraBytesNeeded:" << numExtraBytesNeeded << "\n";
	  //envir() << "responseBufferSize:" << responseBufferSize << "\n";
	  //envir() << "fResponseBytesAlreadySeen:" << fResponseBytesAlreadySeen << "\n";
	  //envir() << "remainingBufferSize:" << remainingBufferSize << "\n";
	  
      if (numExtraBytesNeeded > remainingBufferSize) {
        char tmpBuf[200];
		envir() << "Received an incomplete "
