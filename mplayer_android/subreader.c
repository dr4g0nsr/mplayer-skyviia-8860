...
extern int sub_temp_utf8;	//Fuchun 2010.06.08
...
static subtitle *sub_read_line_sami(stream_t* st, subtitle *current, int utf16) {
    ...
    //Polun Huang 2011-6-30 ++s    
    int byte_cunt = 0;
    int checkflg = 0;
    //Polun Huang 2011-6-30 ++e
    do {
	switch (state) {
	...
	case 3: /* get all text until '<' appears */
		...
		*p = '\0'; p = text; trail_space (text);
	    ...
	    else if(!strncasecmp (s, "&lt;/i&gt;", 10)) {	//Fuchun 20110726 transform symbol
		    *p = '\0'; p = text; trail_space (text);
		    if (text[0] != '\0')
			    current->text[current->lines++] = strdup (text);
		    s += 10;
	    }
	    else if ((*s == '{') && !sub_no_text_pp && !checkflg) { state = 5; ++s; continue; } //Polun Huang 2011-6-30  add checkflg
	    else if (*s == '<') { state = 4; }
	    else if (!strncasecmp (s, "&nbsp;", 6)) { *p++ = ' '; s += 6; }
	    else if (!strncasecmp (s, "&lt;i&gt;", 9)) { *p++ = ' '; s += 9; }	//Fuchun 20110726 transform symbol
	    else if (*s == '\t') { *p++ = ' '; s++; }
	    else if (*s == '\r' || *s == '\n') { s++; }
	    else *p++ = *s++;
	...
	case 4: /* get current->end or skip <TAG> */
       ...
       case 5: /* get rid of {...} text, but read the alignment code */
	    ...
	    if (*s == '}') state = 3;
           //Polun Huang 2011-6-30 ++s  for no get '}' , the state is not right  
           byte_cunt += 1;
           if (*s == '\0')
           {
               printf("The subtitle state is not right \n");
               state = 3;
               checkflg = 1;
               s -= byte_cunt ;
               continue;
 ...
 */
        int comma;
        static int max_comma = MAX_COMMA_NUM; /* let's use 32 for the case that the */
	...
	do {
	    ...
	    //20120110 charleslin - '{' and '}' must be in pair
	    if(tmp1 && tmp2 && tmp1 < tmp && tmp < tmp2) break;
            if(*(++tmp) == ' ') break;
                  /* a space after a comma means we're already in a sentence */
            line2 = tmp;
          }

	//20120110 charleslin - only update max_comma once
        if(max_comma == MAX_COMMA_NUM && comma < max_comma) max_comma = comma;
...
subtitle *temp_sub = NULL;		//Fuchun 2010.06.11

void subcp_open_1(void)	//Fuchun 2010.06.08
{
	char *tocp = "UTF-8";
...
{
	int l=sub->lines;
	size_t ileft, oleft, sub_size;
	char *op, *ip, *ot;
	if(icdsc == (iconv_t)(-1)) return sub;

	{
		int k = temp_sub->lines;
		while(k)
		{
			free(temp_sub->text[--k]);
		}
		temp_sub->end = sub->end;
		temp_sub->start = sub->start;
		temp_sub->lines = sub->lines;
		temp_sub->alignment = sub->alignment;
	}
	...
	while (l){
		...
		if (iconv(icdsc, &ip, &ileft,
			  &op, &oleft) == (size_t)(-1)) {
//			mp_msg(MSGT_SUBREADER,MSGL_WARN,"SUB: error recoding line.\n");	//Fuchun 2010.06.08 disable, avoid system to lag
			free(ot);
			sub_size = strlen(sub->text[l]);
			temp_sub->text[l] = malloc(sub_size+1);
			memcpy(temp_sub->text[l], sub->text[l], sub_size);
			memset(temp_sub->text[l] + sub_size, '\0', 1);
			temp_sub->endpts[l] = sub->endpts[l];
			continue;
		}
 ...
 */
subtitle* sub_fribidi (subtitle *sub, int sub_utf8, int from)
{
...
//Fuchun 2010.10.28
static void microdvd_adjust_subs_time(subtitle* sub, float subtime, float fps, int block,
                             int sub_num, int sub_uses_time) {
	int n,m;
	subtitle* nextsub;
	int i = sub_num;
	unsigned long subfms = (sub_uses_time ? 100 : fps) * subtime;
	unsigned long overlap = (sub_uses_time ? 100 : fps) / 5; // 0.2s
	float adjust_fps = 0.0;
	if(sub->start == 1 && sub->end == 1)
		sscanf(sub->text[0], "%f", &adjust_fps);
	if(adjust_fps == fps)
		adjust_fps = 0.0;
	...
	if (i)	for (;;){
		 ...
		 * where find_sub is called */
		if(adjust_fps) {
			sub->start *= fps/adjust_fps;
			sub->end   *= fps/adjust_fps;
		}
...
//	    if (k<0) subcp_open(fd);	//Fuchun 2010.06.08 disable
		if (k<0 && icdsc == (iconv_t)(-1)) 
		{
			subcp_open_1();
			sub_temp_utf8 = sub_utf8;
			if(temp_sub == NULL) 
			{
	...
	memset(sub, '\0', sizeof(subtitle));
        ...
        if(!sub) break;   // EOF
#ifdef CONFIG_ICONV
//	if ((sub!=ERR) && sub_utf8 == 2) sub=subcp_recode(sub);	//Fuchun 2010.06.08 disable
#endif
#ifdef CONFIG_FRIBIDI
//	if (sub!=ERR) sub=sub_fribidi(sub,sub_utf8,0); //Fuchun 20120105 disable
#endif
	if ( sub == ERR )
	 {
#ifdef CONFIG_ICONV
...
#ifdef CONFIG_ICONV
//    subcp_close();		//Fuchun 2010.06.08 disable
#endif
    free(alloced_sub);
...
else if(suboverlap_enabled && sub_format == SUB_MICRODVD)	//Fuchun 2010.10.28
{
	microdvd_adjust_subs_time(first, 6.0, fps, 1, sub_num, uses_time);/*~6 secs AST*/
	return_sub = first;
}
...
void sub_add_text(subtitle *sub, const char *txt, int len, double endpts) {
  ...
  if (sub->lines < SUB_MAX_TEXT &&
    ...
    sub->lines++;
#ifdef CONFIG_FRIBIDI
  if(sub_cp && !strncasecmp(sub_cp, "cp1256", 6))
