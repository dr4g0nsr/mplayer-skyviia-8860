...
extern int ao_fd;             // 2010/10/4 Honda for DD
...
static int init(int rate,int channels,int format,int flags){
  ...
  ao_fd = audio_fd;             // 2010/10/4 Honda for DD 
...
ac3_retry:
  ...
  {
  	int tmp_oss_format;
	tmp_oss_format = 0x20111114;  //for DTS certification
  
  	if( ioctl(audio_fd, SNDCTL_DSP_SETFMT, &tmp_oss_format) < 0 )
  	{
  	    printf("############  FAIL   [%s - %d]   oss_format =0x%x     tmp_oss_format = 0x%x\n", __func__, __LINE__, oss_format, tmp_oss_format);
	    mp_msg(MSGT_AO,MSGL_WARN, MSGTR_AO_OSS_CantSet, dsp,
	            af_fmt2str_short(format), af_fmt2str_short(AF_FORMAT_S16_NE) );
  	}
	else
		printf("############  SUCCESS    [%s - %d]    oss_format =0x%x     0x20111114\n", __func__, __LINE__, oss_format);
  }
  else
#endif
...
//Robert 20100526 fix play/pause
// stop playing, keep buffers (for pause)
static void audio_pause(void)
{
    //prepause_space = get_space();//original
    //uninit(1);//original
	int val = 1;//chiyau: 20091015
    ioctl(audio_fd, 0x88500000, &val);//chiyau: 20091015
}
...
#else
  {
//20100728 Robert use svread() instead of ioctl
      vdec_svread_buf_t nrb;
      svread(1, &nrb, 0);
      memcpy(&zz, &nrb.abinfo, sizeof(audio_buf_info));
#endif
      //Barry 2010-09-21
      if (zz.fragments < 0)
	  	printf("BUG ao_oss: frags: %d < 0, need to check this!\n", zz.fragments);
...
static int play(void* data,int len,int flags){
	...
	if(len>ao_data.outburst || !(flags & AOPLAY_FINAL_CHUNK))
	{
		...
		printf("%s:%d s:%x len:%x\n", __func__, __LINE__, s, len);
#endif
		len=write(audio_fd,data,len);
	}
	else
	{
		memset(data+len, 0, ao_data.outburst-len);
#if 0
		unsigned short *buf = (unsigned short *)data;
		unsigned int s = 0;
		int i;
		for(i=0; i<ao_data.outburst/2; i++){
			s+= buf[i];
		}
		printf("%s:%d s:%x len:%x\n", __func__, __LINE__, s, len);
#endif
...
//2010/10/6 Honda for DD 
int oss_get_frag(int *fragments, int *fragstotal, int *fragsize)
{
   if(ioctl(audio_fd, SNDCTL_DSP_GETOSPACE, &zz)!=-1){
      *fragments = zz.fragments;
      *fragstotal = zz.fragstotal; 
      *fragsize = zz.fragsize;
      return 1;
   }
   else
      return 0;
