...
unsigned int get_output_channels = 0;		//Fuchun 2011.05.17
int ad_real_channels = 0;	//Fuchun 2011.05.17
int change_flag;
...
extern unsigned int thumbnail_mode;	 //SkyViia_Vincent03232010
extern char subtitle_names[];
extern int sub_change_success;
extern char audio_names[];
extern char video_names[];
extern int audio_change_success;
extern int h264_frame_mbs_only;
extern int decode_num_cnt;
//extern int vo_need_osd;
//extern int mpeg_fast;
extern int top_field_frame;
...
extern int get_curpos_on;
extern int get_buffering_on;
extern pthread_t ipc_thread_curpos_id;
extern pthread_t ipc_thread_buffering_id;
extern int is_dvdnav;
extern int dvdnav_movie_sub_id;
extern int dvdnav_movie_audio_id;
extern int subcc_enabled;
extern int subcc_force_off;
extern int has_subcc;
extern int subcc_reg;

extern void ipc_callback_curpos(void);
extern void ipc_callback_buffering(void);
extern int audio_not_support;
extern int seek_by_time_done;
extern int duration_sec_cur;
extern unsigned int skydroid;
int tmp_sid=0;
int needs_change_dsub_id=0;
extern int set_disp_off;	//Barry 2011-09-19
#ifdef DTS_CERTIFICATION	
extern int SpkrOut;
#endif

#ifdef SUPPORT_SKYDVB_DYNAMIC_PID
static void do_dvb_switch(char *cmd, MPContext *mpctx);
...
//+Skyviia_Vincent11232010 return to menu if no chapter num
    clear_osd();
    //if (mpctx->demuxer->num_chapters == 0)
        stream_control(mpctx->demuxer->stream, STREAM_CTRL_GET_NUM_CHAPTERS, &mpctx->demuxer->num_chapters);

    if(chapter == 0 && 
       mpctx->demuxer->num_chapters == 1 &&
       mpctx->stream->type == STREAMTYPE_DVDNAV &&
       (action == M_PROPERTY_STEP_UP|| action == M_PROPERTY_STEP_DOWN)
      )
    {
        int title_nr=0;
        int title_cur=0;
        int title_switch_to=0;
        printf("Mplayer:No chapter, using switch title\n");

        title_cur = mp_dvdnav_get_cur_title(mpctx->stream,&title_nr);
        step_all = (arg && *(int*)arg != 0 ? *(int*)arg : 1)
                   * (action == M_PROPERTY_STEP_UP ? 1 : -1);

        if(title_cur == 1 || title_cur == 0)
            title_cur = 2;
        title_switch_to = title_cur+step_all;
//printf("step_all[%d],title_switch_to[%d]\n",step_all,title_switch_to);
        if(title_switch_to <= title_nr)
        {
            mp_dvdnav_switch_title(mpctx->stream, title_switch_to);

            if(skydroid) //Mantis:3849
            {
		...
		sprintf(callback_str,"ChapterInfo=%02d,%02d\0",chapter+1,mpctx->demuxer->num_chapters);
        ...
        {
            int button = -1;
            printf("End of title, return to dvdnav menu\n");
            mp_dvdnav_handle_input(mpctx->stream,MP_CMD_DVDNAV_MENU,&button);            
        }

        return M_PROPERTY_OK;
    }
//Skyviia_Vincent11232010+
    ...
    if (chapter >= 0) {
	if(is_dvdnav)	//Fuchun 2011.04.21 fix a/v not sync when switch between menu and movie repeatedly or seek chapter
	{	
		extern unsigned int resync_video; //Mantis:5895 jumanji mosaic while seek chapter
		resync_video = 2;
		change_flag = 2;
	}
	...
	if(mpctx->sh_audio && mpctx->audio_out)
        ...
        if (next_pts > -1.0) {
            ...
            abs_seek_pos = 0;
            rel_seek_secs = next_pts;
			/* fixed mantis 4702, seek_chapter caused mosaic, kernel driver slow down the speed of AXI */
			abs_seek_pos |= SEEK_CHAPTER;
	}
#endif
        if (chapter_name)
    ...
    if (chapter_name)
        free(chapter_name);
    if ((action == M_PROPERTY_STEP_UP) || (action == M_PROPERTY_STEP_DOWN) ||
	    (action == M_PROPERTY_SET))
    {
	char callback_str[256];
	sprintf(callback_str,"ChapterInfo=%02d,%02d\0",chapter+1,chapter_num);
	ipc_callback(callback_str);

	//Mantis:4564 BOLT seek_chapter cause set subtitle and audio from NAV command
	extern int seek_chapter;
	if(is_dvdnav && seek_chapter==1)
		seek_chapter = 0;
    }
    return M_PROPERTY_OK;
}
                               ...
                               MPContext *mpctx)
{
    ...
    if (angles <= 1)
    {
        if(action == M_PROPERTY_STEP_UP)
        {
            sprintf(callback_str, "Angle: 1/1");
            ipc_callback(callback_str);
        }
        return M_PROPERTY_UNAVAILABLE;
    }
    ...
    sleep(1); //switch angle needs some time. Let switch_angle more stable with more frequent.
    sprintf(callback_str, "Angle: %d/%d",angle ,angles);
    ipc_callback(callback_str);
...
pthread_t thread_switch_audio;
int switch_audio_current_id;
extern int switch_audio_thread_status;
int not_support_audio_id = -2;	//Barry 2011-02-27
extern unsigned int sky_hwac3;
int do_switch_audio_flag = -1;

void do_switch_audio_pthread(MPContext * mpctx)
{
	int len;
	int audio_prequeue = 0;
	char callback_str[256];

	if (switch_audio_thread_status)
	{
#ifdef HW_TS_DEMUX
		if(switch_audio_thread_status == 2)
		{
			...
			{
				while(!check_need_write_ts_video(mpctx->demuxer->priv, 1))
				{
					usleep(1000);
				}
			}
...
#endif
//printf("[%s - %d]audio_id=%d, mpctx->demuxer->audio->id=%d, current_id=%d\n", __func__, __LINE__, audio_id, mpctx->demuxer->audio->id, switch_audio_current_id);
		if (audio_id == -2 || (audio_id > -1 && mpctx->demuxer->audio->id != switch_audio_current_id && switch_audio_current_id != -2))
			uninit_player(INITIALIZED_ACODEC | INITIALIZED_AO);
		if (audio_id > -1 && mpctx->demuxer->audio->id != switch_audio_current_id)
		{
			sh_audio_t *sh2;
//Robert 20110322 fine tune AVsync while switch audio channel for AVI/MKV
			if (mpctx->d_audio && mpctx->d_audio->packs > 0)
			{
                            if (mpctx->d_audio->packs > 1)
                            {
...
#ifdef AVI_NOT_SORT
			if(mpctx->demuxer->type == DEMUXER_TYPE_AVI_NI)
				demux_avi_read_audio_index(mpctx->demuxer);
#endif
			...
			if (sh2)
			{
				...
				//if ( (sh2->format == 0x2001 && !svsd_state->dec_cap.dts) || (sh2->format == 0x2000 && !svsd_state->dec_cap.dolby) )
				if ((((sh2->format == 0x2001 && !svsd_state->dec_cap.dts) || (sh2->format == 0x2000 && !svsd_state->dec_cap.dolby) || (sh2->format == mmioFOURCC('T', 'R', 'H', 'D')) || (sh2->format == mmioFOURCC('A', 'C', '3', 'P')) || (sh2->format == mmioFOURCC('E', 'A', 'C', '3'))) && !sky_hwac3)
					|| (sh2->format == 0x4D435042 && sh2->bpcm_channels != 0))
				{
					//Barry 2011-03-01
					if((mpctx->demuxer->type == DEMUXER_TYPE_MPEG_TS) && hwtsdemux) 
					{
						/* Stop audio filter */
						ts_priv_t* priv = (ts_priv_t *)mpctx->demuxer->priv;
						if (priv->fifo[0].pfd)
							dvb_demux_stop(priv->fifo[0].pfd);
					}

					len = sprintf(audio_names, "No Audio can switch\0");
...
#endif
				{
					...
					if (!mpctx->sh_audio)
					{
#if 0	//Barry 2011-07-04  //Fuchun 20110712 disable
						if((mpctx->demuxer->type == DEMUXER_TYPE_MPEG_TS) && hwtsdemux && (sh2->format == 0x4D435042))
						{
							/* Stop audio filter */
							ts_priv_t* priv = (ts_priv_t *)mpctx->demuxer->priv;
							if (priv->fifo[0].pfd)
						...
						len = sprintf(audio_names, "AUDIO: No Audio can switch\0");
		...
		if(mpctx->sh_audio && switch_audio_thread_status != 2)
		{
			char lang[40] = MSGTR_Unknown_Audio;
			char a_codec_name[1024];
			sh_audio_t* sh = mpctx->sh_audio;
			float showchannels = 0.0;
			int a_total_num = 0, a_current_num = 0;

			if(sh && sh->codec)
			{
				if(!strcmp(sh->codec->drv, "saiaac3") || !strcmp(sh->codec->drv, "saac3"))
					sprintf(a_codec_name, "Dolby D.");
				else if(!strcmp(sh->codec->drv, "sara"))
					sprintf(a_codec_name, "RA");
				else if(!strcmp(sh->codec->drv, "sawmapro"))
					sprintf(a_codec_name, "WMA-PRO");
				else if(!strcmp(sh->codec->drv, "samp3"))
					sprintf(a_codec_name, "MP3");
				else if(!strcmp(sh->codec->drv, "samchmp3"))
					sprintf(a_codec_name, "Multi-Channel_MP3");
				else if(!strcmp(sh->codec->drv, "saaac"))
					sprintf(a_codec_name, "AAC");
				else if(!strcmp(sh->codec->drv, "saflac"))
					sprintf(a_codec_name, "FLAC");
				else if(!strcmp(sh->codec->drv, "skdts") || !strcmp(sh->codec->drv, "sadts") || !strcmp(sh->codec->drv, "sadtshd2"))
					sprintf(a_codec_name, "DTS");
				else if(!strcmp(sh->codec->drv, "sawma"))
					sprintf(a_codec_name, "WMA");
				else if(!strcmp(sh->codec->drv, "saogg"))
					sprintf(a_codec_name, "OGG3");
				else if(!strcmp(sh->codec->drv, "saamrnb"))
					sprintf(a_codec_name, "AMR_Narrowband");
				else if(!strcmp(sh->codec->drv, "hwac3"))
				{
                                   #if 0 //Polun 2011-11-1 fixed sh->codec->fourcc not correction cause audio info error
                                   if (sh->codec->fourcc == 0x2001)
                                   #else
                                   if (sh->codec->fourcc == 0x2001 || sh->format == 0x2001)
                                   #endif
						sprintf(a_codec_name, "DTS");
					else
						sprintf(a_codec_name, "Dolby D.");
					//	 	sprintf(a_codec_name, "AC3/DTS");
				}
				else
					sprintf(a_codec_name, "%s", sh->codec->name);
			}
			else
			{
				sprintf(a_codec_name, "%s", "No Audio can switch");
				audio_change_success = 1;
				//strcpy(callback_str,"status: playing 2\0");
				strcpy(callback_str,"status: playing 4\0");
				ipc_callback(callback_str);
				audio_not_support = 1;
			...
			if(get_output_channels == 0)	//Fuchun 2011.05.17
			{	
				if(sh->wf)
					ad_real_channels = sh->wf->nChannels;
#ifdef PARSE_REAL_CHANNELS
				else
					ad_real_channels = get_audio_channels(sh);
#endif
				if(ad_real_channels > 5)
					showchannels = (float)(ad_real_channels - 1) + 0.1;
				else
					showchannels = (float)(ad_real_channels);
			}
			else
			{
				if(sh->channels > 5)
					showchannels = (float)(sh->channels - 1) + 0.1;
				else
					showchannels = (float)(sh->channels);
			}

			if(is_dvdnav)	//Fuchun 20110729 calculate audio total number and current number
			{
				int i;
				for(i = 0; i < 8; i++)
				{
					if(dvdnavstate.aid[i] != -1)
						a_total_num++;
//printf("mpctx->d_audio->id[%d], dvdnavstate.stream_aid[%d]\n",mpctx->d_audio->id,dvdnavstate.stream_aid[i]);
					if(dvdnavstate.stream_aid[i] == mpctx->d_audio->id)
						a_current_num = a_total_num;
				}
			}
			else
			{
				...
				{
					if(mpctx->demuxer->a_streams[i])
						a_total_num++;
					if(i == mpctx->d_audio->id)
						a_current_num = a_total_num;
				}
			}
			...
			{
				int code = dvd_lang_from_aid(mpctx->stream, mpctx->demuxer->audio->id);
				if (code)
				{
					lang[0] = code >> 8;
					lang[1] = code;
			...
			{
				mp_dvdnav_lang_from_aid(mpctx->stream, mpctx->demuxer->audio->id, lang);
				if(is_dvdnav)
				{
					int i;
					for(i = 0; i < 8; i++)
			...
			if(sh)
			{
				...
				if(0<=SpkrOut && SpkrOut<=14)
					len = sprintf(audio_names, "%d-%d-%d %s %s %2.2fkHz\0", sh->aid, a_current_num, a_total_num, lang, a_codec_name, (float)(sh->samplerate/1000));
				else
#endif
					len = sprintf(audio_names, "%d-%d-%d %s %s %1.1fCH %2.2fkHz\0", sh->aid, a_current_num, a_total_num, lang, a_codec_name, showchannels, (float)(sh->samplerate/1000));
			}
			else
			{
#ifdef DTS_CERTIFICATION
				if(0<=SpkrOut && SpkrOut<=14)
					len = sprintf(audio_names, "%d-%d-%d %s %s %2.2fkHz\0", mpctx->d_audio->id, a_current_num, a_total_num, lang, a_codec_name, (float)(sh->samplerate/1000));
				else
#endif
					len = sprintf(audio_names, "%d-%d-%d %s %s %1.1fCH %2.2fkHz\0", mpctx->d_audio->id, a_current_num, a_total_num, lang, a_codec_name, showchannels, (float)(sh->samplerate/1000));
			}
			audio_change_success = 1;

			if (audio_change_success)
			{
				sprintf(callback_str, "AUDIO: %s\0", audio_names);
				ipc_callback(callback_str);
		...
		printf("===========>    do_switch_audio_pthread down aq[%d] vq[%d]\n", mpctx->d_audio->packs, mpctx->d_video->packs);
	}
}
#endif
...
//Barry 2011-06-29
extern short no_audio_channels;
extern int no_audio_samplerate;
extern int no_audio_format;
/// Selected audio id (RW)
static int mp_property_audio(m_option_t * prop, int action, void *arg,
			     MPContext * mpctx)
{
	int current_id, tmp,len;

#ifdef HAS_DEC_CAPABILITY        //Barry 2011-03-24
	if (not_support_audio_id != -2 && !mpctx->sh_audio)
	{
		mpctx->demuxer->audio->id = not_support_audio_id;		
#if 1
		if (mpctx->demuxer->type != DEMUXER_TYPE_MPEG_TS)
			mpctx->sh_audio = mpctx->demuxer->audio->sh = mpctx->d_audio->sh = mpctx->demuxer->a_streams[mpctx->demuxer->audio->id];
#endif		
...
#if 0
#ifdef SWITCH_AUDIO_THREAD
	if (!switch_audio_thread_status && (!mpctx->sh_audio || !mpctx->demuxer || !mpctx->demuxer->audio))
#else
	if (!mpctx->sh_audio || !mpctx->demuxer || !mpctx->demuxer->audio)
#endif
	{
		/* Carlos add for avoid UI switch audio caused mplayer lock when the file has no audio stream , 2010-12-30 */
		len = sprintf(audio_names, "No Audio can switch\0");
		audio_not_support = 1;
		audio_change_success = 1;
		return M_PROPERTY_UNAVAILABLE;
	...
	if(is_dvdnav)
	{
		dvdnav_movie_audio_id = audio_id;
//int x;
//for(x=0;x<8;x++)
//printf("dvdnavstate.aid[%d],%d\n",x,dvdnavstate.aid[x]);
	}
	...
	switch (action)
	{
		...
		case M_PROPERTY_PRINT:
			if (!arg)
				return M_PROPERTY_ERROR;
			printf("#########mpctx->sh_audio->aid[%d]\n",mpctx->sh_audio ? mpctx->sh_audio->aid : -2);
			if (current_id < 0)
			{
				char *tmp = "SKY_AUDIO_DISABLE";
				len = sprintf(audio_names, "%d %s\0",0,tmp);
				audio_change_success = 1;
			...
			else
			{
				...
				int a_total_num = 0, a_current_num = 0;
#ifdef SWITCH_AUDIO_THREAD
				if (!switch_audio_thread_status && mpctx->sh_audio)
#endif
				{					
					char lang[40] = MSGTR_Unknown_Audio;
					sh_audio_t* sh = mpctx->sh_audio;
					...
					if(get_output_channels == 0)    //Fuchun 2011.05.17
					{
						if(sh->wf)
							ad_real_channels = sh->wf->nChannels;
#ifdef PARSE_REAL_CHANNELS
                                          #if 0  //Polun 2011-10-25 fixed audio eof to get_audio_channels handup. 
						else 
							ad_real_channels = get_audio_channels(sh);
                                          #else
                                           else if(mpctx->d_audio->eof != 1) 
							ad_real_channels = get_audio_channels(sh);
                                          #endif
#endif
						if(ad_real_channels > 5)
...
#ifdef DTS_CERTIFICATION
						if(0<=SpkrOut && SpkrOut<=14)
							len = sprintf(audio_names, "%d-%d-%d %s %s %2.2fkHz\0", current_id, a_current_num, a_total_num, lang, a_codec_name, (float)(sh->samplerate/1000));
						else
#endif
							len = sprintf(audio_names, "%d-%d-%d %s %s %1.1fCH %2.2fkHz\0", current_id, a_current_num, a_total_num, lang, a_codec_name, showchannels, (float)(sh->samplerate/1000));
					}
					audio_change_success = 1;
...
#if 0	//Barry 2011-07-06 disable	//Barry 2011-06-29
				else
				{
					if (no_audio_format && no_audio_samplerate && no_audio_channels)
					{
						if (no_audio_format == 0x43614C66)
							sprintf(a_codec_name, "FLAC");
						else if (no_audio_format == 0x2001)
							sprintf(a_codec_name, "DTS");
						else if (no_audio_format == 0x2000)
							sprintf(a_codec_name, "Dolby");
						else if (no_audio_format == 0x4D435042)
							sprintf(a_codec_name, "LPCM");

						if(get_output_channels == 0)
						{
							ad_real_channels = no_audio_channels;
						...
						len = sprintf(audio_names, "AUDIO: 1 audio %s %1.1fCH %2.2fkHz\0", a_codec_name, showchannels, (float)(no_audio_samplerate/1000));
						ipc_callback(audio_names);
					}
				}
#endif
		...
		case M_PROPERTY_SET:
			...
			//printf("[%s - %d]=====>  audio_id = %d\n", __func__, __LINE__, audio_id);
			audio_id = demuxer_switch_audio(mpctx->demuxer, tmp);
			//printf("[%s - %d]=====>  audio_id = %d\n", __func__, __LINE__, audio_id);
			if(mpctx->osd_function == OSD_PAUSE)	//Fuchun 2011.02.10
				return M_PROPERTY_OK;
			...
			if (!switch_audio_thread_status)
			{
				...
				switch_audio_thread_status = 1;
				switch_audio_current_id = current_id;
//printf("current_id[%d], mpctx->demuxer->audio->id[%d], audio_id[%d]\n",current_id,mpctx->demuxer->audio->id,audio_id);
                            #if 0  //Polun 2011-10-28 fixed mantis 6422 
				if (audio_id == -2 || (audio_id > -1 && mpctx->demuxer->audio->id != switch_audio_current_id && switch_audio_current_id != -2))
                            #else
				if (audio_id == -2 || (audio_id > -1 && mpctx->demuxer->audio->id != switch_audio_current_id))
                            #endif
				{
					if (pthread_create(&thread_switch_audio, NULL, (void *)do_switch_audio_pthread, mpctx) == 0)
					{
						printf("### switch_audio: pthread_create aq [%d] vq[%d]###\n", mpctx->d_audio->packs, mpctx->d_video->packs);
					}
					else
				...
				{
					printf("### switch_audio: audio same as last audio, don't need switch audio ###\n");
					switch_audio_thread_status = 0;
                                   //Polun 2011-10-25 ++s fixed switch audio the same audio track because I2S is no data cause noise.
		                     if ((mpctx->d_audio->eof != 1) && (mpctx->d_video->eof != 1) && (mpctx->demuxer->audio->id == switch_audio_current_id))
			                  seek_sync_flag = 4;
                                   //Polun 2011-10-25 ++e
				}
			}
...
static void update_global_sub_size(MPContext *mpctx)
{
	...
	if (mpctx->global_sub_indices[SUB_SOURCE_SUBS] != -1 
		&& cnt+mpctx->global_sub_indices[SUB_SOURCE_DEMUX] > mpctx->global_sub_indices[SUB_SOURCE_SUBS])
		mpctx->global_sub_indices[SUB_SOURCE_SUBS] = cnt+mpctx->global_sub_indices[SUB_SOURCE_DEMUX];

	if(mpctx->stream->type == STREAMTYPE_DVDNAV)
	{
		mpctx->global_sub_size = dvdnav_globle_num_subtitle;
		if(has_subcc)	//Fuchun 20110823 add subcc into global_sub_size
		{
	...
	{
		mpctx->global_sub_size += mpctx->set_of_sub_size;	//#SUBS
		mpctx->global_sub_size += (mpctx->global_sub_indices[SUB_SOURCE_SUBS]-mpctx->global_sub_indices[SUB_SOURCE_DEMUX]);	//#DEMUX
	}
	else
		mpctx->global_sub_size += cnt;		//#DEMUX
	
	if(mpctx->global_sub_indices[SUB_SOURCE_VOBSUB] >= 0)
		mpctx->global_sub_size += mpctx->global_sub_indices[SUB_SOURCE_DEMUX];	//#VOBSUB
}

static int subid_to_globalid(MPContext * mpctx, int sid){
	int nRet = -1;
	sh_sub_t *sh;
	int ii, jj = 0;
	...
	if (sid >= 0)
	{
		for (ii = 0; ii < MAX_S_STREAMS; ii++) {
			sh = mpctx->demuxer->s_streams[ii];
			if (sh)
			{
				if (sh->sid == sid) {
					nRet = mpctx->global_sub_indices[SUB_SOURCE_DEMUX] + jj;
					break;
				}
				jj++;
			}
...
//    const int global_sub_size = mpctx->global_sub_size;
    ...
    {
//printf("subcc_force_off[%d],subcc_enabled[%d],action[%d],dvdnav_globle_num_subtitle[%d],title_state[%d],has_subcc[%d]\n",subcc_force_off,subcc_enabled,action,dvdnav_globle_num_subtitle,dvdnavstate.dvdnav_title_state,has_subcc);
//printf("-----global_sub_size <= 0,mpctx->global_sub_pos[%d],global_sub_size[%d],subcc_enabled[%d],subtitle_names[%s]\n",mpctx->global_sub_pos,global_sub_size,subcc_enabled,subtitle_names);
#if 0	//Fuchun 20110823 disable, it's not in here to set subcc
    	if(is_dvdnav &&
    	   (subcc_enabled || subcc_force_off || skydroid) &&
    	   dvdnavstate.dvdnav_title_state == DVDNAV_TITLE_STATE_MOVIE &&
    	   dvdnav_globle_num_subtitle==0)
    	{
		if(skydroid)
		{
			if(!subcc_enabled && has_subcc && action ==4)
			{
				char *tmp1 = "SUBCC 100 cc";
				memcpy(subtitle_names, tmp1, strlen(tmp1));
				subtitle_names[strlen(tmp1)] = '\0';
				sub_change_success = 1;
			
				subcc_enabled = 1;
				subcc_reg = 1;
				return M_PROPERTY_UNAVAILABLE;
			}
			else if(subcc_enabled && has_subcc && action ==4)
			{
				subcc_enabled = 0;
				subcc_reg = 0;
				subcc_force_off = 1;
				sprintf(subtitle_names, "%s\0",MSGTR_Disabled);
				sub_change_success = 3;
				return M_PROPERTY_UNAVAILABLE;
			}
			else
			{
    				char *tmp = "SKY_NOSUB";
				memcpy(subtitle_names, tmp, strlen(tmp));
				subtitle_names[strlen(tmp)] = '\0';
				sub_change_success = 3;
				return M_PROPERTY_UNAVAILABLE;
			}
		}
	...
	*(char **) arg = malloc(64);
	...
	if (sub_name) {
		...
		//memcpy(subtitle_names, tmp, strlen(tmp));
		//subtitle_names[strlen(tmp)] = '\0';
		//Polun 2011-9-19 ++s for mantis 6149
		//len = sprintf(subtitle_names, "%d %s\0",mpctx->set_of_sub_pos+1,tmp);
		if((mpctx->global_sub_size > mpctx->set_of_sub_size) && mpctx->global_sub_size && mpctx->set_of_sub_size)  
		  len = sprintf(subtitle_names, "%d %s\0",(mpctx->set_of_sub_pos+1+ mpctx->global_sub_size -mpctx->set_of_sub_size ),tmp);
              else
		  len = sprintf(subtitle_names, "%d %s\0",mpctx->set_of_sub_pos+1,tmp);
	       //Polun 2011-9-19 ++e
		if(ass_track && ass_track->name)
			sub_change_success = 2;
		else
			sub_change_success = 1;
...
//printf("---------------dvdsub_id[%d]\n",dvdsub_id);
//int z;
//for(z=0;z<dvdnav_globle_num_subtitle+3;z++)
//printf("z[%d],sid[%d]\n",z,dvdnavstate.sid[z]);
//printf("dvdnavstate.sid[%d], dvdsub_id[%d], d_sub->id[%d]\n",dvdnavstate.sid[dvdsub_id],dvdsub_id, d_sub->id);
...
//printf("dvdnav_sub_reg[%d]\n",dvdnav_sub_reg);

			if(needs_change_dsub_id)
				needs_change_dsub_id = 0;

			if (mp_dvdnav_lang_from_sid(mpctx->stream, dvdnavstate.sid[dvdnav_sub_reg], lang)) {
				if(dvdnav_sub_reg==0 && d_sub->id == -2)
				{
					len = sprintf(subtitle_names, "%s\0",MSGTR_Disabled);
					//snprintf(*(char **) arg, 63, "%s", MSGTR_Disabled);
					sub_change_success = 3;
				}
				else
				{
					len = sprintf(subtitle_names, "%d %s\0",dvdnav_sub_reg+1,lang);
					snprintf(*(char **) arg, 63, "(%d) %s", dvdnav_sub_reg, lang);
					sub_change_success = 2;
				}
				
				//printf("dvdnav_sub_result[%d]\n",dvdnav_sub_result);
			    	return M_PROPERTY_OK;
			}
		}
		else
		{
		if (mp_dvdnav_lang_from_sid(mpctx->stream, dvdnavstate.sid[dvdsub_id], lang)) {
			//memcpy(subtitle_names, lang, 3);
			//subtitle_names[2] = '\0';
			len = sprintf(subtitle_names, "%d %s\0",dvdsub_id+1,lang);
			sub_change_success = 2;
	...
	//Mantis:5939
	if(bluray_device && skydroid) {
		unsigned char lang[4];
		if (mp_bd_lang_from_sid(mpctx->stream, dvdsub_id, lang)) {

			len = sprintf(subtitle_names, "%d %s\0",dvdsub_id+1,lang);
			sub_change_success = 2;
			snprintf(*(char **) arg, 63, "(%d) %s",
				dvdsub_id, lang ? lang : MSGTR_Unknown_Subtitle);
	...
	if ((mpctx->demuxer->type == DEMUXER_TYPE_MATROSKA
            ...
            if (!lang) lang = MSGTR_Unknown_Subtitle;
			//memcpy(subtitle_names, lang, strlen(lang));
			//subtitle_names[strlen(lang)] = '\0';
			len = sprintf(subtitle_names, "%d %s\0",dvdsub_id+1,lang);
			if(mpctx->demuxer->type == DEMUXER_TYPE_MPEG_TS)
				sub_change_success = 2;
			else
	...
	if (vo_vobsub && vobsub_id >= 0) {
	    const char *language = MSGTR_Unknown_Subtitle;
	    language = vobsub_get_id(vo_vobsub, (unsigned int) vobsub_id);
		//memcpy(subtitle_names, language, strlen(language));
		//subtitle_names[strlen(language)] = '\0';
		len = sprintf(subtitle_names, "%d %s\0",vobsub_id+1,language);
		sub_change_success = 2;
	    snprintf(*(char **) arg, 63, "(%d) %s",
		     vobsub_id, language ? language : MSGTR_Unknown_Subtitle);
	    return M_PROPERTY_OK;
	}
#ifdef CONFIG_DVDREAD
...
#endif
	if (dvdsub_id >= 0) {
		//memcpy(subtitle_names, MSGTR_Unknown_Subtitle, strlen(MSGTR_Unknown_Subtitle));
		//subtitle_names[strlen(MSGTR_Unknown_Subtitle)] = '\0';
		len = sprintf(subtitle_names, "%d %s\0",dvdsub_id+1,MSGTR_Unknown_Subtitle);
		sub_change_success = 2;
	    snprintf(*(char **) arg, 63, "(%d) %s", dvdsub_id, MSGTR_Unknown_Subtitle);
	    return M_PROPERTY_OK;
	}
	//memcpy(subtitle_names, MSGTR_Disabled, strlen(MSGTR_Disabled));
	//subtitle_names[strlen(MSGTR_Disabled)] = '\0';
	if(subcc_enabled)
	{
		len = sprintf(subtitle_names, "%s\0","SUBCC 100 cc");
		snprintf(*(char **) arg, 63, "SUBCC 100 cc");
		sub_change_success = 1;
	}
	else
	{
	...
	if((mpctx->demuxer->type == DEMUXER_TYPE_MPEG_TS) && hwtsdemux) 
	{
		/* Stop subtitle filter */
		if (priv->fifo[2].pfd)
			dvb_demux_stop(priv->fifo[2].pfd);
		switch_debug_printf("### In [%s][%d] disable subtitle###\n", __func__, __LINE__);
	}
	...
	if(skydroid)
	{
		if(has_subcc && is_dvdnav)
		{
			if(mpctx->global_sub_pos == -1 && subcc_enabled == 0) //subtitle disable
			{
				subcc_enabled = 1;
			}
			else
    ...
    mp_msg(MSGT_CPLAYER, MSGL_DBG3,
	   "subtitles: %d subs, (v@%d s@%d d@%d), @%d, source @%d dvd_id %d\n",
	   global_sub_size,
	   ...
	   mpctx->global_sub_pos, source, dvdsub_id);
    ...
    vobsub_id = -1;
    dvdsub_id = -1;
    subcc_enabled = 0;	//Fuchun 20110823
    subcc_reg = 0;
    subcc_force_off = 1;	//Fuchun 20110823
    if (d_sub) {
	if (d_sub->id > -2)
	    reset_spu = 1;
	d_sub->id = -2;
	...
	int timeout_cnt = 0;
	while(draw_osd_cmdqueue)
	{
//		printf("%s:%d draw_osd_cmdqueue not empty, contious wait...timeout_cnt[%d]\n", __func__, __LINE__, timeout_cnt);
		timeout_cnt++;
		if(timeout_cnt > 1000)
			break;
		usleep(1000);
	...
	if((mpctx->demuxer->type == DEMUXER_TYPE_MPEG_TS) && hwtsdemux) 
	{
		...
		if (priv->fifo[2].pfd)
			dvb_demux_stop(priv->fifo[2].pfd);
		d_sub->sh = NULL;
		switch_debug_printf("##### In [%s][%d] switch to srt####\n", __func__, __LINE__);
	}
#endif // end of SUPPORT_HWTS_CHANGE_AUDIO_OR_SUBTITLE
#ifdef CONFIG_ASS
	...
	if (d_sub && (sub_sid > 0))
	{
		int tmp_sub = subid_to_globalid(mpctx, sub_sid);
		if (tmp_sub != -1)
			*(int *) arg = mpctx->global_sub_pos = tmp_sub;
	}

        int z=0;
        do{
#if 1	//Fuchun 20110823 for subcc
		if(mpctx->stream->type == STREAMTYPE_DVDNAV && is_dvdnav && has_subcc
			&& (mpctx->global_sub_pos - mpctx->global_sub_indices[SUB_SOURCE_DEMUX]) == dvdnav_globle_num_subtitle)
		{
			subcc_enabled = 1;
			subcc_reg = 1;
			subcc_force_off = 0;
			//printf("=== In[%s][%d]  enable subcc ... dvdsub_id:%d ===\n", __func__, __LINE__, dvdsub_id);
			break;
		}
#endif
        	dvdsub_id =
...
//printf("d_sub->id[%d],dvdnavstate.sid[%d],dvdsub_id[%d],mpctx->global_sub_pos[%d]\n",d_sub->id,dvdnavstate.sid[d_sub->id],dvdsub_id,mpctx->global_sub_pos);
                  if(is_dvdnav)
                  {
			//if(skydroid)
			if(1) //Mantis:6066
			{
				...
				sid=d_sub->id=tmp_sid;
				d_sub->sh = mpctx->demuxer->s_streams[tmp_sid];
			}
			else
			{
				int sid = dvdnavstate.sid[d_sub->id];
				d_sub->sh = mpctx->demuxer->s_streams[sid];
			}
//printf("--s_streams---sid[%d],d_sub->id[%d],tmp_sid[%d]\n",sid,d_sub->id,tmp_sid);				
                  }
                  else
				d_sub->sh = mpctx->demuxer->s_streams[d_sub->id];
#ifdef SUPPORT_HWTS_CHANGE_AUDIO_OR_SUBTITLE
        	    ...
        	    ds_free_packs(d_sub);
#ifdef SUPPORT_HWTS_CHANGE_AUDIO_OR_SUBTITLE
				if((!no_osd) && (dvdsub_id != -2) && (mpctx->demuxer->type == DEMUXER_TYPE_MPEG_TS) && hwtsdemux && priv->fifo[2].pfd)
				{
					sh_sub_t *sh = d_sub->sh;
					if (sh && sh->sid > 0)
					{
						dvb_set_tsout_filt(priv->fifo[2].pfd, sh->sid, DMX_PES_SUBTITLE);
						switch_debug_printf("##### In [%s][%d] change sub-id to [%d]#####\n", __func__, __LINE__, sh->sid);
					}
				}
#endif // end of SUPPORT_HWTS_CHANGE_AUDIO_OR_SUBTITLE
		//if(!skydroid)
		if(!is_dvdnav) //Mantis:6066
		{
                   for (i = 0; i < MAX_S_STREAMS; i++) {
...
//	                char lang[3];
	                ...
	                {
	                    mpctx->global_sub_pos += 2;
	                    mpctx->global_sub_pos = (mpctx->global_sub_pos % (mpctx->global_sub_size + 1)) - 1;
	                }
	                else
	                    break;
                }
////////////////////
	        ...
	        z++;
        }while(mpctx->stream->type==STREAMTYPE_DVD&&z<5);
        
        if((action == 4 || action == 2) && is_dvdnav) //Mantis:5940
        {
		...
		if(1) //Mantis:6066
		{
			dvdnav_sub_result = dvdnav_demuxer_set_sub_reg(mpctx->demuxer, needs_change_dsub_id? tmp_sid : dvdsub_id);
		}
		else
...
#ifdef AVI_NOT_SORT
	if(mpctx->demuxer->type == DEMUXER_TYPE_AVI_NI)
		demuxer_switch_sub(mpctx->demuxer);
#endif
			      ...
			      MPContext * mpctx)
{
    ...
    switch (action) {
		...
		//Fuchun 2010.11.11 limit the font size
		if(text_font_scale_factor >= 5.0)
			text_font_scale_factor = 5.0;
		else if(text_font_scale_factor <= 2.0)
			text_font_scale_factor = 2.0;
...
#ifdef SUPPORT_SKYDVB_DYNAMIC_PID
	{ "dvb_switch", MP_CMD_DVB_SWITCH, 1, 0, -1, MSGTR_OSD_DVB_AUDIO },
#endif // end of SUPPORT_SKYDVB_DYNAMIC_PID
    // video
...
static int set_property_command(MPContext * mpctx, mp_cmd_t * cmd)
{
	...
	//Fuchun 2010.06.01
	if(set_prop_cmd[i].cmd == MP_CMD_SUB_POS) 
	{
		sub_change_success = 1;
		mpctx->video_out->draw_osd();
	}
	else if(set_prop_cmd[i].cmd == MP_CMD_SUB_SCALE)
	{
		int source = -1;
		if (mpctx->global_sub_pos >= 0)
			source = sub_source(mpctx);
		if(source == SUB_SOURCE_SUBS || source == SUB_SOURCE_DEMUX || subcc_enabled == 1)
			vo_osd_changed(OSDTYPE_SUBTITLE);
		sub_change_success = 1;
		mpctx->video_out->draw_osd();
			
	}
	else if(set_prop_cmd[i].cmd == MP_CMD_SUB_SELECT)
	{
		mplayer_dvdnav_debug("## DVDNAV In [%s][%d],MP_CMD_SUB_SELECT cmd->args[0].v.i[%d]##\n", __func__, __LINE__,cmd->args[0].v.i);
		...
		{
			//for subcc
			if(cmd->args[0].v.i == 100 && dvdnav_globle_num_subtitle==0)
			{
				subcc_enabled = 1;
				subcc_force_off = 0;
				char *tmp = "SUBCC 100 cc";
				memcpy(subtitle_names, tmp, strlen(tmp));
				subtitle_names[strlen(tmp)] = '\0';
				sub_change_success = 1;
			}
			else if(cmd->args[0].v.i == -1 && dvdnav_globle_num_subtitle==0)
			{
				subcc_enabled = 0;
				subcc_reg = 0;
				subcc_force_off = 1;
	...
	else if(set_prop_cmd[i].cmd == MP_CMD_SUB_DELAY)
	{
		sub_change_success = 1;
	}
...
static int demuxer_playlist_track(MPContext * mpctx)
{
	int ii;
	int s_mall_size = 5;
	int a_mall_size = 5;
	demux_program_list_t p_list;
	char callback_str[512];
	char tmp_str[32];
	extern int is_dvdnav;
	
	demuxer_t *d = mpctx->demuxer;

	//Barry 2011-06-27
	if (video_id == -2 && !mpctx->sh_video)
		return -1;

	memset((void *)&p_list, 0, sizeof(demux_program_list_t));
	if (demux_control (d, DEMUXER_CTRL_PROGRAM_LIST, &p_list) == DEMUXER_CTRL_NOTIMPL)
	{
		p_list.a_list = (program_list_item_t *)malloc(sizeof(program_list_item_t) * a_mall_size);
		p_list.s_list = (program_list_item_t *)malloc(sizeof(program_list_item_t) * s_mall_size);

		for (ii = 0; ii < MAX_A_STREAMS; ii++) {
			sh_audio_t *sh = d->a_streams[ii];
			if (sh)
			{
				if (p_list.aid_num == a_mall_size)
				{
					a_mall_size += 5;
					p_list.a_list = (program_list_item_t *)realloc((void *)p_list.a_list,
							sizeof(program_list_item_t) * a_mall_size);
				}
				p_list.a_list[p_list.aid_num].id = sh->aid;
				if (sh->lang)
				{
					memcpy(p_list.a_list[p_list.aid_num].lang, sh->lang, 4);
					p_list.a_list[p_list.aid_num].lang[3] = '\0';
				} else {
					p_list.a_list[p_list.aid_num].lang[0] = '\0';
				}
				p_list.aid_num++;
			}
		}
		...
		for (ii = 0; ii < MAX_S_STREAMS; ii++) {
			sh_sub_t *sh = d->s_streams[ii];
			if (sh)
			{
				if (p_list.sid_num == s_mall_size)
				{
					s_mall_size += 5;
					p_list.s_list = (program_list_item_t *)realloc((void *)p_list.s_list,
							sizeof(program_list_item_t) * s_mall_size);
				}
				p_list.s_list[p_list.sid_num].id = subid_to_globalid(mpctx, sh->sid);
				if (sh->lang)
				{
					memcpy(p_list.s_list[p_list.sid_num].lang, sh->lang, 4);
					p_list.s_list[p_list.sid_num].lang[3] = '\0';
				} else {
					p_list.s_list[p_list.sid_num].lang[0] = '\0';
				}
				p_list.sid_num++;
			}
		}
		...
		{
			s_mall_size = ((p_list.sid_num / 5) + 1 ) * 5;
			p_list.s_list = (program_list_item_t *)realloc((void *)p_list.s_list,
					sizeof(program_list_item_t) * s_mall_size);
		}
		for (ii = 0; ii < p_list.sid_num; ii++)
		{
			p_list.s_list[ii].id = subid_to_globalid(mpctx, p_list.s_list[ii].id);
		}
	}
	...
	if (mpctx->set_of_sub_size > 0)
	{
		for (ii = 0; ii < mpctx->set_of_sub_size; ++ii) {
			if (p_list.sid_num == s_mall_size)
			{
				s_mall_size += 5;
				p_list.s_list = (program_list_item_t *)realloc((void *)p_list.s_list,
			...
			p_list.s_list[p_list.sid_num].id = ii + mpctx->global_sub_indices[SUB_SOURCE_SUBS];;
			p_list.s_list[p_list.sid_num].lang[0] = '\0';
			p_list.sid_num++;
		}
	}
	...
	if(is_dvdnav)
	{
		int i, j , skip_sid_cnt=0;
		int spu_num,audio_num;

		if(dvdnavstate.dvdnav_title_state == DVDNAV_TITLE_STATE_MENU || (duration_sec_cur < 300 && dvdnavstate.dvdnav_title_state == DVDNAV_TITLE_STATE_MOVIE))
		{
			sprintf(callback_str, "list: 0||0\0");
		}
		else
		{
			//mp_dvdnav_get_all_audio_info(mpctx->stream);
		
			//audio
			audio_num = mp_dvdnav_number_of_audios(mpctx->stream);
			sprintf(tmp_str, "list: %d", audio_num);
			sprintf(callback_str, tmp_str);
		
			for(i=0; i<8; i++)
			{
				unsigned char lang[3];
				if (mp_dvdnav_lang_from_aid(mpctx->stream, i, lang))
				{
					sprintf(tmp_str, ":%d,%s", dvdnavstate.aid[i], lang);
					strcat(callback_str, tmp_str);
				}
			}
		    
			//sub
			if((subcc_enabled || subcc_force_off) && dvdnav_globle_num_subtitle==0)
			{
				//for subcc
				sprintf(tmp_str, "||1:0,cc");
				strcat(callback_str, tmp_str);
			}
			else
			{
				spu_num = mp_dvdnav_number_of_subs(mpctx->stream);
				sprintf(tmp_str, "||%d", spu_num);
				strcat(callback_str, tmp_str);
			
				for(i=0, j=0; i<32; i++)
				{
					unsigned char lang[3];
					//printf("dvdnavstate.sid[i][%d],i[%d]\n",dvdnavstate.sid[i],i);
					if(dvdnavstate.sid[i] == (uint16_t)-1) //2_angle: sid has one null needs to skip
					{
						j++;
						skip_sid_cnt++;
						continue;
					...
					if (mp_dvdnav_lang_from_sid(mpctx->stream, dvdnavstate.sid[i], lang))
					{
						sprintf(tmp_str, ":%d,%s", j, lang);
						strcat(callback_str, tmp_str);
						j++;
						if(j == dvdnav_globle_num_subtitle+skip_sid_cnt)
							break;
					}
				}
			}
	...
	{
		sprintf(tmp_str, "list: %d", p_list.aid_num);
		sprintf(callback_str, tmp_str);
		if (p_list.aid_num > 0)
		{
			for (ii = 0; ii < p_list.aid_num; ii++)
			{
				//printf("aid: %d, lang: %s\n", p_list.a_list[ii].id, p_list.a_list[ii].lang);
				sprintf(tmp_str, ":%d,%s", p_list.a_list[ii].id, 
						(p_list.a_list[ii].lang[0] == 0)? "N": p_list.a_list[ii].lang);
				strcat(callback_str, tmp_str);
			}
		}
	
		sprintf(tmp_str, "||%d", p_list.sid_num);
		strcat(callback_str, tmp_str);
		if (p_list.sid_num > 0)
		{
			for (ii = 0; ii < p_list.sid_num; ii++)
			{
				//printf("sid: %d, lang: %s\n", p_list.s_list[ii].id, p_list.s_list[ii].lang);
				sprintf(tmp_str, ":%d,%s", p_list.s_list[ii].id, 
						(p_list.s_list[ii].lang[0] == 0)? "N": p_list.s_list[ii].lang);
				strcat(callback_str, tmp_str);
			}
		}
	}
...
int run_command(MPContext * mpctx, mp_cmd_t * cmd)
{
    ...
    memset(callback_str,0,sizeof(callback_str));
    
    if (!set_property_command(mpctx, cmd))
	switch (cmd->id) {
	case MP_CMD_SEEK:{
		clear_osd();	//Fuchun 2010.10.21
		float v;
		int abs;
		if (sh_video)
		...
		if(v > 1000 || v < -1000) // for callback seektimedone
			seek_by_time_done = 1;

		if(is_dvdnav)
		{
extern int my_current_pts;
			//Mantis:4499 ignore seek while seek time > duration time
			//if(dvdnavstate.dvdnav_title_state == DVDNAV_TITLE_STATE_MENU || v > duration_sec_cur)
			if(dvdnavstate.dvdnav_title_state == DVDNAV_TITLE_STATE_MENU || (duration_sec_cur - my_current_pts) <5)
			{
				break;
			}
			else if(dvdnavstate.dvdnav_title_state == DVDNAV_TITLE_STATE_MOVIE && duration_sec_cur < 1200 && v == 1200)
			{
//printf("--------duration_sec_cur[%d]\n",duration_sec_cur);
				v = duration_sec_cur - 1;
				abs = 2;
				seek_by_time_done = 1;
			}
			else
		...
		brk_cmd = 1;
		if(speed_mult != 0 || audio_speed != 0)
		{
			//Robert 2011-2-18 back to x1, call workaround(0)
#ifdef HW_TS_DEMUX
			if((mpctx->demuxer->type == DEMUXER_TYPE_MPEG_TS) && hwtsdemux) 
                        {
	...
	case MP_CMD_SPEED_MULT:{
		float v = cmd->args[0].v.f;
#if 0 	//Fuchun 2009.12.02		
		playback_speed *= v;
#else

		if(sh_video)	//Fuchun 2010.03.01
		{
			if(Can_FF_FB == -1 || FR_to_end != 0) break;
			if(is_dvdnav)
			{
				//Mantis:4499 ignore FF/FR while duration < 30
				if(dvdnavstate.dvdnav_title_state == DVDNAV_TITLE_STATE_MENU || duration_sec_cur < 30)
				{
					ipc_callback_fast();
					break;
				}
			...
			if(v == 2.0)
			{
				if(speed_mult >= 0)
				{
					if((speed_mult == FAST_SPEED) || (speed_mult == 1 && Can_FF_FB == 0))
					{
						cmd->pausing = 2;
						brk_cmd = 1;
						//Robert 2011-2-18 back to x1, call workaround(0)
...
#endif
						break;	//Barry 2010-11-04
					}

					if(speed_mult < FAST_SPEED)
					{
						if (speed_mult == 0)
						{
							if(mpctx->audio_out)
								mpctx->audio_out->pause();
							//Robert 2011-2-18 jump to x2, call workaround(1)
#ifdef HW_TS_DEMUX
                                                        if((mpctx->demuxer->type == DEMUXER_TYPE_MPEG_TS) && hwtsdemux) 
                                                        {
						...
						speed_mult++;
#ifdef SUPPORT_DIVX_DRM
						if((DRM_Open > 0) && (mpctx->demuxer->type == DEMUXER_TYPE_AVI || mpctx->demuxer->type == DEMUXER_TYPE_AVI_NI) && speed_mult == 2)
							demux_avi_save_idxpos(mpctx->demuxer);
#endif /* end of SUPPORT_DIVX_DRM */
					}
					if(rel_audio_id == -5)
						rel_audio_id = audio_id;
					audio_id = -2;
#ifdef HW_TS_DEMUX
					if((mpctx->demuxer->type == DEMUXER_TYPE_MPEG_TS) && hwtsdemux) 
					{
						/* pause filter */
						restart_audio_buffer(mpctx->demuxer->audio, mpctx->demuxer->priv, NEED_PAUSE_FILTER);
						restart_sub_buffer(mpctx->demuxer->sub, mpctx->demuxer->priv, NEED_PAUSE_FILTER);
						/* slow down axi speed */
						change_axi_speed(mpctx->demuxer->video, mpctx->demuxer->priv, SLOW_DOWN_SPEED);
					}
#endif
#if 0 //Robert 20101130 Reserved function
mp_set_aopcm_zero();
#endif
					if(mpctx->global_sub_size > 0 && rel_global_sub_pos == -5)
					{
						rel_global_sub_pos = -1;

						update_subtitles(sh_video, sh_video->pts, mpctx->d_sub, 1);
						update_teletext(mpctx->sh_video, mpctx->demuxer, 1);
//						mpctx->video_out->draw_osd();
						clear_osd();
					}
				...
				else		//speed_mult < 0
				{
					...
					//Barry 2010-12-24
					rel_seek_secs += 1.0;
//					if(h264_frame_mbs_only)	//Fuchun 20110812 disable
						FRtoFF = 1;
				}
				...
				if(speed_mult >= 2 && top_field_frame == 1&& (mpctx->demuxer->type == DEMUXER_TYPE_MPEG_ES 
					|| mpctx->demuxer->type == DEMUXER_TYPE_MPEG_PS || mpctx->demuxer->type == DEMUXER_TYPE_MPEG_TS))
					mpeg_fast = 1;

				if(speed_mult == 2 && is_mjpeg == 0)
				{
					svsd_ipc_drop_video_cnts();
					sky_set_reset_display_queue(0);
					sky_set_display_drop_cnt(1);
				}
			}
			else if(v == 0.5 && Can_FF_FB == 1)
			{
				if(speed_mult <= 0)
				{
					if(speed_mult == -1*FAST_SPEED)
					{
						cmd->pausing = 2;
						brk_cmd = 1;
						//Robert 2011-2-18 back to x1, call workaround(0)
					...
					if(speed_mult > -1*FAST_SPEED)
					{
						...
						speed_mult--;
#ifdef SUPPORT_DIVX_DRM
						if((DRM_Open > 0) && (mpctx->demuxer->type == DEMUXER_TYPE_AVI || mpctx->demuxer->type == DEMUXER_TYPE_AVI_NI) && speed_mult == -1)
							demux_avi_save_idxpos(mpctx->demuxer);
#endif /* end of SUPPORT_DIVX_DRM */
...
//						mpctx->video_out->draw_osd();
				...
				else		//speed_mult > 0
				{
					speed_mult = -1;
#ifdef SUPPORT_DIVX_DRM
					if((DRM_Open > 0) && (mpctx->demuxer->type == DEMUXER_TYPE_AVI || mpctx->demuxer->type == DEMUXER_TYPE_AVI_NI))
					                        demux_avi_save_idxpos(mpctx->demuxer);
#endif /* end of SUPPORT_DIVX_DRM */
				}
				...
				if(speed_mult <= -1 && top_field_frame == 1 && (mpctx->demuxer->type == DEMUXER_TYPE_MPEG_ES 
					|| mpctx->demuxer->type == DEMUXER_TYPE_MPEG_PS || mpctx->demuxer->type == DEMUXER_TYPE_MPEG_TS))
					mpeg_fast = 1;

				if(speed_mult == -1 && is_mjpeg == 0)
				{
					svsd_ipc_drop_video_cnts();
					sky_set_reset_display_queue(0);
					sky_set_display_drop_cnt(1);
			...
			if(cmd->pausing != 2) ipc_callback_fast();

			if(speed_mult != 0 && is_mjpeg == 1)//Fuchun 2009.12.01
			{
				mjpeg_skip1 = 1;
				mjpeg_skip2 = 2 * abs(speed_mult);
			}
		}
#endif
		build_afilter_chain(sh_audio, &ao_data);
//		set_osd_msg(OSD_MSG_SPEED, 1, osd_duration, MSGTR_OSDSpeed,
//			    playback_speed);
		float speed_int = 1.0, i;
		if(speed_mult > 0)
			for(i = 0; i < speed_mult; i++) speed_int *= 2.0;
		else if(speed_mult < 0)
		{
			for(i = 0; i < ((-1)*speed_mult); i++) speed_int *= 2.0;
			speed_int *= -1;
		}
	
		set_osd_msg(OSD_MSG_SPEED, 1, osd_duration, MSGTR_OSDSpeed,
			    speed_int);
		if(((v == 2.0 && speed_mult > 0) || (v == 0.5 && speed_mult <= 0)) && is_dvdnav)
		{
			FFFR_to_normal = 0;
			dvdnav_rel_seek = 0;
		}
	...
	case MP_CMD_SPEED_SET:{
		...
		ipc_callback_audio_fast();	//Fuchun 2010.10.25
		build_afilter_chain(sh_audio, &ao_data);
		set_osd_msg(OSD_MSG_SPEED, 1, osd_duration, MSGTR_OSDSpeed,
			    playback_speed);
	    } break;
...
//SkyViia_Vincent03232010+
	...
	case MP_CMD_STOP:
		#if 0 /* we move the below code to mplayer.c, keyword: 2010-11-24 carlos */
	    // Go back to the starting point.
	    while (play_tree_iter_up_step
		   (mpctx->playtree_iter, 0, 1) != PLAY_TREE_ITER_END)
		/* NOP */ ;
	...
	case MP_CMD_TV_STEP_CHANNEL:{
	    ...
	    if (mpctx->stream->type == STREAMTYPE_DVB) {
			...
			extern int exit_skydvb_thread;
			exit_skydvb_thread = 1;
			/* We need turn off display */
			skyfb_set_display_status(0);
			set_disp_off = 1;
#endif // end of DVBT_USING_NORMAL_METHOD
		    if (dvb_step_channel(mpctx->stream, dir))
				mpctx->eof = mpctx->dvbin_reopen = 1;
			else /* Can't lock tunner */
			{
				printf("### DVB-T Lock new freq failed ###\n");
				mpctx->eof = 1; 
			}
	    }
#endif /* CONFIG_DVBIN */
	...
	case MP_CMD_DVB_SET_CHANNEL:
	    if (mpctx->stream->type == STREAMTYPE_DVB) {
		    ...
		    if (dvb_set_channel(mpctx->stream, cmd->args[1].v.i, cmd->args[0].v.i))
				mpctx->eof = mpctx->dvbin_reopen = 1;
			else /* Can't lock tunner */
			{
				printf("### DVB-T Lock new freq failed ###\n");
	...
	case MP_CMD_GET_CHAPTER_INFO:
    	if (mpctx->demuxer)
        	chapter = demuxer_get_current_chapter(mpctx->demuxer);

    	//if (mpctx->demuxer->num_chapters == 0) mantis:3696
        	stream_control(mpctx->demuxer->stream, STREAM_CTRL_GET_NUM_CHAPTERS, &mpctx->demuxer->num_chapters);
	...
	case MP_CMD_GET_TITLE_INFO:
		if (duration_sec_cur > 300)
		{
			stream_control(mpctx->stream, STREAM_CTRL_GET_CURRENT_TITLE,&title);
			sprintf(callback_str,"DVDTITLE=%02d\0",title);
			ipc_callback(callback_str);
		}
		break;

	case MP_CMD_GET_ANGLE_INFO:
    		if (mpctx->demuxer)
        		angle = demuxer_get_current_angle(mpctx->demuxer);

        	angles = demuxer_angles_count(mpctx->demuxer);

		sprintf(callback_str,"Angle:%d/%d\0",angle,angles);
		ipc_callback(callback_str);

		break;

	case MP_CMD_GET_CUR_POS_CALLBACK:
		printf("In [%s] [%d] will enable ipc_callback_curpos get_curpos_on [%d] cmd->args[0].v.i [%d]@@@@\n", __func__, __LINE__, get_curpos_on, cmd->args[0].v.i);
		if (cmd->args[0].v.i > 0)
		{
	...
	case MP_CMD_GET_BUFFERING_CALLBACK:
		printf("In [%s] [%d] will enable ipc_callback_buffering get_buffering_on [%d] cmd->args[0].v.i [%d]@@@@\n", __func__, __LINE__, get_buffering_on, cmd->args[0].v.i);
		if (cmd->args[0].v.i > 0)
		{
			printf("== callback get buffering: on ==\n");
		...
		else
		{
			if (get_buffering_on == IPC_THREAD_RUNNING){
				printf("== callback get buffering: off ==\n");
				extern int buffering_pthread_flag;
				buffering_pthread_flag=1;
				ipc_callback_thread_stop(&ipc_thread_buffering_id, &get_buffering_on);
			}else{
				printf("== callback get buffering: isn't on ==\n");
			}
		}

		break;

	case MP_CMD_GET_AUDIO_INFO:
		mp_property_print("switch_audio", mpctx);
		break;

	case MP_CMD_GET_SUB_INFO:
		mp_property_print("sub", mpctx);
		break;

	case MP_CMD_GET_VIDEO_INFO:
		sprintf(callback_str, "VIDEO: %s\0", video_names);
		ipc_callback(callback_str);
		break;
	...
	case MP_CMD_SUB_ON:
		...
		if (cmd->args[0].v.i > 0)
		{
			sub_on = 1;
			printf("== subtitle: on ==\n");
			sprintf(callback_str, "SUBTITLE: ON\0");
			ipc_callback(callback_str);
		}
		else
		{
			...
			clear_osd();
			printf("== subtitle: off ==\n");
			sprintf(callback_str, "SUBTITLE: OFF\0");
			ipc_callback(callback_str);
		}

		break;

	case MP_CMD_SET_SUBCP:		//Fuchun 2010.06.03
		if(cmd->args[0].v.s)
		{
			sub_cp = cmd->args[0].v.s;
	...
	case MP_CMD_PLAYLIST:
		demuxer_playlist_track(mpctx);
		break;

	case MP_CMD_AUDIO_SPEED:
	...
	case MP_CMD_SUB_COLOR_NUM:
		if(vo_need_osd)
		{
			int c = cmd->args[0].v.i;
			color_num = c;
//			update_subtitles(sh_video, sh_video->pts, mpctx->d_sub, 1);
//			update_teletext(mpctx->sh_video, mpctx->demuxer, 1);
//			clear_osd();
		...
		break;
#ifdef SUPPORT_SKYDVB_DYNAMIC_PID
	case MP_CMD_DVB_SWITCH:
		if (cmd->args[0].v.s)
		{
			//mplayer_debug(" $$$ In [%s][%d] command is [%s] MP_CMD_DVB_SWITCH$$$\n", __func__, __LINE__, cmd->args[0].v.s);
			if (mpctx->demuxer->type == DEMUXER_TYPE_SKYMPEG_TS)
				do_dvb_switch(cmd->args[0].v.s, mpctx);
	...
	if((cmd->id == MP_CMD_SWITCH_AUDIO || cmd->id == MP_CMD_GET_AUDIO_INFO)&& audio_change_success == 1)
	{
		sprintf(callback_str, "AUDIO: %s\0", audio_names);
		ipc_callback(callback_str);
		audio_change_success = 0;
	...
	else if((cmd->id == MP_CMD_SUB_SELECT || cmd->id == MP_CMD_GET_SUB_INFO) && sub_change_success != 0)
	{
...
static void do_dvb_switch(char *cmd, MPContext *mpctx)
{
	...
	int tmp = 1;
    demux_program_t prog;
	number = sscanf(cmd, new_dvb_cmd, &type, &pid, &format);
	//printf("==== In [%s][%d] number is [%d] type[%d] pid[%d] format[%d] ===\n", __func__, __LINE__, number, type, pid, format);
	if (number == 3) /* 3 mean type, pid, format */
	{
		if (type == TYPE_AUDIO) /* reference libmpdemux/demux_ts.h */
		{
			prog.aid = pid;
			prog.aformat = format;
			ret = demux_control(mpctx->demuxer, DEMUXER_CTRL_ADD_AUDIO_STREAM, &prog);
			if (ret == DEMUXER_CTRL_OK)
			{
				mplayer_debug("@@@@ In [%s][%d] call ctrl success @@@@\n", __func__, __LINE__);
				mp_property_audio(NULL, M_PROPERTY_SET, &pid, mpctx);
			}
			else
			{
				mplayer_debug("@@@@ In [%s][%d] call ctrl failed , ret [%d]@@@@\n", __func__, __LINE__, ret);
			}
		}
		else if (type == TYPE_SUB) /* 0x04 */
		{
			prog.sid = pid;
			prog.sformat = format;
			ret = demux_control(mpctx->demuxer, DEMUXER_CTRL_ADD_SUBTITLE_STREAM, &prog);
			if (ret != DEMUXER_CTRL_OK)
			{
				mplayer_debug("@@@@ In [%s][%d] call ctrl failed , ret [%d]@@@@\n", __func__, __LINE__, ret);
			}
		...
		else
		{
			prog.vid = pid;
			prog.vformat = format;
			/* switch_video_stream , maybe not this case*/
		}
	}
