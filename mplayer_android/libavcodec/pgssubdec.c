...
static av_cold int close_decoder(AVCodecContext *avctx)
{
    ...
    if(temp_buf != NULL) av_free(temp_buf);
                                       ...
                                       const uint8_t *buf, int buf_size)
{
    ...
    else if(x == 0 && y == 0 && block == 0x40)		//Fuchun 2011.01.17
    {
	buf += 7;
    ...
    while (buf < buf_end) {
        ...
        case PICTURE_SEGMENT:
	     ...
	     if(ret < 0)
	     {
			...
			if(ret == -2 || (ret == -3 && temp_buf != NULL))
			{
				int last_length = temp_length;
//				av_log(NULL, AV_LOG_ERROR, "### segment_length[%d] ###\n", segment_length);
				if(temp_length == 0)
				{
					temp_length = segment_length;
					temp_buf = (unsigned char *)av_malloc(temp_length);
					memcpy(temp_buf, buf, segment_length);
				}
				else
				{
					temp_length += (segment_length-4);
					temp_buf = (unsigned char *)av_realloc(temp_buf, temp_length);
					memcpy(temp_buf+last_length, buf+4, segment_length-4);
					if((buf[3]) == 0x40)
					{
						av_log(NULL, AV_LOG_ERROR, "retry parse, temp_length[%d]\n", temp_length);
						ret = parse_picture_segment(avctx, temp_buf, temp_length);
					}
					if(ret == 0)
					{
						av_free(temp_buf);
			...
			else if(ret == -3 && temp_buf != NULL)
			{
				temp_buf = (unsigned char *)av_realloc(temp_buf, temp_length+segment_length-4);
				memcpy(temp_buf+temp_length, buf+4, segment_length-4);
				temp_length += segment_length-4;
				av_log(NULL, AV_LOG_ERROR, "retry parse, temp_length[%d]\n", temp_length);
				ret = parse_picture_segment(avctx, temp_buf, temp_length);
				av_free(temp_buf);
