...
#if 1	/* Carlos add, sometimes we have odd y_pos in starting if we followed ffmpeg rules that will be strange subtitle , 2011-11-23*/
	if (top_bottom)
		y_pos++;
#else 
													...
													int buf_size)
{
...
#ifdef DEBUG_PACKET_CONTENTS		
	{
		...
		av_log(avctx, AV_LOG_INFO, "buf_size[%d] display_definition packet: [", buf_size);
		for(ii = 0 ; ii < buf_size; ii++)
			av_log(avctx, AV_LOG_INFO, "%.2x ", buf[ii]);
		av_log(avctx, AV_LOG_INFO, "]\n");
	}
#endif	
	...
	//av_log(avctx, AV_LOG_INFO, "========== In [%s][%d] alloc display_definition display_def is [%p] buf_size[%d] info_byte & 1 <<3[%d]=========\n", __func__, __LINE__, display_def, buf_size, info_byte & 1 <<3);
	...
	//av_log(avctx, AV_LOG_INFO, "========== In [%s][%d] x [%d] y [%d] width[%d] height[%d] avctx w[%d] avctxh[%d] ================\n", __func__, __LINE__, display_def->x, display_def->y, display_def->width, display_def->height, avctx->width, avctx->height);
	...
	if (info_byte & 1<<3) { // display_window_flag
		...
		//av_log(avctx, AV_LOG_INFO, "========== In [%s][%d] x[%d] y[%d] width[%d] height[%d]=========\n", __func__, __LINE__, display_def->x, display_def->y, display_def->width, display_def->height);
	}
}
                                        ...
                                        int buf_size, AVSubtitle *sub)
{
	...
	if (display_def) {
		//av_log(avctx, AV_LOG_INFO, "========== In [%s][%d] x[%d] y[%d]=========\n", __func__, __LINE__, display_def->x, display_def->y);
                         ...
                         AVPacket *avpkt)
{
	...
	while(!(p[0] == 0x20 && p[1] == 0x00 && p[2] == 0x0f))	//Fuchun 2011.01.31
	{
		p++;
		if(p == p_end)
		{
			av_log(avctx, AV_LOG_ERROR, "Not find start code.\n");
			return -1;
		}
	}
	p += 2;
...
//        if (page_id == ctx->composition_id || page_id == ctx->ancillary_id)		//Fuchun 2011.01.31 disable
	 {
            switch (segment_type) {
    ...
    if (p != p_end) {
	 ...
	 av_log(avctx, AV_LOG_ERROR, "Junk at end of packet\n");
        return -1;
    }
