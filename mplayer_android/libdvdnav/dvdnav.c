 ...
 */
int32_t dvdnav_get_vobu(dvdnav_t *this, dsi_t *nav_dsi, pci_t *nav_pci, dvdnav_vobu_t *vobu) {
  uint32_t next;
   ...
   */
//printf("1get_vobu: vobu_next[%d], vobu_start[%d], vobu_length[%d],nav_dsi->vobu_sri.next_vobu[%x]\n",vobu->vobu_next,vobu->vobu_start,vobu->vobu_length,nav_dsi->vobu_sri.next_vobu);
#if 0
  ...
  //vobu->vobu_next = ( nav_dsi->vobu_sri.next_vobu & 0x3fffffff );
  //Mantis:5996    
  if((nav_dsi->vobu_sri.next_vobu & 0x80000000) == 0 && nav_dsi->vobu_sri.next_vobu != 0x3fffffff){
  	//video data doesn't exist in the next VOBU. Spec. VI4-147
	vobu->vobu_next = vobu->vobu_length+1;
  } else {
	//video data exists in the next VOBU
	vobu->vobu_next = ( nav_dsi->vobu_sri.next_vobu & 0x3fffffff );
  }
#endif
//printf("2get_vobu: vobu_next[%d], vobu_start[%d], vobu_length[%d]\n",vobu->vobu_next,vobu->vobu_start,vobu->vobu_length);
  ...
  if(num_angle != 0 /*&& speed_mult == 0*/)	// 20110425 charleslin Mantis 3469 - comment to fix multi-angle issue //Fuchun 2011.03.14 add speed_mult == 0, avoid video freeze at FF 4x state (ICE AGE 3)
  {
...
dvdnav_status_t dvdnav_get_next_block(dvdnav_t *this, uint8_t *buf,
  ...
  //Polun 2011-10-07 fixed ´cÆF¥j³ùCG.iso FF x auto return to  speed_mult 0
...
//printf(" ==> vobu_start[%d],vobu_length[%d],start+length+1[%d],blockN[%d],position_current.cell_start[%d]\n",	this->vobu.vobu_start,this->vobu.vobu_length, this->vobu.vobu_start+this->vobu.vobu_length+1,this->vobu.blockN,this->position_current.cell_start);

  if(this->file!=NULL)
  {
//printf(" ==> filesize[%d],start+length+1[%d]\n",this->file->filesize,this->vobu.vobu_start+this->vobu.vobu_length+1);
  	//Mantis:5999
  	dvd_state_t *state = &this->vm->state;
//printf("first_sector[%d],last_sector[%d]\n",state->pgc->cell_playback[state->cellN-1].first_sector,state->pgc->cell_playback[state->cellN-1].last_sector);
    #if 0     //Polun 2011-10-07 fixed ´cÆF¥j³ùCG.iso FF x auto return to  speed_mult 0
    if( (int)(state->pgc->cell_playback[state->cellN-1].last_sector - (this->vobu.vobu_start+this->vobu.vobu_length+1)) < 550 
  	    && speed_mult != 0)
    #else
    if( (int)(state->pgc->cell_playback[state->cellN-1].last_sector - (this->vobu.vobu_start+this->vobu.vobu_length+1)) < 250 
          && speed_mult != 0 && (((duration_sec_cur-my_current_pts) < 3) || duration_sec_cur < 100))
    #endif
  		FFFR_to_normalspeed(NULL);
  }

  if(this->file!=NULL && dvdnav_error_file)
  {
    //printf("2-filesize[%d]\n",(ssize_t)this->file->filesize);
  	if(this->file->filesize <= (ssize_t)(this->vobu.vobu_start+this->vobu.vobu_length+1+this->vobu.blockN))
	{
		printf("dvdnav.c: Error dvd file play end.\n");
  ...
  if(status == DVDNAV_STATUS_ERR_FILE_INCOMPLETE)
  {
	block = buf;
	dvdnav_error_file = 1;
	dsi_t *nav_dsi = &this->dsi;
//      this->vobu.vobu_start = this->vobu.vobu_start + this->vobu.vobu_length+2;
	nav_dsi->dsi_gi.nv_pck_lbn += this->vobu.vobu_length;
	this->vm->state.blockN = this->vobu.vobu_start - this->position_current.cell_start;

//printf("vobu_start[%d], position_current.cell_start[%d] ,vobu.blockN[%d]\n",this->vobu.vobu_start, this->position_current.cell_start,this->vobu.blockN);

	dvdnav_get_vobu(this, &this->dsi, &this->pci, &this->vobu);
	this->vobu.blockN = 0;

//printf(" ==> vobu.vobu_start[%d],vobu.vobu_length[%d], vobu_start+vobu_length+1[%d]\n",this->vobu.vobu_start,this->vobu.vobu_length, this->vobu.vobu_start+this->vobu.vobu_length+1);
  ...
  else if(status == DVDNAV_STATUS_ERR_NAV_NOT_FOUND)
  {
  	dvdnav_next_pg_search(this);
  	status = DVDNAV_STATUS_OK;
...
dvdnav_status_t dvdnav_get_next_cache_block(dvdnav_t *this, uint8_t **buf,
  ...
  dvd_state_t *state;
  int32_t result;
  static int got_first_NAV=0;
  pthread_mutex_lock(&this->vm_lock);
  static int dvdnav_error_NAV_pkt=0;
//uint8_t *tmp_buf1;
//tmp_buf1 = *buf;
//int z;
//for(z=0;z<30;z++)
//printf("%02x-",tmp_buf1[z]);
//printf("\n");
  if(!this->started) {
    /* Start the VM */
    if (!vm_start(this->vm)) {
  ...
  if(this->position_current.hop_channel != this->position_next.hop_channel) {
	...
	if(result <= 0) {
	  printf("Error reading NAV packet-0.\n");
	...
	if(!dvdnav_decode_packet(this, *buf, &this->dsi, &this->pci)) {
	  printf("Expected NAV packet but none found.-0\n");
	...
	/* update VM state */
	//if(speed_mult > 0)	// 20110425 charleslin Mantis 3469, comment this line to fix multi-angle FR issue
  ...
  if(this->position_current.button != this->position_next.button) {
    ...
    mplayer_dvdnav_debug("## DVDNAV In [%s][%d], hevent->button[%x], this->position_next.button)[%x] ##\n", __func__, __LINE__, hevent->buttonN, this->position_next.button);
...
//+Skyviia_Vincent04252011 //Mantis:3963 Toy.Story.3 menu stage screen lock
// move to after still freame
  /* Check the WAIT flag */
#if 0
  if(this->sync_wait) {
    (*event) = DVDNAV_WAIT;
...
//Skyviia_Vincent04252011+ //Mantis:3963 Toy.Story.3 menu stage screen lock
  ...
  if((this->position_current.vts != this->position_next.vts) ||
    ...
    case VTS_DOMAIN:
      ...
      break;
    default:
      printf("Unknown domain when changing VTS.\n");
    ...
    if(this->file == NULL) {
	  ...
	  //+Skyviia_Vincent04202011 Mantis:4005 CARS can't play menu screen
	  printf("Try to open file from menu domain\n");
      this->file = DVDOpenFile(vm_get_dvd_reader(this->vm), vtsN, DVD_READ_MENU_VOBS);
      vts_event->new_vtsN = this->position_next.vts;
      vts_event->new_domain = this->position_next.domain;
	  if(this->file == NULL) //Skyviia_Vincent04202011+ Mantis:4005 CARS can't play menu screen
	  {
...
//printf("1-vobu_start[%d], blockN[%d], vobu_next[%d]\n",this->vobu.vobu_start, this->vobu.blockN, this->vobu.vobu_next);
if(dvdnav_error_file == 0)
{
...
mplayer_dvdnav_debug("## DVDNAV In [%s][%d], current.cell[%d],next.cell[%d] ##\n", __func__, __LINE__, this->position_current.cell, this->position_next.cell);
mplayer_dvdnav_debug("## DVDNAV In [%s][%d], current.cell_restart[%d],next.cell_restart[%d] ##\n", __func__, __LINE__, this->position_current.cell_restart, this->position_next.cell_restart);
mplayer_dvdnav_debug("## DVDNAV In [%s][%d], current.cell_start[%d],next.cell_start[%d] ##\n", __func__, __LINE__,this->position_current.cell_start, this->position_next.cell_start);
mplayer_dvdnav_debug("## DVDNAV In [%s][%d], cell_event cellN[%d], pgN[%d], cell_length[%u]##\n", __func__, __LINE__, cell_event->cellN ,cell_event->pgN,cell_event->cell_length);
...
//printf("cell_event->cell_length [%lx],state->cellN[%d]\n",cell_event->cell_length ,state->cellN);
//printf("playbacktime minute[%d] sec[%x]\n", state->pgc->cell_playback[state->cellN-1].playback_time.minute,state->pgc->cell_playback[state->cellN-1].playback_time.second);
//printf("first_sector[%d],last_sector[%d]\n",state->pgc->cell_playback[state->cellN-1].first_sector,state->pgc->cell_playback[state->cellN-1].last_sector);
  ...
  /* has the CLUT(colour lookup table) changed? */
...
//Skyviia_Vincent04252011+ //Mantis:3963 Toy.Story.3 menu stage screen lock
  ...
  if (this->vobu.blockN >= this->vobu.vobu_length && dvdnav_error_file==0) {
    ...
    if(this->vobu.vobu_next == SRI_END_OF_CELL) {
      ...
      if ((this->position_current.still || this->pci.hli.hl_gi.hli_ss) && !this->sync_wait_skip) {
        this->sync_wait = 1;
        if(this->pci.hli.hl_gi.hli_ss)
		dvdnav_menupage_chg = 1;
      } else {
    ...
    if(dvdnavstate.dvdnav_title_state == DVDNAV_TITLE_STATE_MOVIE && got_first_NAV==0 && dvdnavstate.dvdnav_state_change == 1)
    {
	uint8_t *tmp_buf;
	int x=0;
...
Error_NAV_PKT:
	    ...
	    result = dvdnav_read_cache_block(this->cache, this->vobu.vobu_start + this->vobu.vobu_next+x, 1, buf);
	    x++;
	    tmp_buf = *buf;
	    int z;
	
	    if(result <= 0) {
	      printf("Error reading NAV packet.-1\n");
	      printerr("Error reading NAV packet.");
	      pthread_mutex_unlock(&this->vm_lock);
	      return DVDNAV_STATUS_ERR;
	    }
	    ...
	    /* Decode nav into pci and dsi. Then get next VOBU info. */
	    //if(!dvdnav_decode_packet(this, *buf, &this->dsi, &this->pci)) {
	    if(!dvdnav_decode_packet(this, tmp_buf, &this->dsi, &this->pci)) {
	      printf("Expected NAV packet but none found.-1\n");
	      printerr("Expected NAV packet but none found.");
	      dvdnav_error_NAV_pkt = 1;
	      pthread_mutex_unlock(&this->vm_lock);
	      //return DVDNAV_STATUS_ERR;
	      if(x>30) // try 30 packets whether it NAV packet
	      	return DVDNAV_STATUS_ERR;
	    }
	    else
	    {
...
//printf("1-vobu_start[%d], blockN[%d], vobu_next[%d]\n",this->vobu.vobu_start, this->vobu.blockN, this->vobu.vobu_next);
	    ...
	    if(result <= 0) {
	      printf("Error reading NAV packet.-2\n");
	      printerr("Error reading NAV packet.");
	      pthread_mutex_unlock(&this->vm_lock);
	      return DVDNAV_STATUS_ERR;
	    }
	    ...
	    /* Decode nav into pci and dsi. Then get next VOBU info. */
	    if(!dvdnav_decode_packet(this, *buf, &this->dsi, &this->pci)) {
	      printf("Expected NAV packet but none found.-2\n");
	      printerr("Expected NAV packet but none found.");
	      if(dvdnav_error_NAV_pkt == 1)
	      	goto Error_NAV_PKT; //Mantis:4889
//int i;
//for(i=0;i<state->pgc->nr_of_cells;i++)
//printf("i[%d],first_sector[%d],last_sector[%d]\n",i,state->pgc->cell_playback[i].first_sector,
//	state->pgc->cell_playback[i].last_sector);

//for(i=0;i<state->pgc->nr_of_cells;i++)
//printf("first_sector[%d],last_sector[%d],last_sector-2[%d]\n",state->pgc->cell_playback[state->pgc->nr_of_cells-1].first_sector,
//	state->pgc->cell_playback[state->pgc->nr_of_cells-1].last_sector,state->pgc->cell_playback[state->pgc->nr_of_cells-1].last_sector+1);
//printf("2-filesize[%d]\n",(ssize_t)this->file->filesize);

		pthread_mutex_unlock(&this->vm_lock);
		if(this->file->filesize < ((state->pgc->cell_playback[state->pgc->nr_of_cells-1].last_sector+1)))
		{
			printf("dvdnav.c: dvd file size incomplete\n");
			return DVDNAV_STATUS_ERR_FILE_INCOMPLETE;
		}

		//return DVDNAV_STATUS_ERR;
		return DVDNAV_STATUS_ERR_NAV_NOT_FOUND;
	    }
    }
    ...
    if(dvdnavstate.dvdnav_title_state == DVDNAV_TITLE_STATE_MOVIE && dvdnavstate.dvdnav_state_change == 1)
    {
    	got_first_NAV=1;
    }
    else if(dvdnavstate.dvdnav_title_state == DVDNAV_TITLE_STATE_MENU)
	got_first_NAV=0;
    ...
    pthread_mutex_unlock(&this->vm_lock);
    return DVDNAV_STATUS_OK;
  } //End Have we reached the end of a VOBU? 

  /* If we've got here, it must just be a normal block. */
  if(!this->file) {
    printf("Attempting to read without opening file-1.\n");
    printerr("Attempting to read without opening file.");
    pthread_mutex_unlock(&this->vm_lock);
    return DVDNAV_STATUS_ERR;
  }
//printf("2-vobu_start[%d], blockN[%d], vobu_next[%d], vobu_length[%d]\n",this->vobu.vobu_start, this->vobu.blockN, this->vobu.vobu_next, this->vobu.vobu_length);
  this->vobu.blockN++;
  result = dvdnav_read_cache_block(this->cache, this->vobu.vobu_start + this->vobu.blockN, 1, buf);
  if(result <= 0) {
    printf("Error reading from DVD.\n");
    printerr("Error reading from DVD.");
    pthread_mutex_unlock(&this->vm_lock);
    return DVDNAV_STATUS_ERR;
  }
#if 1
  //Mantis:3601 the file of the vobu_length and vobu_next information are wrong
  if(this->file->filesize >= ((state->pgc->cell_playback[state->pgc->nr_of_cells-1].last_sector+1)))
  {
	if (dvdnav_decode_packet(this, *buf, &this->dsi, &this->pci))
  	{
		this->vobu.blockN--;
		this->vobu.vobu_length = this->vobu.blockN;
		this->vobu.vobu_next = this->vobu.blockN+1;
  	}
  ...
  if (dvdnav_decode_packet(this, *buf, &this->dsi, &this->pci)) {
    ...
    (*event) = DVDNAV_NAV_PACKET;
#ifdef LOG_DEBUG
    fprintf(MSG_OUT, "libdvdnav: NAV_PACKET-2\n");
#endif
    this->cur_cell_time = dvdnav_convert_time(&this->dsi.dsi_gi.c_eltm);
  } else 
#endif
...
/*  */
dvdnav_status_t dvdnav_sub_change(dvdnav_t *this, int32_t sub) {
  int32_t num, current;

  pthread_mutex_lock(&this->vm_lock);

printf("----------dvdnav_sub_change----------\n");
//printf("1-this->vm->state.SPST_REG[%x],sub[%d]\n",this->vm->state.SPST_REG,sub);
  if(sub == -3)
  {
  	//clear SPRM 2 (subtitle register)
  	this->vm->state.SPST_REG = 0;
  }
  else
  {
  	//set SPRM 2 (subtitle register)
  	this->vm->state.SPST_REG = 0x40 | sub;
  }
//printf("2-this->vm->state.SPST_REG[%x],sub[%d]\n",this->vm->state.SPST_REG,sub);
  pthread_mutex_unlock(&this->vm_lock);

  return DVDNAV_STATUS_OK;
}

dvdnav_status_t dvdnav_audio_change(dvdnav_t *this, int32_t audio) {
  int32_t num, current;

  pthread_mutex_lock(&this->vm_lock);

//printf("----------set audio registerdvdnav_sub_change------audio[%d]----\n",audio);
  //set SPRM 1 (audio register)
  this->vm->state.AST_REG = audio;
...
dvdnav_status_t dvdnav_get_spu_info(dvdnav_t *this, int32_t *current_spu,
				      int32_t *number_of_spu) {
  pthread_mutex_lock(&this->vm_lock);
  vm_get_subp_info(this->vm, current_spu, number_of_spu);
  pthread_mutex_unlock(&this->vm_lock);

  return DVDNAV_STATUS_OK;
}

dvdnav_status_t dvdnav_get_audio_info(dvdnav_t *this, int32_t *current_audio,
				      int32_t *number_of_audio) {
  pthread_mutex_lock(&this->vm_lock);
  vm_get_audio_info(this->vm, current_audio, number_of_audio);
  pthread_mutex_unlock(&this->vm_lock);
