...
dvdnav_status_t dvdnav_get_number_of_program(dvdnav_t *this, int32_t *nr_of_programs) {
  if (!this->vm->vmgi) {
    printerr("Bad VM state.");
    return DVDNAV_STATUS_ERR;
  }

  (*nr_of_programs) = this->vm->state.pgc->nr_of_programs;
...
dvdnav_status_t dvdnav_part_play(dvdnav_t *this, int32_t title, int32_t part) {
  ...
  {
    this->vm->hop_channel++;
    this->position_current.still = 0; //JUNO seek chapter at warning page cause Encounterred error
  }
  pthread_mutex_unlock(&this->vm_lock);
