...
int skip_cmd=0; //Skyviia_Vincent10062011 Taken_partial
int dvdnav_cmd_exit_check = 0;//Polun 2012-02-03 fixed mantis6790 playback to eof signal 11 issue  
...
static int32_t eval_jump_instruction(command_t* command, int32_t cond, link_t *return_values) {
  ...
  switch(vm_getbits(command, 51, 4)) {
    ...
    case 5:
      ...
      return_values->data1 = vm_getbits(command, 22, 7); //VTS_TTN
      return_values->data2 = vm_getbits(command, 41, 10); //PTTN
      return cond;
    case 6:
...
static int32_t eval_system_set(command_t* command, int32_t cond, link_t *return_values) {
  ...
  extern int seek_chapter; //Mantis:4564 BOLT seek_chapter cause set subtitle and audio from NAV command
    ...
    case 1: /*  Set system reg 1 &| 2 &| 3 (Audio, Subp. Angle) */
      for(i = 1; i <= 3; i++) {
        if(vm_getbits(command, 63 - ((2 + i)*8), 1)) {
          ...
          if(cond) {
			mplayer_dvdnav_debug("## DVDNAV In [%s][%d], cond[%d],data[%d],i[%d]##\n", __func__, __LINE__,cond,data,i);
			if(i==2 && seek_chapter==0) //if seek chapter than ignore to run pgc command
				dvdnav_sub_menu_select = 1;
			if(i==1 && seek_chapter==0) //if seek chapter than ignore to run pgc command
...
static int32_t eval_command(uint8_t *bytes, registers_t* registers, link_t *return_values) {
  ...
  switch(vm_getbits(&command, 63, 3)) { /* three first old_bits */
    ...
    case 1: /*  Link/jump instructions */
      ...
      if(res) //1 if jump or 0 if no jump
	res = -1;
      break;
    ...
    case 6: /*  Compare -> Set, allways Link Sub-Instruction */
      /* FIXME: These are wrong. Need to be updated from vmcmd.c */
      cond = eval_if_version_4(&command);
	  if(!cond) //mantis:3935 illegal operator will be skipped
	  {
		printf("Unknown set op\n");
		res = 0;
		break;
	  }
      eval_set_version_2(&command, cond);
    ...
    case 7: 
      cond = eval_if_version_4(&command);
	  if(!cond) //mantis:4697 illegal operator will be skipped
	  {
		printf("Unknown set op\n");
		res = 0;
		break;
...
//////////////////////////////////////////
//TAKEN_partial
//check dvdnav command first, to check which command should be skipped.
  command_t command;
  int jump=0;
  command.instruction =( (uint64_t) commands[i].bytes[0] << 56 ) |
        ( (uint64_t) commands[i].bytes[1] << 48 ) |
        ( (uint64_t) commands[i].bytes[2] << 40 ) |
        ( (uint64_t) commands[i].bytes[3] << 32 ) |
        ( (uint64_t) commands[i].bytes[4] << 24 ) |
        ( (uint64_t) commands[i].bytes[5] << 16 ) |
        ( (uint64_t) commands[i].bytes[6] <<  8 ) |
          (uint64_t) commands[i].bytes[7] ;
  command.examined = 0;
  ...
  switch(vm_getbits(&command,63,3)) { /* three first bits */
    ...
    case 1: /*  Jump/Call or Link instructions */
      if(vm_getbits(&command,60,1)) {
          ...
          //else if(vm_getbits(&command, 51, 4) == 2)
          else //Polun 2012-02-03 fixed mantis6790 playback to eof signal 11 issue  
          { 
              int jump_inst;
              jump_inst = vm_getbits(&command, 51, 4);
              if(jump_inst == 2)
              {
        	    vm_getbits(&command, 22, 7);
        	    jump = 1;
        	}
              else if(jump_inst == 1)
              {
                  dvdnav_cmd_exit_check = 1;
                  printf("##Decoder.c  vmEval_CMD() check cmd is exit ## \n");
              }
          }
  ...
  if((command.instruction & ~ command.examined) && jump)
  {
  	skip_cmd = 1;
  	printf("decoder.c: Illegal instruction, skip it.\n");
  }
  jump = 0;
//////////////////////////////////////////
