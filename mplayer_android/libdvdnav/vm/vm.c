...
static link_t play_Cell_past(vm_t *vm);
...
int Dvdnav_cmd_exit;//Polun 2012-02-03 fixed mantis6790 playback to eof signal 11 issue  
extern int dvdnav_cmd_exit_check;//Polun 2012-02-03 fixed mantis6790 playback to eof signal 11 issue  
/* Helper functions */
...
//printf("first_sector[%x], last_vobu_start_sector[%x], last_sector[%x]\n",(vm->state).pgc->cell_playback[(vm->state).cellN - 1].first_sector,
...
//printf("sec-1[%x],sec-2[%x]\n",((vm->state).pgc->cell_playback[(vm->state).cellN - 1].playback_time.second >> 4  ) * 10,((vm->state).pgc->cell_playback[(vm->state).cellN - 1].playback_time.second & 0x0f) * 1);

    //+JUNO seek chapter at warning page cause Encounterred error
    if((vm->state).pgc->cell_playback[(vm->state).cellN - 1].first_sector ==0 && 
       (vm->state).pgc->cell_playback[(vm->state).cellN - 1].last_vobu_start_sector ==0 &&
       (vm->state).pgc->cell_playback[(vm->state).cellN - 1].last_sector ==0)
    {
	return;
    }
    //JUNO+ seek chapter at warning page cause Encounterred error
...
int vm_jump_cell_block(vm_t *vm, int cell, int block) {
	extern int speed_mult;
       //Polun 2011-07-21 ++s  for mantis 5550
       int angle_num = 0, current;
       vm_get_angle_info(vm, &current, &angle_num);
       //Polun 2011-07-21 ++e  for mantis 5550
	if(speed_mult >= 2 && cell != (vm->state).cellN)		//Fuchun 2011.03.14 avoid show begining video of film at FF 4x
	{
//printf("\n======> speed_mult=%d cell:%d (vm->state).cellN:%d\n", speed_mult, cell, (vm->state).cellN);
		process_command(vm, play_Cell_post(vm));
	}
#if 1
	//else if(speed_mult < 0 && cell != (vm->state).cellN)
	else if(speed_mult < 0 && cell != (vm->state).cellN && angle_num > 1) //Polun 2011-07-21 ++s  for mantis 5550
	{
//printf("\n======> speed_mult=%d cell:%d (vm->state).cellN:%d\n", speed_mult, cell, (vm->state).cellN);
		process_command(vm, play_Cell_past(vm));
//printf("(vm->state).cellN=%d\n", (vm->state).cellN);
                (vm->state).blockN = (vm->state).pgc->cell_playback[(vm->state).cellN - 1].last_sector - (vm->state).pgc->cell_playback[(vm->state).cellN - 1].first_sector+1;
//printf(" .... new  (vm->state).blockN = %d\n",  (vm->state).blockN);
	}
#endif
	else
	{
...
/* 
maybe we can using
// http://lists.mplayerhq.hu/pipermail/dvdnav-discuss/2010-July/001280.html
...
//printf("(vm->state).domain[%d], subpN[%d], streamN[%d] ,mode[%d], aspect[%d]\n",(vm->state).domain,subpN,streamN,mode,source_aspect);
  if((vm->state).domain != VTS_DOMAIN && streamN == -1)
    streamN = 0;
...
int vm_get_subp_active_stream(vm_t *vm, int mode) {
  ...
  mplayer_dvdnav_debug("## DVDNAV In [%s][%d], subpN[%d],streamN[%d],(vm->state).SPST_REG[%x]##\n", __func__, __LINE__,subpN,streamN,(vm->state).SPST_REG);
...
void vm_get_subp_info(vm_t *vm, int *current, int *num_avail) {
  ...
  case VTS_DOMAIN:
    ...
    *current = (vm->state).SPST_REG & ~0x40;
    break;
  case VTSM_DOMAIN:
...
extern int duration_sec_cur;
  ...
  if(!set_PGCN(vm, (vm->state).pgc->next_pgc_nr) && duration_sec_cur != 0) {
...
#ifdef TRACE
  fprintf(MSG_OUT, "libdvdnav: play_Cell_past: (vm->state).cellN (%i)\n", (vm->state).cellN);
#endif
//printf("\n=====> libdvdnav: play_Cell_past: (vm->state).cellN (%i)\n", (vm->state).cellN);
...
//printf("\n==> 000  Normal,  new cellN=%d\n", (vm->state).cellN);
  ...
  default:
/*
if ((vm->state).pgc->cell_playback[(vm->state).cellN - 1].block_mode == 1)
printf("\n==> 000  The first cell in the block\n");
else if ((vm->state).pgc->cell_playback[(vm->state).cellN - 1].block_mode == 2)
printf("\n==> 000  A cell in the block\n");
else if ((vm->state).pgc->cell_playback[(vm->state).cellN - 1].block_mode == 3)
printf("\n==> 000  The last cell in the block\n");
*/
    ...
    case 1: /*  Angle block */
      ...
      /* Skip the 'other' angles */
      (vm->state).cellN--;
//printf("\n==> 001  --> 00 Angle block, new cellN=%d sector_length:%d\n", (vm->state).cellN, (vm->state).pgc->cell_playback[(vm->state).cellN - 1].last_sector - (vm->state).pgc->cell_playback[(vm->state).cellN - 1].first_sector);
      vm_get_angle_info(vm, &current, &angle_num);

      if ((angle_num > 1 && current == 1) || angle_num == 1)
      {
          while((vm->state).cellN <= (vm->state).pgc->nr_of_cells &&
	    (vm->state).pgc->cell_playback[(vm->state).cellN - 1].block_mode >= 2) {
//printf("\n==> 001  --> 01 block_mode = %d, new cellN:%d\n", (vm->state).pgc->cell_playback[(vm->state).cellN - 1].block_mode, (vm->state).cellN-1);
  	      (vm->state).cellN--;
...
//printf("\n==> 001  --> 01 block_mode = %d, new cellN:%d\n", (vm->state).pgc->cell_playback[(vm->state).cellN - 1].block_mode, (vm->state).cellN-1);
              ...
              (vm->state).cellN--;
//printf("\n===> 00 0.5  current=%d  (vm->state).cellN=%d\n", current, (vm->state).cellN);

              if ((vm->state).pgc->cell_playback[(vm->state).cellN - 1].block_mode > 0)
              {
                while((vm->state).cellN <= (vm->state).pgc->nr_of_cells && ((vm->state).cellN > 2) &&
	            (vm->state).pgc->cell_playback[(vm->state).cellN - current].block_mode != 1) {
//printf("\n==> 002  --> 01 block_mode = %d, new cellN:%d\n", (vm->state).pgc->cell_playback[(vm->state).cellN - 1].block_mode, (vm->state).cellN-1);
	            (vm->state).cellN--;
                }
              }
//printf("\n====> 003  new (vm->state).cellN = %d\n", (vm->state).cellN);
          }
      }
      break;
    case 2: /*  ?? */
...
#endif
//printf("-------------link_values.command[%d]\n",link_values.command); //Vincent
	...
	int i;
	extern int dvdnav_error_call_menu;

	if(dvdnav_error_call_menu) //stream read error, play at beginning 
	{
		dvdnav_error_call_menu = 0;
      		if(set_TT(vm, 1))
        		link_values = play_PGC(vm);
		break;
	}	
...
//printf("menu_rsm_cmd[%d]\n",menu_rsm_cmd);
      extern int change_flag;
      if(menu_rsm_cmd)
      	change_flag = 2;
      menu_rsm_cmd = 0;

      extern unsigned int resync_video; //LADDA_LAND_D5.iso
      resync_video = 0;
    ...
    case Exit:
      printf("vm.c: something wrong or cmd exit, we need to exit now.\n");
      if(dvdnav_cmd_exit_check == 1)
          Dvdnav_cmd_exit = 1; //Polun 2012-02-03 fixed mantis6790 playback to eof signal 11 issue  
      vm->stopped = 1;
      return 0;
    ...
    case JumpTT:
      ...
      if (!((vm->state).domain == VMGM_DOMAIN || (vm->state).domain == FP_DOMAIN))
      {
	    //This is work around for war_1.iso
	    printf("assert (vm->state).domain == VMGM_DOMAIN || (vm->state).domain == FP_DOMAIN\n");
      }
	...
	if (link_values.data1 != (vm->state).vtsN) {
	  ...
	  if(!((vm->state).domain == VMGM_DOMAIN || (vm->state).domain == FP_DOMAIN))
	  {
	    //Mantis:5367 lastman.iso 
	    printf("JumpSS_VTSM:assert (vm->state).domain == VMGM_DOMAIN || (vm->state).domain == FP_DOMAIN\n");
	  }
	  (vm->state).domain = VTSM_DOMAIN;
...
//printf("_____set_PGCN:::pgcN[%d], (vm->state).pgcN[%d]\n",pgcN, (vm->state).pgcN);
  if(pgcN != (vm->state).pgcN)
  {
  	dvdnav_pgcN_change = 1;
  }
  else
...
//printf("set_PGN----new_pgN[%d],cellN[%x]\n",new_pgN,(vm->state).cellN);
  if((vm->state).domain == VTS_DOMAIN) {
...
//ifo_print(vm->dvd, title);//Skyviia_Vincent07152011 for debug
