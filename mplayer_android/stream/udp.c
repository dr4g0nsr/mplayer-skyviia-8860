...
udp_open_socket (URL_t *url)
{
  ...
  //20111202 charleslin 240KB is too small for HiHD streaming, sock buf full may lost data and cause mosaic
  // use SO_RCVBUFFORCE instead of SO_RCVBUF to modify receive buffer size more than 240K
  //rxsockbufsz = 240 * 1024;
  //if (setsockopt (socket_server_fd, SOL_SOCKET, SO_RCVBUF, &rxsockbufsz, sizeof (rxsockbufsz)))
  rxsockbufsz = 1024 * 1024;
  if (setsockopt (socket_server_fd, SOL_SOCKET, SO_RCVBUFFORCE, &rxsockbufsz, sizeof (rxsockbufsz)))
  {
    mp_msg (MSGT_NETWORK, MSGL_ERR,
            "Couldn't set receive socket buffer size\n");
  }

  int rxsockbufsz_len = sizeof(rxsockbufsz);
  getsockopt(socket_server_fd, SOL_SOCKET, SO_RCVBUF, &rxsockbufsz, &rxsockbufsz_len);
  printf("=========== rxsockbufsz:%d ===========\n", rxsockbufsz);
