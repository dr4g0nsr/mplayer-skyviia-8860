...
connect2Server_with_af(char *host, int port, int af,int verb) {
	...
	static int prev_af = 0;
	static int conn_fail = 0;

	if(prev_host == NULL && prev_port == 0 && prev_af == 0){
		prev_host = strdup(host);
		prev_port = port;
		prev_af = af;
	}else if(conn_fail && !strcmp(host, prev_host) && port == prev_port && af == prev_af){
		printf("%s:%d previous connection fail host:%s port:%d af:%d\n", __func__, __LINE__, host, port, af);
...
//		mp_msg(MSGT_NETWORK,MSGL_ERR,"Failed to create %s socket:\n", af2String(af));
	...
	to = network_timeout * 1000;
#else
	to.tv_sec = network_timeout;
	to.tv_usec = 0;
#endif
	...
	//setsockopt(socket_server_fd, SOL_SOCKET, SO_RCVBUF, &bufsize, sizeof(bufsize));
	setsockopt(socket_server_fd, SOL_SOCKET, SO_KEEPALIVE, &keepalive, sizeof(keepalive));
	//setsockopt(socket_server_fd, IPPROTO_TCP, TCP_NODELAY, &nodelay, sizeof(nodelay));
#endif
	...
	if(err > 0) {
		if(err == ETIMEDOUT){
			char msg[128];
			snprintf(msg, 128, "status: %s", strerror(err));
			ipc_callback(msg);
		}
