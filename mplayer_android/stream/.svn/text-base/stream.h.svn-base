...
#define STREAM_CTRL_SET_SUB 12
#define STREAM_CTRL_GET_CURRENT_TITLE 13
#define STREAM_CTRL_SET_AUDIO 14
#define STREAM_CTRL_GET_SUB 15
...
typedef struct stream_st {
  ...
  unsigned char *abuf;
  unsigned char *vbuf;
  unsigned int abuf_pos,abuf_len;
  unsigned int vbuf_pos,vbuf_len;
#endif
} stream_t;
...
extern int aio;
extern int nbuf;
extern ssize_t aio_fill_buffer(stream_t *s);
#endif
...
inline static unsigned int stream_read_word(stream_t *s){
  int x,y;
if (s->buf_len-2>s->buf_pos)
{
  s->buf_pos += 2;
  return s->buffer[s->buf_pos-2]<<8 | s->buffer[s->buf_pos-1];
}
else
{
...
inline static unsigned int stream_read_dword(stream_t *s){
  unsigned int y;
if (s->buf_len-4>s->buf_pos)
{
  s->buf_pos += 4;
  return s->buffer[s->buf_pos-4]<<24 | s->buffer[s->buf_pos-3]<<16 | s->buffer[s->buf_pos-2]<<8 | s->buffer[s->buf_pos-1];
}
else
{
  y=stream_read_char(s);
...
inline static int stream_seek(stream_t *s,off_t pos){
  ...
  //printf("%s: seek to 0x%llX s->pos 0x%llx s->buf_len 0x%x\n", __func__, (long long)pos, s->pos, s->buf_len);
...
extern int bluray_title;
extern int bluray_m2ts;
