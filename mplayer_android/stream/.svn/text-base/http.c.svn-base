...
#define BACK_TIME	5.0
#define SEEK_KEY	"ivij"
//#define SEEK_KEY	"begin"
#define PLAYBACK_KEY	"/videoplayback"
//#include "mp_core.h"
//extern MPContext *mpctx;
extern double ds_last_pts;
int youtube = 0;
int youtube_reconnect = 0;
extern unsigned long long last8byte;
#endif
int reconnect_threshold = 20;
...
static int nop_streaming_start( stream_t *stream ) {
	...
	fd = stream->fd;
	if( fd<0 ) {
		//charleslin 20110324 stream shouldn't read previous buffer, free it
		if( stream->streaming_ctrl->buffer ) {
			streaming_buffer_free(stream->streaming_ctrl);
		}
...
http_set_uri( HTTP_header_t *http_hdr, const char *uri ) {
...
#ifdef YOUTUBE_RECONNECT
	if(youtube && youtube_reconnect && ds_last_pts > BACK_TIME){
		char *lasts = NULL;
		char *tmpuri = NULL;
		char *token = NULL;
		char *ptr = NULL;
		int i, b, r;
		int len, remain;
		int begin = (ds_last_pts - BACK_TIME) * 1000;

		//printf("%s:%d %s\n", __func__, __LINE__, uri);

		printf("ds_last_pts %f begin %d\n", ds_last_pts, begin);

		tmpuri = strdup(uri);
		if( tmpuri == NULL ) {
			printf("%s:%d duplicate uri failed\n", __func__, __LINE__);
			return;
		}

		remain = strlen(uri) + 100 + 1;
		http_hdr->uri = malloc(remain);
		...
		i = b = r = 0;
		ptr = http_hdr->uri;
		token = strtok_r(tmpuri, "?", &lasts);
		do{
			if(token == tmpuri){
				len = snprintf(ptr, remain, "%s?", token);
#if 1
			}else if(strncmp(token, "ivij", 4) == 0){
				len = snprintf(ptr, remain, "&ivij=%d", begin);
				i = 1;
#endif
#if 1
			}else if(strncmp(token, "begin", 5) == 0){
				len = snprintf(ptr, remain, "&begin=%d", begin);
				b = 1;
#endif
#if 1
			}else if(strncmp(token, "redirect_counter", 16) == 0){
				len = snprintf(ptr, remain, "&redirect_counter=%d", 1);
				r = 1;
#endif
			}else{
				len = snprintf(ptr, remain, "&%s", token);
...
#endif
		//snprintf(ptr, remain, "&ivij=%d&begin=%d&redirect_counter=1", begin, begin);
	...
	//printf("%s:%d %s\n", __func__, __LINE__, http_hdr->uri);
}
...
int http_streaming_start(stream_t *stream, int* file_format) {
	...
	do
	{
		...
		if( http_hdr!=NULL && http_hdr->status_code==200 ) {
			...
			seekable = 1;
			printf("%s:%d force http retry\n", __func__, __LINE__);
		    }
		    printf("%s:%d this http stream is %s seekable\n", __func__, __LINE__, seekable ? "" : "not");
		}
		...
		switch( http_hdr->status_code ) {
			case 200: // OK
				...
				if (content_length) {
					int newlen = atoll(content_length);
					mp_msg(MSGT_NETWORK,MSGL_V,"Content-Length: [%s]\n", content_length);
					printf("%s:%d Content-Length: [%s]\n", __func__, __LINE__, content_length);
#ifdef YOUTUBE_RECONNECT
					// reconnect to youtube, don't update end_pos
					if(youtube && stream->end_pos){
						int size;
						off_t remain = stream->end_pos - stream->pos;
						off_t seeklen = newlen - remain;

						// we may read extra body data in http_hdr
						if(http_hdr->body_size){
							printf("%s:%d body_size %x\n", __func__, __LINE__, http_hdr->body_size);
							if(seeklen >= http_hdr->body_size){
								seeklen -= http_hdr->body_size;
							}else{
								//FIXME: we should use the data in http_hdr->body
								printf("%s:%d FIXME: body_size > seeklen\n", __func__, __LINE__);
								seeklen = 0;
							}
						}

						printf("%s:%d endpos %llx pos %llx remain %llx seeklen %llx\n", __func__, __LINE__, stream->end_pos, stream->pos, remain, seeklen);
						while(seeklen > 0){
							char buf[1024];
							size = (seeklen < 1024) ? seeklen : 1024;
							size = recv(fd, buf, size, 0);
							if(size <= 0){
								printf("skip failed\n");
								break;
							}
							seeklen -= size;
#if 0
							for(i=0; i + sizeof(unsigned long long) <= size; i++){
								if(*(unsigned long long *)&buf[i] == last8byte){
									int residue = size - i - 8;
									printf("%s:%d found pattern:%llx seeklen:%llx residue:%x\n", __func__, __LINE__, last8byte, seeklen, residue);
									break;
								}
							}
#endif
...
#ifdef CACHE_IN_FILE
static void *http_save(void *data)
{
	int len, ret;
	char buf[2048];
	...
	int newfd;
	int infd = stream->fd;
	int outfd = open(fcache_fn, O_WRONLY | O_CREAT | O_TRUNC, 0644);
	...
	newfd = open(fcache_fn, O_RDONLY);
	if(newfd < 0){
		printf("open file for read error:%s\n", strerror(errno));
		return NULL;
	}

	printf("Start buffering\n");
	stream->fd = newfd;
	while(len = read(infd, buf, 2048)){
		if(len < 0){
...
static int open_s1(stream_t *stream,int mode, void* opts, int* file_format) {
	...
	// 20110805 charleslin, mantis 5668, we don't want to use asf streaming to handle upnp wma music, because seek function will not work
	if((seekable < 0) ||
	   (upnp == 0 && upnp_duration == 0 && *file_format == DEMUXER_TYPE_ASF))
	{
		if (stream->fd >= 0)
			closesocket(stream->fd);
		stream->fd = -1;
	...
	if(fcache && fcache_fn){
		stream->type = STREAMTYPE_STREAM;
		pthread_create(&tid, NULL, http_save, stream);
		if(fcache_time == 0){ // wait untile whole file downloaded
