...
//20110721 charleslin use default user agent, otherwise shoutcast may not work
char *network_useragent=NULL;
//char *network_useragent="Mozilla/5.0 (Windows; U; Windows NT 5.1; en-US) AppleWebKit/534.7 (KHTML, like Gecko) Chrome/7.0.517.44";
//char *network_useragent="Mozilla/5.0 (Windows; U; Windows NT 5.1; zh-TW; rv:1.9.2.12) Gecko/20101026 Firefox/3.6.12";
...
#ifdef DUMP_RECVDATA
#define DUMP_FILENAME "/mnt/usb81/recvdata.raw"
int dumpfd = -1;
#endif
...
#ifdef DUMP_RECVDATA
	dumpfd = open(DUMP_FILENAME, O_WRONLY | O_CREAT | O_TRUNC, 0644);
#endif
...
void streaming_buffer_free(streaming_ctrl_t *streaming_ctrl)
{
...
streaming_bufferize( streaming_ctrl_t *streaming_ctrl, char *buffer, int size) {
	//charleslin 20110325 free previous buffer to avoid memory leak
	if( streaming_ctrl->buffer ) {
		//printf("%s:%d free previous buffer:%p len:%x\n", __func__, __LINE__, streaming_ctrl->buffer, streaming_ctrl->buffer_size);
		streaming_buffer_free(streaming_ctrl);
	}
...
//printf("read %x bytes from buffer\n", len );
	}
...
retry:
		...
		if( ret<0 ) {
			mp_msg(MSGT_NETWORK,MSGL_ERR,"%s:%d retry_cnt:%d error:%d(%s)\n", __func__, __LINE__, retry_cnt, errno, strerror(errno));
			if(errno == EINTR && retry_cnt > 0){
				retry_cnt--;
				goto retry;
			}else{
				if(errno == EAGAIN){
					char msg[128];
					snprintf(msg, 128, "status: %s", strerror(errno));
					ipc_callback(msg);
				}
				printf("%s:%d stop retry\n", __func__, __LINE__);
			}
		}
#ifdef DUMP_RECVDATA
		else if(ret > 0 && dumpfd >= 0)
			write(dumpfd, buffer+len, ret);
#endif
		len += ret;
...
void fixup_network_stream_cache(stream_t *stream) {
  if(stream->streaming_ctrl->buffering) {
    if(stream_cache_size<0) {
      ...
      //printf("prebuffer_size %d, stream_cache_size %d\n", stream->streaming_ctrl->prebuffer_size, stream_cache_size);
