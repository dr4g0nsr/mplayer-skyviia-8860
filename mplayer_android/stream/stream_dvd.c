...
static int dvd_playback_time(dvd_time_t *dtime)
{
    int nRet;
    int hour, minute, second;
    
    hour = (((dtime->hour >> 4) * 10) + (dtime->hour & 0xf));
    minute = (((dtime->minute >> 4) * 10) + (dtime->minute & 0xf));
    second = (((dtime->second >> 4) * 10) + (dtime->second & 0xf));
//printf("hour[%d],minute[%d],second[%d]\n",hour,minute,second);
    nRet = ((dtime->hour * 60) + dtime->minute) * 60 + dtime->second;

    return nRet;
}

static int dvd_number_of_audios(ifo_handle_t *vts_file, int ttn)
{
    int nRet = 0;
    int i;
    pgc_t *pgc;
...
///////////////////reference from VLC start/////////////////////////////
    int ii, jj,title_last,vts_num,i_ttn;
    uint32_t title_sector_len=0,title_sector_len_tmp;
    int pgc_id_1 = 0, pgn_1 = 0;
    int i_title_start_cell;
    int i_title_end_cell;
    int i_title_start_block;
    int i_title_end_block;
    int i_title_blocks,max_title_blocks=0;
    int i_title_offset;
    pgc_t *p_pgc;
    int i_start_cell, i_end_cell;
    int i;
    ifo_handle_t *p_vts;
    // choice largest sector as default title
    for (ii = 0; ii < tt_srpt->nr_of_srpts; ii++)
    {
//    	if(tt_srpt->title[ii].vts_ttn == 1)
    	{
        	i_ttn = tt_srpt->title[ii].vts_ttn;
		p_vts = ifoOpen( dvd, tt_srpt->title[ii].title_set_nr );
		if (!p_vts) {
			mp_msg(MSGT_OPEN,MSGL_ERR, MSGTR_DVDnoIFO, tt_srpt->title[ii].title_set_nr );
			#if 1	//Barry 2011-05-13
			printf("[%s - %d] Loss IFO file for DVD title!!\n", __func__, __LINE__);
			continue;
			#else
			goto fail;
			#endif
		...
		if(p_vts->vtsi_mat->vts_tmapt == 0)
		{
			// data incomplete, we skip this title
			if( p_vts ) ifoClose( p_vts );
			continue;
		}

        	/* Find title start/end */
        	pgc_id_1 = p_vts->vts_ptt_srpt->title[i_ttn - 1].ptt[0].pgcn;
        	pgn_1 = p_vts->vts_ptt_srpt->title[i_ttn - 1].ptt[0].pgn;
        	p_pgc = p_vts->vts_pgcit->pgci_srp[pgc_id_1 - 1].pgc;
        	
        	i_title_start_cell =
        	    i_start_cell = p_pgc->program_map[pgn_1 - 1] - 1;
        	i_title_start_block =
        	    p_pgc->cell_playback[i_start_cell].first_sector;
//printf("\n");
//printf("cell--playback_time[%d]\n",dvd_playback_time(&p_pgc->cell_playback[i_start_cell].playback_time));
//printf("pgc--playback_time[%d]\n",dvd_playback_time(&p_pgc->playback_time));
        	...
        	{
        	    i_title_blocks += p_pgc->cell_playback[i].last_sector -
        	        p_pgc->cell_playback[i].first_sector + 1;
        	}
#if 0
        	printf("title %d vts_title %d pgc %d pgn_1 %d "
        	         "start %d end %d blocks: %d\n",
        	         ii + 1, i_ttn, pgc_id_1, pgn_1,
        	         i_title_start_block, i_title_end_block,
        	         i_title_blocks );
#endif
        	if(i_title_blocks > max_title_blocks)
        	{
        		max_title_blocks = i_title_blocks;
        		//printf("----------ii[%d]\n",ii);
        		dvd_title = ii;
        	}
        	if( p_vts ) ifoClose( p_vts );
...
printf("dvd_title[%d]\n",dvd_title);
    ...
    if(!vts_file) {
    	...
    	int num_audio,playback_time;
		ttn = tt_srpt->title[dvd_title].vts_ttn - 1;
		num_audio = dvd_number_of_audios(vts_file, ttn);

		if (num_audio == 0)
	    	cond_fail = 1;

		pgc = vts_file->vts_pgcit ? vts_file->vts_pgcit->pgci_srp[ttn].pgc : NULL;
		playback_time = dvd_playback_time(&pgc->playback_time);
		if (!(pgc && playback_time > 400))
	    	cond_fail = 1;
    }
	...
	for (ii = 0, jj = 0; ii < tt_srpt->nr_of_srpts; ii++)
	{
...
#if 0
	    printf("nr_of_srpts: %d, open %d nr_of_ptt(chapter): %d title_set_nr: %d, vts_ttn: %d\n",
	    ...
	    if (tt_srpt->title[ii].nr_of_ptts > jj && tt_srpt->title[ii].vts_ttn == 1)
	    {
